...
and the **393 tests that need no running stack** (computed by `scripts/dev/common/stack_free_tests.py`,
...
same case: where `scripts/TESTS/` exists and `scripts/dev/common/tool.sh` cannot run, the push is refused.
...
- **The push then meets clause 3, not clause 4.** The report `deliver` commits names the tree it
  certified, and everything added after it — that report, the plan renamed `(Merged)`, the kanban
  card moved — is bookkeeping. So the gate below takes the cheap acceptance rather than re-running
  the whole suite over a paragraph, and it does so having read a run that covers the delivery's own
  documentation.
