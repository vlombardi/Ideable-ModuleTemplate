...
`docs/`, no `scripts/dev/master/`, no `scripts/TESTS/`.
...
"this tree authors the framework" — applied per assertion: the half that only the maintainer's tree
can make is skipped there, naming what is absent and where it *is* asserted, and every half a
received project can still get wrong keeps running. **Narrowed, never relaxed**, and the
