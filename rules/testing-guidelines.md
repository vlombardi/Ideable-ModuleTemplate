...
**`scripts/dev/common/run_enabled_tests.sh`** (invoked directly, or via the
...
**The suite is never skipped and never scoped to what changed.** The dev-cycle router skips a node
whose input the executing sub-set's diff cannot touch — a sub-set that changed no build input runs
no `BuildDeploy` (`rules/implementation-plan.md` § *Overall view*) — and `Testing` is deliberately
outside that treatment. Skipping a build removes work whose **input** is provably absent; scoping
the tests would remove a check whose **output** was assumed, which is the one thing a test run is
for. The whole suite is a small fraction of a cycle and has caught regressions in modules the
change never named, so the trade is not close.
...
* **Runner**: `scripts/dev/common/run_enabled_tests.sh` runs, per enabled module, both `pytest`
...
* **Each per-module report contains**, in order:
  ...
  - **What was tested** — a table with one row per test: its result badge, a
    ...
    **This table is a machine-read contract, not only a human one.** `scripts/dev/common/dev_cycle.py`
...
- `config/test-users.yaml` (per module) — the e2e accounts, `e2e_`-prefixed so they are
  ...
  because they carry a known password: `E2E_TEST_USERS_ENABLED=true` and
  ...
  **Where the flag lives, and what it actually defaults to.** The *code* default is `false` — an
  unset `E2E_TEST_USERS_ENABLED` creates nothing — but the framework **ships it `true`**, in
  `modules/host_app/.env.config` and in `project.env.config.example`, so a project that changes
  nothing has it on. The gate that is genuinely closed by default is the other one: `E2E_TEST_PASSWORD`
  ships **empty**, and no password means no accounts however the flag reads. Set the flag in
  **`project.env.config`**, not in `modules/host_app/.env.config` — the latter is framework-owned and
  force-synced, so an edit there is replaced on the next sync, while `project.env.config` is merged
  first and wins.

  **Half the pair is the mistake that is easy to make**, because the password is the half that looks
  like the work. A password set with no flag creates nothing and used to do it silently, surfacing
  much later as an Authentik consent screen the suite cannot get past — two env layers from the
  cause. Both gates now report that combination by name:
  `bootstrap_authentik.py::_test_users_allowed` and `authz_seed.py::test_users_allowed`.
...
- It also **reinstalls a frontend's dependencies when `reusable.ui/` is newer than the
  copy of `@ideable/ui` in them**. Those trees persist between runs inside the dev tools
  container and `--install-links` copies the shared library rather than linking it, so
  without this the static gate and the dev server the UI suite drives would both measure
  a snapshot of the widget library instead of the one in the checkout.
...
the shape the remote receives it** — a renamed module and slug, a host_app that arrives as pre-built
images and therefore carries **neither sub-module SOURCES nor sub-module TESTS**, no `docs/`, no
`scripts/dev/master/`, no `scripts/TESTS/`.
`scripts/dev/master/verify_remote_shape.sh` is the mechanism: it stages that shape and runs the
tests against it, before anything is pushed.

**The shape is not only what the tests see — it is what the deploy does.** That verifier runs a
staged project's *tests* and never its *deploy*, and the deploy is where the two trees differ most:
here host_app is enabled `local` and built from source, while every module project consumes it, so
the whole consumed-host_app branch of `build_and_deploy.py` runs in no test this repository has ever
had and in every real project. A branch reached only outside this repository is verified by
deploying a staged project that consumes host_app —
`scripts/TESTS/test_the_consumed_hostapp_deploy_lands_a_site.py`, which calls
`deploy_remote_hostapp()` against a staged tree and asserts on the site it produces. Nothing is
started: what is checked is the files a site receives, which is what that branch produces.
...
**So a check finds its subject by STRUCTURE, never by the reference module's name.**
`modules/module_template/…` names nothing in a project whose module carries its own name, and the
failure does not present as "this test cannot find its subject" — it presents as the module being
wrong. Measured on two of them: one read a guard that was absent and then reported the module's own
shipped `playwright.config.ts` as instructing a retired variable; the other said a force-synced
document "does not exist", which reads as a file that failed to ship. Five assertions, one literal
path, and no remote could have done anything about any of it.

Discover what the check examines — `modules/*/frontend/TESTS/playwright/playwright.config.ts`, or
every module directory that holds the entry being checked — which is also the question the check
actually asks (*which trees have this?*) rather than the adjacent one (*what is this module
called?*), and it covers a project with two modules where a single name cannot. **Carry a vacuity
guard with it**: discovery that finds nothing turns the file into a check that passes by measuring
nothing, which is worse than the failure it replaced. Deriving is the first answer and skipping is
the second — a test that *could* find the module and is skipped instead hides coverage a received
project should have.

**A test whose subject is a maintainer-only path narrows; it never asserts that path exists.**
`scripts/dev/master/` and `scripts/TESTS/` are the two paths a module project does not receive, so
an assertion that either is present is a statement about the authoring tree, and in a remote it
turns the shipped shape working correctly into a red suite. The form is a documented predicate —
"this tree authors the framework", spelled `(_REPO / "scripts" / "dev" / "master").is_dir()`, since
that is the one tree the template push never copies — applied per assertion: the half that only the
maintainer's tree can make is skipped there, naming what is absent and where it *is* asserted, and
every half a received project can still get wrong keeps running.

**Keep citing the test that enforces your sentence, even in a rule that ships.** A rule names its
enforcing test by path (`rules/implementation-plan.md` § *Enforced, not aspirational*), and most of
those live in `scripts/TESTS/`, which no module project receives — so a checker reading shipped
prose for dead pointers reports them, and the tempting fix is to delete the citation. Do not: the
sentence would lose the one thing that stops it eroding. `test_specs_name_only_paths_that_exist.py`
carries `ALLOWED_ABSENT_TREES` for this — whole trees a received project was never given, excused
there and still resolved here, so a typo in a cited path fails in the tree that can fix it.

**A curated LIST is the same shape as a maintainer-only path.** An allow-list, an exemption
register or any set a maintainer prunes is curated against the corpus of the tree that maintains it,
so "every entry still matches something" is an authoring-tree claim: a received project holds a
subset of that corpus — no `docs/`, a host_app skeleton, its own module — and would report entries
as stale that it has no standing to delete, the file being framework-owned. Narrow the *staleness*
half on the same predicate and leave the list's real work — the patterns, the exemptions themselves —
running against every document the project does have. **Narrowed, never relaxed**, and the
expectation is pinned rather than read off the tree being checked: an expectation derived from the
directory it examines passes for a deleted directory.
...
- **Destructive setup goes on a scratch database**, never the deployed one — see
  `scripts/dev/common/schema.sh`, which creates and drops throwaway databases for exactly this reason.
...
`scripts/dev/common/run_enabled_tests.sh` runs **`ruff`**, **`mypy`** and **`tsc --noEmit`** before any
