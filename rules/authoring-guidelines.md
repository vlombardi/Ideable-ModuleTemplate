...
- **Module‑specific** (module‑maintainer‑owned): the remaining files under `SPECS/`, `backend/SPECS/`, `frontend/SPECS/`, `database/SPECS/` (e.g. `module-ui-specs.md`, `module-bl-specs.md`, `database/SPECS/seed.sql`).
...
**Skills** — `.agents/skills/<name>/` is the **one and only** copy (framework‑owned, synced). `.claude/skills` and `.kiro/skills` are **symlinks** to it, so editing the `.agents/` copy updates them automatically. Devin needs no entry of its own: `.agents/skills/<name>/SKILL.md` is the first and recommended path in its [documented skill search](https://docs.devin.ai/product-guides/skills), so it reads the canonical files directly. Never add a second copy — `.devin/skills` was one, kept "byte‑identical" by a manual `shasum` check, and it was measured **stale in two skills** when it was removed on 2026‑08‑31; a duplicate policed by human memory drifts, and Devin scans both paths, so it was reading the outdated instructions. Run `scripts/dev/common/update_skills.py --validate` to check the topology:
...
- All three layers are **framework‑owned** and reach remote modules via the standard sync: `scripts/dev/master/push-updates-to-module_template-repo.sh` (maintainer repository only — `scripts/dev/master/` is never synced, so it is absent from your project) → `scripts/dev/module/sync-template-updates.sh`. The folders `rules/`, `.agents/`, and the `ideable-framework-specs/` spec subfolders are in that script's `is_infrastructure` allowlist and in `modules/<MODULE>/SPECS/ideable-framework-specs/infrastructure-file-list.md`.
