...
- **`modules/host_app/.env.config` is the one exception, and it is not a source of truth for anybody but the framework.** host_app is framework-owned in its entirety, and that file is **force-synced**: `scripts/dev/module/sync-template-updates.sh` replaces it **whole**, it is not key-merged, and an edit made there is gone at the next sync. That is deliberate and it is the only route a *structural* default has to a project that already exists — a service name, a port, the database authentik connects to. A merge would preserve the project's existing value, which is precisely the value that has to change. **Put your own values in `project.env.config`**, which is merged before every module's env and whose keys win (§ *Environment Variable Strategy*), so they override the framework's default and survive every sync. The sync announces the replacement, naming the file and the remedy, rather than reporting it as one more `[updated]` line.
