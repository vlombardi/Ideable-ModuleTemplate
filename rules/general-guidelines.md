...
- `modules/` folder: contains everything needed to build, deploy and run a module. In particular:
   - `<module_folder>`: there is a `<module_folder>` for each module, containing:
      ...
      - `docker-compose.yml`: the compose file for the module. During deployment, `scripts/build_and_deploy.py` copies it to `deployment_root/modules/<MODULE>/docker-compose.yml` and then merges all enabled modules' compose files into a single `deployment_root/docker-compose.yml`.
...
Each module's compose file is named `docker-compose.yml` and lives in the module's root folder (`modules/<MODULE>/docker-compose.yml`). During deployment, `scripts/build_and_deploy.py` copies it to `deployment_root/modules/<MODULE>/docker-compose.yml` and merges all enabled modules into `deployment_root/docker-compose.yml`. All compose commands are always run from `deployment_root/` using explicit `-f modules/<MODULE>/docker-compose.yml` flags.
...
- **Image references only**: every service must reference a pre-built image by name via the `image:` key (e.g. `image: ideable/hostapp-backend:latest`). Image names must follow the *Docker image naming convention* below.
- **Docker image naming convention**: image names are derived exclusively from the module's own `APP_SLUG` (defined in `modules/<MODULE>/.env`), never from the project that builds it. This guarantees that any project can build a module and produce the same image name, enabling safe reuse across multi-module deployments:
  - **HostApp**: `${APP_SLUG}/hostapp-<submodule>:latest` (e.g. `ideable/hostapp-frontend:latest`)
  - **All other modules**: `${APP_SLUG}/<submodule>:latest` (e.g. `sra/frontend:latest`)
  - `APP_SLUG` in a module's `.env` **must equal the module's own slug** — never the HostApp slug or any other module's slug. `module-init.sh` enforces this automatically.
...
- **`deployment_root/.env` is auto-generated** by `scripts/build_and_deploy.py` — do not edit it manually.
...
**CRITICAL — Modularity and Decoupling Principle**: All build logic that is specific to a sub-module MUST be defined inside that sub-module and MUST NOT be hardcoded in the global build script (`scripts/build_and_deploy.py`).
...
3. **`scripts/build_and_deploy.py` detects `SPECS/build.sh` automatically**: if `SPECS/build.sh` exists for a sub-module, the script runs it instead of applying generic build logic. No changes to `build_and_deploy.py` are needed.
