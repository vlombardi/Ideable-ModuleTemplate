...
files it wrote (`framework.env` and the regenerated lock) and tags that commit in both git
repositories. A partial publish is a failed publish.

**A project adopts a release the same way**, with `scripts/dev/module/adopt_framework.sh <version>`:
it writes the version and runs the sync that brings that release's files, restoring the file if the
sync fails. The two commands are the same event from each end, which is why they are spelled alike.
