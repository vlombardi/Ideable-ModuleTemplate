...
- **The active plan has a stable name.** While a plan is in flight the router maintains
  `implementation-plans/ACTIVE PLAN - <description>.md`, a **symlink** to whichever file is current,
  repointed at every transition. Everything else about a plan's filename moves — the timestamp is
  re-stamped at each execution and the state is part of the name — which is deliberate and makes the
  plan impossible to cite. This is the one name that holds for the life of the run, so a skill, a
  rule, an editor tab or a shell alias can refer to it.
  ...
  - **Reaching `Merged` removes it.** Its absence is the signal that no plan is in flight; a link
    that survived delivery would claim one forever. The real plan file — or every file, under
    `--keep-history` — is never touched by any of this.
  - It is **not** a plan. `active_plan()` excludes symlinks explicitly, because resolving the alias
    would make the router read and rewrite a plan through it and rename the target out from under
    the link on the first transition.
  - **Anything that scans `implementation-plans/` excludes it the same way, and excludes it before
    ...
    survivors are `stat`ed, read or sorted by mtime. Inside the dev tools container the bind mount
...
- **History mode.** By default a run keeps **one** plan file, renamed onto the execution
  ...
  `scripts/dev/common/dev-cycle.sh` with `--keep-history` instead **keeps every transition's file** — same
...
- **Branch at `Branching`.** A plan owns a dedicated branch named `plan/<description>` — the
  ...
  `NotStarted` on whatever branch the author is on, the first `scripts/dev/common/dev-cycle.sh run` advances
  it to `Branching`, and the next creates and checks out the branch from the current one and
  advances to `Implementing` (`run --auto-advance 2` does both). Nothing is committed before the
...
- **The router answers the agent it runs.** At an LLM node the skill is performed through the
  Claude Agent SDK, and `scripts/dev/common/agent_conversation.py` supplies its `can_use_tool`: a
  ...
  policy — `ask` on a terminal, `deny` without one, so an unattended run neither blocks on a
  question nor grants it. This is § *Decision Making Authority* made mechanical: the agent stops
...
- **Sub-set table (detail)** — one row per sub-set, in execution order, with columns **Description** and
  **State**. See § *Sub-sets* below for how they are chosen. `State` is `—` before a sub-set
  ...
  `scripts/dev/common/dev-cycle.sh` writes both together so they can never disagree.
...
flowchart TB
    NotStarted["Not started"] --> Branching
    Branching --> Implementing
    subgraph cycle ["Sub-set cycle"]
    ...
    style cycle fill:none,stroke:#9e9e9e,stroke-dasharray:4 3

    %% --- state lines (rewritten on every advance): all-but-current = idle, the current = current ---
    class Branching,Implementing,BuildDeploy,Testing,Fixing,Documenting,Committing,Done,Merged,Blocked idle;
    class NotStarted current;
```
````

- **The dashed box is the `Sub-set cycle`.** It groups `Implementing → BuildDeploy → Testing →
  (Documenting | Fixing) → Committing`, the loop § *Sub-sets* says the router runs **once per
  sub-set**. It is entered once, from `Branching`, and `Committing` is its only exit, with two
  arcs: `Next sub-set` back to `Implementing` (the first node of the box) when a later sub-set is
  still pending, and `All sub-sets done` to `Done`. The box is a grouping, not a state: the
  highlight is always on one of the nodes inside it, never on the box.
- **Nodes (dev-cycle states):** `NotStarted` (the plan has been written and nothing has run: no
  branch exists, every sub-set is `—`; the first `scripts/dev/common/dev-cycle.sh run` leaves it), `Branching`
...
graph. `scripts/dev/common/dev-cycle.sh` enforces this deterministically: after the
`Committing` step it reads the plan branch's commits (`main..HEAD`), attributes each to the **scopes**
whose paths it touched, and writes the matching Repos rows itself — the same
...
|---|---|---|
| `ideable-implement-specs` | `Implementing` | **Creates** the plan at `NotStarted` (Purpose, Overall view, Main table, Detailed summary), then runs `scripts/dev/common/dev-cycle.sh run --auto-advance 2` so the router performs `Branching`; drives the `Impl` column (🔲→🔄→✅) and the Repos `Implementation` cell as it works. |
