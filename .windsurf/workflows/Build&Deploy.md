...
The canonical implementation of this workflow is the script `scripts/build_and_deploy.py`. **Always run that script** rather than performing manual build/copy steps, unless the script does not yet support the module being built (see Step 4).
...
|---|---|---|
| `SOURCES/Dockerfile` exists | **Docker image** | Image stored in local Docker registry; no `DIST/` required |
| `SOURCES/Dockerfile` exists AND non-Dockerfile files are present | **Docker image + file artifacts** | Image in registry; non-Dockerfile files copied to `DIST/` |
...
7. Copies `docker-compose.yml` and `.env` to `deployment_root/`
8. Generates `deployment_root/start.sh` and `deployment_root/stop.sh`
...
6. No `Dockerfile` or `SOURCES/` path references exist anywhere inside `deployment_root/`
7. No hardcoded values in compose files where an env var is available
8. All Docker images referenced by `docker-compose.yml` exist in the local Docker registry (`docker images`)
