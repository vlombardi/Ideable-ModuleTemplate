...
1. Create the plan file in `implementation-plans/` using the naming convention in that rule
   ...
   complete (steps 2–3), run `./scripts/dev/common/dev-cycle.sh run --auto-advance 2` — the router advances
   `NotStarted → Branching` (nothing to do), then performs `Branching` (creates and checks out
...
2. Write the **Purpose** chapter (a few sentences on what this run implements) and the
   ...
   **Current step** = `Not started (./scripts/dev/common/dev-cycle.sh run …)`, every sub-set row `—`, and
