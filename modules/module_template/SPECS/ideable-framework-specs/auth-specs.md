...
7. Read the access token from the session store **host_app** wrote, addressed exactly as
   `oidc-client-ts` keys it — `oidc.user:<authority>:<client_id>`, the authority **verbatim**
   (it is the documented Authentik form and it ends in `/`; normalising it means the key matches
   nothing). A module MUST NOT send a token it cannot attribute to the current authority and
   client, MUST NOT send one whose `expires_at` has passed, and where the session cannot be
   identified MUST refuse to guess — one unexpired candidate is the session, several are
   ambiguous. A token belonging to another session is not a worse answer than none; it earns a
   401 that the UI then reports as a denial.
8. A failing `/me` is **not** a denial. A module MUST distinguish *host_app answered and the
   permission is absent* from *the session is invalid* (401) and *authorization is unavailable*
   (503, or no answer at all), and MUST render "not authorized" only for the first. All three
   stay fail-closed on data — the UI hides what it cannot prove the user may do — but only the
   first is a statement about the user, and the other two must say what the user can act on.
...
- Seeding outside a version change requires an explicit operator action, available at a deployed site
  with no redeploy and no rebuild:
  `deployment_root/scripts/authz.sh seed [--module <name>] [--force] [--dry-run]`.
...
- Update `scripts/dev/module/sync-template-updates.sh` and `scripts/dev/master/push-updates-to-module_template-repo.sh` whenever new shared specs are added so that consumer repos automatically receive them.
...
5. **A declared tenancy marker on every model.** `__tenant_scoped__ = True | False`, enforced at
   build time by `scripts/dev/common/check_tenancy_markers.py` (run from
   `scripts/dev/common/validate_modules.sh`). `True` also requires the `tenant_id` column to exist.
...
build, by `validate_cross_tenant_entity_naming` in `scripts/dev/common/validate_modules.sh`: the
...
`scripts/dev/module/sync-template-updates.sh` brings it in.
...
- [ ] Every model declares `__tenant_scoped__`; `scripts/dev/common/validate_modules.sh` fails when one
      does not.
