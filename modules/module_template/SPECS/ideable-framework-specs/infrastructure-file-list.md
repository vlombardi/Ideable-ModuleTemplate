...
- `redeploy.sh`, `start.sh`, `stop.sh`, `status.sh`, `dev-cycle.sh`, `update_backend.sh`,
  ...
  links — including for a project that already holds one of them as a regular file, which differs
  from the template by TYPE and is transferred on that difference — so a project receives seven
  working commands rather than seven copies; the relative target
...
- `modules/host_app/.env.config` — **force-synced: replaced whole, never key-merged**, and the only entry in this section for which that is true. host_app is framework-owned in its entirety and this file holds *structural* defaults — which container authentik's database is, which port a service listens on — so it is the one route such a default has to a project that already exists. A key-merge cannot be that route: it adds missing keys and preserves existing values, and a default that moved is exactly a key the project already holds with the old value. Carried into the template by `push-updates-to-module_template-repo.sh`'s host_app skeleton list and matched by `sync-template-updates.sh`'s `is_infrastructure`; the sync announces each replacement by name rather than as one more `[updated]` line. A deployer's own values belong in `project.env.config`, which is merged first and whose keys win. It is refused by `should_create_missing_infrastructure_file` and by both removal branches (`is_live_env_file`): reverting a configuration to the framework's default is what force-syncing means, while blanking or deleting one is not, and neither would announce itself.
- `modules/host_app/.env.config.example` — the same keys as the file above and, deliberately, **not necessarily the same values**. The example *instructs* — which variables exist, which have to be considered, which are normally changed, and its comments are the guidance; `.env.config` carries what the framework ships. `modules/host_app/TESTS/test_env_config_is_a_framework_default.py` holds the pair to key parity in both directions.
