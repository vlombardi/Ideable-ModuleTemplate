...
- [x] Resolver `scripts/dev/common/module_deps.py`: implicit `host_app`, validation (target-enabled → hard error / `optional` skip, capability-provided, **acyclic**), providers-first topological order, `startup_edges()` kind-filter.
