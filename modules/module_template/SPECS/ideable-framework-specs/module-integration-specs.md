...
the merged `.env.config` namespace is flat), `pull-hostapp-images.sh` reads the same field so the
puller and the deploy can never name the same images differently, and `validate_modules.sh` fails
**before the build** when a remote module declares no tag or declares this repository's own commit.

Two things it is not. It is not `IMAGE_TAG`, which is the tag *this run's build* produces — the
distinction is the whole point, and one variable answering both questions is what made a remote
project pull `hostapp.frontend:<its own commit>`. And there is no fallback to `latest`: a moving tag
...
`scripts/common/check_shared_versions.py` (run by `validate_modules.sh`, so it gates every deploy)
...
- The resolver (`scripts/common/module_deps.py`) validates the enabled set (target enabled,
  ...
  via `scripts/common/validate_modules.sh`.
...
- **Container startup ordering.** A provider declares readiness barriers under
  ...
  `scripts/common/compose_deps.py` **injects** the generated `depends_on` into the per-module
...
- runner:
  - `scripts/master_only/check_module_template_lf_parity.sh`
