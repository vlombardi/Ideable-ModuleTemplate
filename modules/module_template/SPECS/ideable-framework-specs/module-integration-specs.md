...
- Remote modules authorize from host_app: a validated JWT establishes *who* the caller is, and `GET /api/me` answers what they may do and whose data they may touch. The token carries no authorization data.
...
- In maintainer repos where host_app and module_template coexist, module_template must remain independently exportable with:
  ...
  - curated maintainer export flow: `scripts/dev/master/push-updates-to-module_template-repo.sh`

Validation model:
- The canonical validation entry point is `scripts/dev/common/validate_modules.sh`.
...
**CSS loading contract.** A remote module must ensure its compiled stylesheet is loaded when
host_app resolves the module — **not** by relying on the standalone `main.tsx`/`bootstrap.tsx`
CSS import (host_app never executes that chain). The canonical way is a side-effect
`import './index.css'` at the **top of the exposed `./moduleManifest`**, so the expose
carries the stylesheet in its Module-Federation `assets.css` and the MF runtime auto-injects
it on `loadRemote('<slug>/moduleManifest')`. That single stylesheet already contains the
shared `@ideable/ui` layer (`@import "@ideable/ui/styles"`) plus the module's own `<slug>:`
layer and `.<slug>-scope` cascade. host_app **also** auto-injects a module's stylesheet
defensively: after resolving a remote it reads that remote's `mf-manifest.json`
`exposes["./moduleManifest"].assets.css` and injects a deduplicated
`<link data-module-style="<slug>">` (robust to hashed filenames). Cross-module CSS reuse
(loading *another* module's stylesheet) is driven by a `dependsOn` edge with `kind: css`
(see §5.1), using the same injection mechanism.

**Runtime L&F override (no rebuild).** Independently of the build-time chain above, each
frontend's `index.html` appends `config/theme-override.css` at runtime — an unlayered
stylesheet loaded after the compiled bundle, served with `Cache-Control: no-store`. Editing
that file in the deployed `config/` folder (`deployment_root/modules/<slug>/config/`) redefines
the canonical `:root`/`.dark` design-token values and recolors the module and all `@ideable/ui`
widgets live, without rebuilding the image or touching host_app. Because all prefixes resolve
`hsl(var(--token))` at runtime, one override recolors everything. (Adding brand-new utility
classes or swapping bundled icon glyphs still requires a rebuild.)
...
- Each item in `menu_definition` must contain:
  ...
  - optional `authorization_claim` (string): when absent, the menu item is visible by all users; when defined, only users whose `/api/me` permission set contains the exact permission can see the menu item and its entire sub-tree. For menu visibility, the permission must use the `<resource>:menu_access` format (e.g. `BuildingFirstFloor:menu_access`); host_app qualifies each bare value with its item's `module` prefix before matching. Users without that exact permission will not see the item (it is not added to the UI). The name `authorization_claim` is historical — the value is a permission, and it is never read from the token.
...
- optional `authorization_claim` (string): when present, only users whose `/api/me` permission set contains the exact permission (qualified as `<module_slug>.<resource>:<action>`) can see the menu item and its sub-tree
...
- During deployment, `scripts/dev/common/build_and_deploy.py` resolves the module slug for each deployed compose file before generating the merged compose output so the final runtime container names stay unique even though all module env files are merged into `deployment_root/.env.config` and `.env.secrets`.
...
- `scripts/dev/common/build_and_deploy.py` generates a merged `deployment_root/.env.secrets.example` with placeholder values derived from the merged secrets.
...
6. Enforce permissions from host_app, never from the token. A remote backend's `require_permission()` calls `GET /api/me` with the caller's bearer token and caches the answer per token for at most the 60 s revocation SLO; the token itself carries no authorization data (its claims expression is `return {}`). See `auth-specs.md` §3.3.
...
### 4.1) `consumedImageTag` — the release of another module this project runs

A module enabled as **`remote`** in `modules/enabled.md` has its images built and published by
another project, so its tag cannot come from this repository's commit — that names a build no
registry holds. Every such module declares, in its own `modules/<MODULE>/module.json`:

```jsonc
{ "consumedImageTag": "v1.4.0" }
```

`build_and_deploy.py` resolves it into that module's deployed `docker-compose.yml` at generation
time (the same treatment `${MODULE_SLUG}` gets, and for the same reason: the value is per module and
the merged `.env.config` namespace is flat), and `validate_modules.sh` fails **before the build**
when a remote module declares no tag or declares this repository's own commit.

**host_app is the exception, and it is not a special case so much as the general rule applied.** host_app is not a module another team happens to publish: it IS the framework, and which release of it a project runs is what `framework.env` already says. So host_app declares **no** `consumedImageTag` — `validate_modules.sh` fails when one appears there — and its registry and release both come from `framework.lock.json` through `scripts/runtime/framework_version.py`. `pull-hostapp-images.sh` takes no argument, pulls each image by the digest the lock names and tags it under the name the deployed compose uses; `verify_build_identity.py` fails the deploy when a consumed image is not that digest. **Run it as `deployment_root/scripts/pull-hostapp-images.sh`**: the deploy copies the lock and the resolver into the deployment root, so the puller answers from what a site actually holds, and the source copy under `scripts/runtime/` cannot — a script a site runs must not name a path that exists only in a checkout.

Two things it is not. It is not the local `latest` that *this run's build* produces — the
distinction is the whole point, and one name answering both questions is what made a remote
project pull an image nobody had published. And there is no fallback to `latest`: a moving tag
is reported as a warning by `validate_modules.sh` (it resolves, so it cannot be a hard error) but
two different publishes answer to it, and "which build is running?" then has no answer.

`rules/general-guidelines.md` § *A tag for what you build is not a tag for what you consume* is the
binding statement of this contract.
...
### 5.0) Version skew fails the deploy, not the browser

Modules deploy independently, so two of them can declare different `requiredVersion` values for
the same singleton. A singleton loaded twice at incompatible versions fails **in the end user's
browser** — a white screen attributed to the shell rather than to the module that caused it.

`scripts/dev/common/check_shared_versions.py` (run by `validate_modules.sh`, so it gates every deploy)
compares the `requiredVersion` each enabled local module declares in its `rsbuild.config.ts` and
**exits non-zero on disagreement**, naming the dependency and the modules. Align the declarations —
do not silence the check: it is the only place the conflict is visible before a user meets it.

## 3.5) Fault Isolation Contract (`optional`, `loadTimeoutMs`)

A module's failure must cost that module, not the application. Two registry fields govern this,
both emitted by the registry generator and honoured by the shell:

| Field | Default | Meaning |
|---|---|---|
| `optional` | `true` | The shell renders without this module when it fails to load; only its menu entry is missing. Set `false` only for a module the application genuinely cannot run without — that choice makes one module's outage everyone's outage. |
| `loadTimeoutMs` | `10000` | Bound on loading the module's manifest. A dead remote rejects quickly on its own; this exists for the **slow** one, which would otherwise hang the shell's load indefinitely. |

Three layers implement it, and all three are needed — each covers a failure the others do not:

1. **Timeout** (`useRemoteModules`) — the remote never answers.
2. **Caught load failure** — the remote answers with something unusable; the module is dropped and
   the shell continues.
3. **Error boundary** (`RemoteModuleBoundary`, one per remote route) — the remote loads and then
   throws while rendering. Without this, that exception reaches the root and React unmounts the
   whole tree: one module's bug blanks the shell.

## 5.1) Inter-Module Dependency Declaration (`module.json` `dependsOn`)

> **Ideable Framework:** the per-module dependency system. This is about **inter-module**
> dependencies (module A requires module B's API / CSS / widgets / startup ordering), and is
> distinct from `SPECS/dependencies.md`, which tracks **library/image versions**.

A module declares the other modules it depends on in its own `module.json`:

```jsonc
{
  // OPTIONAL — capabilities this module offers others (validates their dependsOn.kinds).
  // Defaults: css=true when the module has a frontend; api=true when backendPort is set.
  "provides": { "css": true, "api": true, "widgets": [] },

  // Modules this module requires, each typed by `kinds`.
  "dependsOn": [
    { "module": "report_generator", "kinds": ["api"], "optional": false,
      "reason": "calls report_generator to render PDFs" }
  ]
}
```

Contract:
- `dependsOn[].module` is the target's `module.json` `name` (the identity used in
  `modules/enabled.md`). `kinds` ∈ `runtime` | `api` | `data` | `css` | `widgets`.
- **`host_app` is an implicit universal dependency** — never declare it.
- A declared dependency is a **prerequisite**: if the target is not enabled, the build/
  validation **fails** with a clear message. Mark an edge `"optional": true` to degrade to a
  warning + skip instead.
- The resolver (`scripts/dev/common/module_deps.py`) validates the enabled set (target enabled,
  capability provided, **acyclic**) and produces a **providers-first topological order** that
  `build_and_deploy.py` uses for build/processing order (host_app first). Run it standalone
  via `scripts/dev/common/validate_modules.sh`.
- **Validation lints** (warn-only, surfaced by `validate_modules.sh`): `provides.api` /
  `provides.widgets` are checked against reality — a declared API must have a backend (a
  `backend/` folder or `backendPort`), and each declared widget must be exposed as `./<Name>`
  in `rsbuild.config.ts`; a **drift lint** flags cross-module usage that doesn't match
  `dependsOn` — using another module's `<slug>:` Tailwind classes or `loadRemote('<slug>/…')`
  without the matching `css`/`widgets` edge, and edges that are declared but never used.
- **`status.sh --deps`** prints the resolved providers-first dependency graph (each module's
  `dependsOn` targets + kinds, plus the implicit host_app edge) from the deployed `module.json`
  files — self-contained, so it works at a deployed site with no build tooling.
- **Container startup ordering.** A provider declares readiness barriers under
  `provides.gates` (e.g. host_app: `database`→`service_healthy`,
  `authentik-bootstrap`→`service_completed_successfully`). At deploy time
  `scripts/dev/common/compose_deps.py` **injects** the generated `depends_on` into the per-module
  *deployed* composes (each dependent's **root, non-healthcheck** entry services), deduped
  against edges already declared. Injecting into the per-module composes (not a separate
  override file) is what makes the ordering survive **both** the build-time merge and the
  standalone runtime `create-merged-configuration.sh`. So a new module that declares a
  dependency gets correct cross-module `depends_on` generated instead of hand-coding another
  module's service names. Only **`runtime`/`api`/`data`** edges gate startup; a `css`/
  `widgets`-only edge is runtime-frontend (Phase 2) and does **not** affect container order,
  though it still counts for build/topological order.
- **Phase 2 (with the CSS-loading spec):** `kind: css` drives host_app injecting the
  provider's stylesheet on mount; `kind: widgets` drives MF remote registration. Note the
  one-prefix-per-build limit — a `css` dependency is resolved by *loading* the provider's
  compiled stylesheet, not by recompiling its classes into the consumer's bundle.
...
- `scripts/dev/common/validate_modules.sh` is the authoritative runner used by build/deploy orchestration.
...
- `scripts/dev/common/build_and_deploy.py` must invoke `scripts/dev/common/validate_modules.sh` after build succeeds and immediately before any deployment copy or compose-generation step.
...
**Concrete example (`my_module` pattern):**

For a module named `my_module` that exposes a sub-remote, the pattern fills in as:
- Route prefix: `/my-module`
- UI origin: `MY_MODULE_SUBREMOTE_ORIGIN` (frontend on port 3002)
- API origin: `MY_MODULE_SUBREMOTE_API_ORIGIN` (backend on port 7050)
- A downstream module references `MY_MODULE_SUBREMOTE_API_ORIGIN` in its proxy code
...
**Example (`my_module` pattern):**

For a concrete example using a module named `my_module` (Module A) consumed by a
downstream module (Module B):
- Module A = `my_module`, Module B = a downstream module
- UI origin var = `MY_MODULE_SUBREMOTE_ORIGIN` (points to frontend on port 3002)
- API origin var = `MY_MODULE_SUBREMOTE_API_ORIGIN` (points to backend on port 7050)
- Route prefix = `/my-module`
...
2. Read the caller's permissions from host_app `GET /api/me` (the token payload carries none).
...
- Remote module frontend build args must include only module-scoped variables (for example `VITE_<SLUG>_API_URL`).
...
4. **Response schema** for each entity:
   ...
   - Example: `class <Entity>Version(BaseVersion): ...`
...
The deploy script (`scripts/dev/common/build_and_deploy.py`) derives a per-module `<SLUG>_LOG_LEVEL` environment variable for every enabled module based on its mode in `modules/enabled.md`:
