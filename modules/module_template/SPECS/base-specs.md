...
- **Every model declares `__tenant_scoped__`.** A build-time gate
  (`scripts/dev/common/check_tenancy_markers.py`) fails the build otherwise, because the alternative is a
...
- **Test**: `rules/testing-guidelines.md` — the runner `scripts/dev/common/run_enabled_tests.sh` is the
