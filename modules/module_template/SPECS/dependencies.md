...
This file is the single source of truth for all dependencies and component versions of the module_template module and its sub-modules.
...
| Depends on | Reason |
|------------|--------|
| host_app | Authentication/authorization context via Authentik JWT claims, project-scoped Docker network `ideable_network` (namespaced per deployment) |
...
| Recharts | 3.9.1 | Charting library for the `TimeSeriesChart` framework widget. Imported only via the (dev-only) Widget Examples page or a real time-series entity page, so it is tree-shaken out of published builds when unused (`WIDGET_EXAMPLES=false`). Version pinned to match host_app. |
...
| prometheus-fastapi-instrumentator | 7.0.0 | Prometheus metrics for FastAPI: per-endpoint latency histogram and status counters, plus the custom pool / JWKS / audit collectors in `app/metrics.py` (the observability work). Pulls in `prometheus_client`. |
