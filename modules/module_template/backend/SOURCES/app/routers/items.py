...
def get_item_history(
    ...
    if not versions:
        all_rows = [make_synthetic_creation_row(
        ...
        all_rows = merge_and_sort_history(field_versions, association_rows, startup_at)

    # Apply server-side sort (default: timestamp desc)
    reverse = (sort_order or 'desc').lower() != 'asc'
    if sort_by == 'operation_type':
        all_rows = sorted(all_rows, key=lambda r: r.operation_type, reverse=reverse)
    elif sort_by == 'actor':
        all_rows = sorted(all_rows, key=lambda r: (r.actor or ''), reverse=reverse)
    else:
        all_rows = sorted(all_rows, key=lambda r: (r.timestamp or startup_at), reverse=reverse)

    # Column filtering (audit table): applied to the materialised list before pagination so
    # `total` reflects the filtered set.
    all_rows = apply_history_filters(
        all_rows, actor=actor, operation_type=operation_type, timestamp=timestamp,
    )

    total = len(all_rows)
    page_rows = all_rows[skip: skip + limit] if limit > 0 else all_rows[skip:]
    pages = (total + limit - 1) // limit if limit > 0 else 1
