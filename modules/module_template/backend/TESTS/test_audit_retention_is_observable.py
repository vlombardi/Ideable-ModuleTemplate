...
Retention is a TimescaleDB policy added by `scripts/runtime/audit-retention.sh`, and it is
...
def test_the_retention_job_still_gates_on_the_variable():
    ...
    job = Path(__file__).resolve().parents[4] / "scripts" / "runtime" / "audit-retention.sh"
    if not job.is_file():
