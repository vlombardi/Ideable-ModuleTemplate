...
The full procedure — design → model → migration → verify, with `scripts/dev/common/schema.sh` — is in
...
2. Generate the migration (`scripts/dev/common/schema.sh migration <module> -m "…"`) and read it before
   committing — autogenerate proposes, it does not decide.
3. Regenerate `database/SPECS/schema.sql` so the rendering matches
   (`scripts/dev/common/schema.sh schema-sql <module>`).
