...
**`schema.sql` is generated, never applied.** It is `scripts/dev/common/schema.sh schema-sql` output: the
...
`scripts/dev/common/schema.sh` implements each phase as a subcommand. All of them work on **throwaway
...
`scripts/dev/common/validate_modules.sh` enforces the same rule **statically** at build time, where no
...
contract — running `scripts/dev/module/sync-template-updates.sh` is what brings it in.
...
**Enforced, not merely written down.** `scripts/dev/common/check_app_db_role.py` runs inside
...
- nothing in the compose file creates the role — the bootstrap block predates this contract, and
  the fix is `scripts/dev/module/sync-template-updates.sh`;
