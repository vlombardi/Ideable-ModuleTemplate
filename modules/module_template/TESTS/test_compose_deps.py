...
GEN = PROJECT_ROOT / "scripts" / "dev" / "common" / "compose_deps.py"
...
def test_generator_source_exists() -> None:
    assert GEN.is_file(), "scripts/dev/common/compose_deps.py must exist (framework-owned)"
...
def test_generator_runs_on_this_project() -> None:
    ...
    md_spec = _il.spec_from_file_location("module_deps", PROJECT_ROOT / "scripts" / "dev" / "common" / "module_deps.py")
