...
# FRONTEND_URL is defined below, once the module's slug is known — see the note there.
...
# The frontend SERVICE, not the user-facing shell.
#
# These assertions are about the frontend container serving its own files — index.html, the static
# assets, the MF manifest, the SPA route — with no login and no proxy involved. That is a different
# address from `<SLUG>_FRONTEND_URL`, which Playwright reads as the SHELL: the URL a browser opens,
# necessarily the public hostname because Authentik's redirects and cookies are bound to it.
#
# One name was serving both meanings. Overriding it so this file could reach the service inside the
# dev tools container pointed Playwright at a service too, its login failed, and the entire UI suite
# ran zero tests while the run still reported 0 failed. So the service address gets its own name,
# and falls back to the old one so nothing changes for anyone who has not set it.
_SLUG_ENV = SLUG.upper()
FRONTEND_URL = (
    os.getenv(f'{_SLUG_ENV}_FRONTEND_SERVICE_URL')
    or os.getenv(f'{_SLUG_ENV}_FRONTEND_URL')
