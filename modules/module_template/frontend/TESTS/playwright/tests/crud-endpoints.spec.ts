...
// Page size for the fallback list-read below. Must stay within the API's MAX_PAGE_SIZE
// (backend `crud.MAX_PAGE_SIZE`, default 200) or the request is rejected with 422.
const READ_PAGE_SIZE = 100

function bearerFromSession(): string {
  const captured = JSON.parse(fs.readFileSync(path.join(here, '..', 'auth', '.auth', `${DEFAULT_PERSONA}.json`), 'utf8'))
...
/** Order the discovered resources parents-first, using the module's own schema.
 *
 * This is what `parseEntityGraph` is FOR, and until now nothing called it: its only invocation was
 * its own unit test, against an inline fixture. So the FK ordering `rules/testing-guidelines.md`
 * calls mandatory ordered nothing, and a child resource whose create needs a parent id was reported
 * SKIPPED — indistinguishable, in the report, from one that genuinely cannot be created
 * generically.
 *
 * A resource the graph does not know keeps its discovered position: an unknown table is not a leaf,
 * it is an unknown, and inventing an order for it would be worse than leaving it alone.
 */
function orderParentsFirst<T extends { collection: string }>(resources: T[]): T[] {
  const schemaPath = path.resolve(here, '../../../../database/SPECS/schema.sql')
  if (!fs.existsSync(schemaPath)) return resources
  const { createOrder } = parseEntityGraph(fs.readFileSync(schemaPath, 'utf8'))
  if (!createOrder.length) return resources
  // `/items` -> `items`; matched by suffix so a slugged table (`<slug>_items`) matches too.
  const rank = (collection: string) => {
    const name = collection.replace(/^\/+/, '')
    const i = createOrder.findIndex((t) => t === name || t.endsWith(`_${name}`) || name.endsWith(t))
    return i === -1 ? Number.MAX_SAFE_INTEGER : i
  }
  return [...resources].sort((a, b) => rank(a.collection) - rank(b.collection))
}
  ...
  test('CRUD round-trip for every discoverable resource', async () => {
    ...
    const ordered = orderParentsFirst(resources)
    if (ordered.map((r) => r.collection).join() !== resources.map((r) => r.collection).join()) {
      log(`FK order (parents first): ${ordered.map((r) => r.collection).join(', ')}`)
    }

    let roundTrips = 0
    for (const { collection, updateVerb } of ordered) {
      ...
      await test.step(`READ ${collection} id=${id}`, async () => {
        ...
        // The cap, not a guess: `limit: '500'` exceeded the API's MAX_PAGE_SIZE (200), so this
        // fallback was answered 422 every time it fired and could never rescue the read it
        // exists for. Pinned by frontend/TESTS/test_e2e_cleanup_pages_within_the_api_cap.py.
        if (!found.ok())
          found = await api.get(url, { params: { id: String(id), limit: String(READ_PAGE_SIZE) } })
    ...
    log(`completed ${roundTrips}/${ordered.length} full CRUD round-trip(s)`)
  })
})
