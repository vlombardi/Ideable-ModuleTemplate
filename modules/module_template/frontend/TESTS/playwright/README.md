...
`E2E_TEST_USERS_ENABLED=true` and `IDEABLE_EXECUTION_MODE != prod`
(refused in production, flag or no flag). The framework **ships the flag `true`** — the `false` is
the code's default for an unset variable, not what a project receives — so the gate that is closed
by default is the password: `E2E_TEST_PASSWORD` ships empty, and no password means no accounts. Set
the flag in `project.env.config`; `modules/host_app/.env.config` is force-synced and an edit there
does not survive. Setting only the password creates nothing, and both gates now say so by name
rather than refusing in silence.

Give each persona a tenant: tenant scoping fails closed,
...
The framework runner `scripts/dev/common/run_enabled_tests.sh` runs the stack-free `npm test`
