...
export async function loginAndCapture(
  ...
  try {
    ...
    if (onLoginForm) {
      ...
      // Advance any remaining stages. There is at least one more for a first-time login: the
      // provider uses EXPLICIT consent, so a brand-new account is asked to authorize the
      // application. `sadmin` had consented long ago, which is why driving the suite as sadmin hid
      // this — the first run with dedicated personas stalled on a screen the harness had never
      // seen. Bounded, and it stops as soon as the flow is left.
      for (let stage = 0; stage < 5; stage += 1) {
        const left = await page
          .waitForURL((u) => !u.toString().includes('/if/flow/'), { timeout: 8_000 })
          .then(() => true)
          .catch(() => false)
        if (left) break
        // Consent renders "Continue"; other stages render their own primary button.
        const advanced = await page
          .locator('button:visible, input[type=submit]:visible')
          .filter({ hasText: /continue|authorize|allow|accept|submit|next|log ?in|sign ?in/i })
          .first()
          .click({ timeout: 5_000 })
          .then(() => true)
          .catch(() => false)
