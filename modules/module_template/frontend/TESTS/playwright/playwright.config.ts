...
// The sanctioned test path is scripts/dev/common/run_enabled_tests.sh (directly or via the
// ideable-test-and-fix skill) — the only path that writes TEST_REPORTS/, and the only run the
// pre-push hook accepts as evidence. It exports IDEABLE_TEST_RUNNER=1. A bare
// `npx playwright test` records nothing, so it hard-fails here at config load — unless the
// developer explicitly opts in with IDEABLE_UNRECORDED_RUN=1 (loud warning; still nothing
// recorded). The opt-in is named for what it COSTS rather than how it is done: it was
// IDEABLE_ALLOW_DIRECT, which described the mechanism and left the consequence to be discovered.
// Framework-owned; force-synced. See rules/testing-guidelines.md § "How tests must be run".
{
  ...
  const unrecorded = process.env.IDEABLE_UNRECORDED_RUN === '1'
  const renamedFrom = process.env.IDEABLE_ALLOW_DIRECT === '1'
  if (!viaRunner && !unrecorded) {
    console.error(
      renamedFrom
        ? '\nERROR: IDEABLE_ALLOW_DIRECT was renamed to IDEABLE_UNRECORDED_RUN — it described\n' +
          'how the run was launched, not what it costs.\n' +
          '    IDEABLE_UNRECORDED_RUN=1 npx playwright test\n'
  ...
  if (!viaRunner && unrecorded) {
    console.warn(
      ...
      '(IDEABLE_UNRECORDED_RUN=1). Nothing is recorded, so this run cannot gate a push.\n',
    )
  }
}
// -------------------------------------------------------------------------------------
