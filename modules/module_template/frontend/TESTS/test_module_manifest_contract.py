...
def test_moduletemplate_specs_define_remote_lf_contract() -> None:
    ...
    # Assert the L&F contract INTENT via stable substrings, not brittle full sentences: the
    # wording resolves L&F to the shared @ideable/ui library (single source of truth) rather
    # than "host_app", but the guarantees (parity by default, opt-in module-scoped overrides,
    # no host-global mutation) must remain present. See look-and-feel-branding.md.
    assert "Default behavior must match" in ui_specs
    assert "Module-specific L&F customizations are opt-in" in ui_specs
    assert "scoped to the module root only" in ui_specs
    ...
    assert "Default mode (mandatory): module_template pages inherit" in base_specs
