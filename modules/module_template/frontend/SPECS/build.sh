...
# The tag comes from build_and_deploy.py (the commit the build is from). No :latest and no
# default: an untagged image cannot be rolled back to.
IMG="${SLUG}.frontend:${IMAGE_TAG:?IMAGE_TAG must be exported by build_and_deploy.py — images are never tagged :latest}"
