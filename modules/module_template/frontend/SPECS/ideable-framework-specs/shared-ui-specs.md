...
In order to provide a homogeneous and consistent user experience, this module should follow the same UI patterns used by the host_app, and use the same visual tokens (colors, radii, typography scale) as the host_app.
This purpose is achieved by adopting the same `shared-ui-widgets-specs.md` specification file and by referencing the same CSS classes, whose actual definition will be available in the execution context of the host_app.
...
- Complete list of `hostapp:` prefixed CSS classes organized by category (colors, typography, spacing, layout, sizing, borders, effects, interactive states, animations, accessibility)
...
- Default behavior must match host_app L&F and widget interaction patterns.
...
- `authorization_claim` (string): gates menu visibility based on JWT claims. When present, only users whose token includes the exact permission string can see the item and its sub-tree.
...
- `modules/module_template/frontend/SOURCES/src/hooks/useUnsavedChangesGuard.ts`
- `modules/module_template/frontend/SOURCES/src/components/UnsavedChangesDialog.tsx`
...
- `modules/module_template/frontend/TESTS/test_i18n_contract.py` verifies i18n coverage and must pass before releasing updates.
