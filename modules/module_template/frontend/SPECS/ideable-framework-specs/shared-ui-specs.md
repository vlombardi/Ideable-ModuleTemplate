...
- `modules/module_template/frontend/TESTS/test_i18n_contract.py` verifies i18n coverage and must pass before releasing updates. It requires the keys the module's own code resolves, and **reports** — never forbids — every library string the module overrides, naming separately any key copied verbatim, which overrides nothing.
