...
This document catalogs the framework's semantic CSS class tokens (colors, typography, spacing, component recipes) so all modules achieve a consistent Look & Feel (L&F). The token names below are listed with the `hostapp:` prefix for readability, but the **class recipes and their design-token definitions are owned by `@ideable/ui`** (`reusable.ui/`) — the single source of truth for UI and L&F — not by host_app.

## Important Notes

- **Source of truth:** shared widgets/primitives and their styling are defined once in `@ideable/ui` and authored with the neutral **`ideable:`** prefix, shipped as static CSS in `reusable.ui/styles/compiled.css`. Each consumer keeps its own prefix for *its own* markup: `hostapp:` (host_app), `template:` / `${APP_SLUG}:` (modules). Substitute your prefix for the `hostapp:` shown below.
- **Canonical design tokens** (the actual color/radius values every prefix resolves to) live in `reusable.ui/styles/base-tokens.css` (`:root` + `.dark`); the `.ideable-scope` cascade and `data-lf` parity/branding live in `reusable.ui/styles/tokens.css`. host_app does **not** define these values — it consumes them via `@import "@ideable/ui/styles"`.
- **Rebrand by overriding token *values*, never class names.** Build-time per module: `--ideable-module-*` / `--<slug>-module-*` under `data-lf="module"`. Runtime, no rebuild and no host_app access: edit `config/theme-override.css` in the deployed folder (served `no-store`, loaded after the bundle, wins the cascade).
- To override a specific class in your own markup, use a new class with the same name under a more specific selector prefixed with your module's CSS prefix.
...
## Runtime theming (no rebuild) and its limits

- **Compiled utilities are baked at build time**, but every color/radius utility resolves `hsl(var(--token))` at runtime — so the *token values* are changeable live.
- **Runtime override (supported):** drop a `config/theme-override.css` in the deployed folder (`deployment_root/modules/<module>/config/`). It is served with `Cache-Control: no-store` and appended after the compiled bundle by `index.html`; being an unlayered stylesheet it wins the cascade and recolors chrome and all `@ideable/ui` widgets. Redefine `:root { --… }` and `.dark { --… }` there. No image rebuild, no host_app access.
- **Also runtime-swappable:** logo/favicon (`config/favicon.png`), login background (`config/login_bg.png`), and `config/home.html` — replace the file in the deployed `config/` folder.
- **NOT runtime-changeable:** which utility classes exist (adding a brand-new utility needs a rebuild), and bundled icon glyphs (lucide, compiled into JS).
- **One-time activation:** the `index.html` + nginx hooks that load the override ship in the image, so a single frontend image build is required once; thereafter L&F edits are fully runtime.
