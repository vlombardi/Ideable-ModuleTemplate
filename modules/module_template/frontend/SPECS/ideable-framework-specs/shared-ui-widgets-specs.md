...
For each main entity defined in the backend (e.g., Users in host_app, Items in a module), there must be a page in the frontend that allows the user to:
...
- Must include an audit trail row action gated by the `audit_trail:view` permission claim
...
2. The **active tab's audit table** — a sortable, filterable, paginated table using the same
   `ServerDataTable` infrastructure as regular entity tables, with exactly four columns:
