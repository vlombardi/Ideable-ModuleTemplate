...
- The frontend consumes the shared **`@ideable/ui`** package (repo-root `reusable.ui/`) via a `file:` dependency. Because a repo-root sibling is outside the SOURCES-rooted Docker build context, both build paths provide it as the **`ideable_ui` named BuildKit build context** (`--build-context ideable_ui=<repo>/reusable.ui`): the local build via **`SPECS/build.sh`** (auto-run by `build_and_deploy.py` in place of the generic build), and the registry-publish path via `push_module_images_to_registry.py`'s `buildx` call — so both stage `@ideable/ui` identically. The Dockerfile `COPY --from=ideable_ui`s it to `./.ideable-ui` and installs with **`npm ci --install-links`** — from the **committed `package-lock.json`**, so rebuilding a commit rebuilds the same versions (the reproducible-build work). Nothing is rewritten at build time: `package.json` declares `@ideable/ui` as `file:./.ideable-ui` permanently, and a developer's clone resolves that same path through **`SOURCES/.ideable-ui`, a tracked symlink** to the repo-root `reusable.ui` (excluded from the build context by `.dockerignore`, since it dangles there; the `COPY` above creates the real directory). The path must stay `./.ideable-ui` and must not be rewritten: npm **normalises `file:` paths when it writes the lock**, so a depth-bearing path like `file:../../../../reusable.ui` produces a different lock in the image than on the host and `npm ci` then refuses. `./.ideable-ui` has no depth to normalise away. `node_modules`/generated files are excluded via `reusable.ui/.dockerignore` and each frontend's `SOURCES/.dockerignore`; `@ideable/ui`'s runtime deps install **fresh inside** the image (never copy host `node_modules` — Tailwind/rspack native binaries are platform-specific).
...
## Runtime image and serving port (mandatory)

The frontend image runs **`nginxinc/nginx-unprivileged:alpine` as uid 101** with `read_only: true`
and a `tmpfs` at `/tmp` (the non-root image work). Two things follow, and both have bitten:

- **It serves on 8080, not 80.** uid 101 cannot bind a privileged port. The port appears in *five*
  places that must agree: `nginx.conf`'s `listen`, the Dockerfile's `EXPOSE`, the compose
  healthcheck, the container side of `*_FRONTEND_PUBLISH`, and Traefik's upstream URL. A partial
  move leaves the container reporting **healthy** while every proxied request is a 502 — the
  healthcheck probes the port nginx binds, so it cannot see where Traefik points.
- **Only `/tmp` is writable.** `docker-entrypoint.sh` writes the runtime `env-config.js` there, and
  `nginx.conf` serves it through a `location = /env-config.js` alias. Do not add a second block for
  that path — nginx refuses to start on a duplicate location.

Traefik's routing is **generated** by `scripts/dev/common/build_and_deploy.py`; editing
`traefik/SOURCES/dynamic.yml.template` by hand is silently reverted on the next deploy.

See `shared-frontend-bug-avoider.md` for the full accounts.
...
- This module implements, as example baseline:
  ...
  - One menu item and one route are required for each main entity the module declares (see
    `database/SPECS/schema.sql`, generated from `app/models.py`)
...
- Every entity page that exposes a history endpoint must include an audit trail action icon
  (`History` from lucide-react) in the table action column, shown only when
  `<module_slug>.audit_trail:view` is present in the permission set `/api/me` returned.
...
Both suites run against your own module in the ordinary test run
(`./scripts/dev/common/run_enabled_tests.sh`), which is where a module developer checks parity.

The maintainer additionally runs them across module_template with
`scripts/dev/master/check_module_template_lf_parity.sh` (maintainer repository only —
`scripts/dev/master/` is never synced, so it is absent from your project).
