...
Traefik's routing is **generated** by `scripts/common/build_and_deploy.py`; editing
...
(`./scripts/common/run_enabled_tests.sh`), which is where a module developer checks parity.

The maintainer additionally runs them across module_template with
`scripts/master_only/check_module_template_lf_parity.sh` (maintainer repository only —
`scripts/master_only/` is never synced, so it is absent from your project).
