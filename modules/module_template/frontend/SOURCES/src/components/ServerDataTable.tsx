...
const formatColumnHeader = (columnId: string, t?: (key: string) => string): string => {
  const _t = t ?? ((k: string) => k)
  const auditMapping: Record<string, string> = {
    'au_creation_timestamp': _t('table.columns.createdAt'),
    'au_last_update_timestamp': _t('table.columns.updatedAt'),
    'au_created_by_user': _t('table.columns.creator'),
    'au_last_updated_by_user': _t('table.columns.updater'),
    'timestamp': _t('table.columns.createdAt'),
    'actor': _t('table.columns.creator'),
  }
    ...
    <div className="template:space-y-4" ref={tableRef}>
      ...
      <div className="template:flex template:items-center template:justify-between template:gap-2">
        <div className="template:flex template:items-center template:gap-2">
          <span className="template:text-sm template:text-muted-foreground">{t('table.rowsPerPage')}</span>
          <select
          ...
          <thead className="template:sticky template:top-0 template:z-10 template:border-b template:bg-background">
            <tr className="template:border-b">
              {visibleColumns.map((column) => {
                ...
                const resolvedHeader = typeof column.header === 'function' ? column.header() : column.header
                const fullHeaderLabel = resolvedHeader || formatColumnHeader(columnId, t)
      ...
      <div className="template:flex template:items-center template:justify-between template:border-t template:bg-background template:py-2">
        <div className="template:text-sm template:text-muted-foreground">
          {t('table.showing', { from: String(data.length > 0 ? ((page - 1) * pageSize) + 1 : 0), to: String(Math.min(page * pageSize, total)), total: String(total) })}
        </div>
        <div className="template:flex template:items-center template:gap-2">
          <button
            onClick={() => onPageChange(1)}
            disabled={page === 1}
            className="template:inline-flex template:items-center template:justify-center template:whitespace-nowrap template:rounded-md template:text-sm template:font-medium template:ring-offset-background template:transition-colors template:focus-visible:outline-none template:focus-visible:ring-2 template:focus-visible:ring-ring template:focus-visible:ring-offset-2 template:disabled:pointer-events-none template:disabled:opacity-50 template:border template:border-input template:bg-background template:hover:bg-accent template:hover:text-accent-foreground template:h-9 template:px-3"
          >
          ...
          <div className="template:text-sm template:font-medium">{t('table.page', { page: String(page), total: String(totalPages) })}</div>
          <button
            onClick={() => onPageChange(page + 1)}
