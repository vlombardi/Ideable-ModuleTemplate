...
import { AuditTrailPopup, AuditPageParams, VersionPage } from '../components/AuditTrailPopup'
...
const collectPermissionClaims = (payload: Record<string, unknown> | null): Set<string> => {
  ...
  const SUFFIXES = ['.permissions', '.permission', '.roles', '.role', '.groups', '.group', '.claims', '.entitlements', '.tenant_ids']
  const BARE_NAMES = new Set(['permissions', 'permission', 'roles', 'role', 'groups', 'group', 'claims', 'entitlements'])
  ...
  for (const [key, value] of Object.entries(payload)) {
    ...
    for (const suffix of SUFFIXES) {
      if (norm.endsWith(suffix)) {
        const prefixPart = norm.slice(0, -suffix.length)
        if (prefixPart) modulePrefix = prefixPart
        matched = true
        break
      }
    }
    if (!matched && BARE_NAMES.has(norm)) matched = true
    if (!matched) continue
    for (const entry of value) {
      if (typeof entry === 'string' && entry.trim()) {
        claims.add(modulePrefix ? `${modulePrefix}.${entry.trim()}` : entry.trim())
      }
    }
  }
...
export default function TemplateItems() {
  ...
  const canView = permissions.has('template.items:view')
  const canEdit = permissions.has('template.items:edit')
  const canViewAuditTrail = permissions.has('template.audit_trail:view')
              ...
              <div className="template:flex template:items-center template:gap-2" onClick={(e) => e.stopPropagation()}>
                {canViewAuditTrail && (
                  <button
                    ...
                    className="template:inline-flex template:items-center template:justify-center template:whitespace-nowrap template:rounded-md template:text-sm template:font-medium template:ring-offset-background template:transition-colors template:focus-visible:outline-none template:focus-visible:ring-2 template:focus-visible:ring-ring template:focus-visible:ring-offset-2 template:disabled:pointer-events-none template:disabled:opacity-50 template:hover:bg-accent template:hover:text-accent-foreground template:h-8 template:w-8 template:p-0"
                  >
                    <History className="template:h-4 template:w-4" aria-hidden="true" />
                  </button>
                )}
