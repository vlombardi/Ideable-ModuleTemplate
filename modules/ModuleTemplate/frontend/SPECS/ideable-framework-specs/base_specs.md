...
- ModuleTemplate specs and implementation must be kept aligned with the HostApp integration rules described in the shared specs/docs (e.g., in `<module_slug>/SPECS/ideable-framework-specs/base-specs.md` and the other framework-spec files in that folder) in the same change cycle whenever integration rules evolve.
- ModuleTemplate specs and implementation must be kept aligned with the HostApp integration rules described in the shared specs/docs (e.g., in `<module_slug>/<SUB-MODULE-NAME>/SPECS/ideable-framework-specs/base-specs.md` and the other framework-spec files in that folder) in the same change cycle whenever integration rules evolve.
