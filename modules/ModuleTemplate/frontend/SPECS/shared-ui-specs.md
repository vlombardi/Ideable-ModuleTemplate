...
- The following table chrome strings must be covered by keys under the `table.*` namespace: `rowsPerPage`, `showAuditData`, `hideAuditData`, `filterPlaceholder`, `all`, `true`, `false`, `noResults`, `showing` (with `{{from}}`, `{{to}}`, `{{total}}` vars), `page` (with `{{page}}`, `{{total}}` vars), `deleteSelected` (with `{{count}}` var), `updating`, `errorLoading`, and `columns.{createdAt,updatedAt,creator,updater}`.
