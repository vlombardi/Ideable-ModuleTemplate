...
export interface TemplateItemVersion {
  ...
  name: string | null
  description: string | null
  au_creation_timestamp: string | null
  au_last_update_timestamp: string | null
...
export const templateItemsService = {
  ...
  /**
   * Get audit trail history for a template item
   */
  async getHistory(id: number): Promise<TemplateItemVersion[]> {
    const response = await fetchWithAuth(`${API_BASE_URL}/items/${id}/history`)
    return response.json()
  },
}
