...
- Backend integration must preserve JWT validation and HostApp `/api/me` permission-resolution contract.
...
- `template.items.read` - for listing/reading items
- `template.items.create` - for creating items
- `template.items.update` - for updating items
- `template.items.delete` - for deleting items
