...
def get_items(
    ...
    _: str = Depends(require_permission('template.items.read')),
):
    try:
...
def post_item(
    ...
    db: Session = Depends(get_db),
    username: str = Depends(require_permission('template.items.create')),
):
...
def put_item(
    ...
    db: Session = Depends(get_db),
    username: str = Depends(require_permission('template.items.update')),
):
...
def remove_item(
    item_id: int,
    db: Session = Depends(get_db),
    _: str = Depends(require_permission('template.items.delete')),
):
    item = crud.get_item(db, item_id)
    if item is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Item not found')
