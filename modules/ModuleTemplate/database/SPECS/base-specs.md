...
- The authoritative schema source is `modules/ModuleTemplate/database/SPECS/datamodel.sql` for the example ModuleTemplate module.
...
ModuleTemplate must support one database target for module entities, configured through dedicated env vars in the example ModuleTemplate setup:
...
- `datamodel.sql` must be executed only for the example ModuleTemplate entities schema lifecycle.
