...
Ideable modules (HostApp and remote modules derived from ModuleTemplate). It covers backend
...
field-change events and association-change events, merged and sorted chronologically:

```
GET /<entity>/{entity_id}/history?skip={skip}&limit={limit}&sort_by={sort_by}&sort_order={sort_order}&filters={filters}
```
...
| `limit` | integer | `100` | Maximum number of history rows to return |
| `sort_by` | string | `timestamp` | Column to sort by (`timestamp`, `actor`, `operation_type`) |
| `sort_order` | string | `desc` | Sort direction (`asc` or `desc`) |
| `filters` | string (JSON) | `{}` | JSON object with per-column filter values |
...
class BaseVersion(BaseModel):
    ...
    au_creation_timestamp: Optional[datetime] = None
    au_last_update_timestamp: Optional[datetime] = None
    au_created_by_user: Optional[str] = None
    au_created_by_user_id: Optional[int] = None
    au_last_updated_by_user: Optional[str] = None
    au_last_updated_by_user_id: Optional[int] = None
```
...
Every audit trail table must use the same shared table infrastructure as regular entity tables.
This means it **must** support:

- **Server-side pagination** — `skip`/`limit` query parameters with standard pagination
  controls (First, Previous, page X of Y, Next, Last) and a "Rows per page" selector.
- **Column sorting** — every column header must be clickable to sort ascending or descending,
  with visual indicators (`↕`, `↑`, `↓`).
- **Column filtering** — a filter input row below the sort headers for substring (text) or
...
|---|---|---|
| **When** | `When` | The event timestamp (`timestamp` / `au_creation_timestamp`), rendered as a locale-aware datetime string. |
