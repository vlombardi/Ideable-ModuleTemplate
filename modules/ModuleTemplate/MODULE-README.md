...
Edit module-level specs first:

```
SPECS/base-specs.md                              # Module-wide contract and build/deploy rules
config/authorization.yaml                        # Authoritative Authentik authorization config for module items
...
- When the composed app needs additional general authorization data, update HostApp's authorization spec.
- When a module needs additional module-specific authorization data, update that module's authorization config.
...
2. **Use token claims as the only runtime source of truth.**
   ...
   - Read menu visibility from `<module_slug>.menu_access`.
   - Read company scoping from `hostapp.company_ids`.
...
4. **Make UI visibility follow the token.**
   - Hide menus when the `*.menu_access` claim is missing.
...
- `frontend/SPECS/base_specs.md` - Frontend build, MF, CSS
- `backend/SPECS/base_specs.md` - FastAPI, auth
- `database/SPECS/base-specs.md` - Schema design
- `SPECS/base-specs.md` - Module integration
