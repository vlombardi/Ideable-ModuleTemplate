...
SUPERADMIN_GROUP_NAME = "authentik-superadmins"
BOOTSTRAP_EVENT_QUERY_PARAM = "bootstrap=1"
SADMIN_ACTIVE_PROFILE = os.getenv("SADMIN_ACTIVE_PROFILE", "admin").strip() or "admin"
...
def _discover_hostapp_root(script_path: Path) -> Path:
    ...
    it may be mounted with only the HostApp authorization spec available.
    We look for a directory containing the HostApp authorization spec,
    with an optional override via HOSTAPP_ROOT.
    """

    def _is_hostapp_root(path: Path) -> bool:
        auth_spec = path / "config" / "authorization.yaml"
        module_json = path / "module.json"

        log(f"  [DEBUG] Checking root candidate: {path}")
        log(f"  [DEBUG]   - authorization.yaml: {auth_spec.exists()}")
        log(f"  [DEBUG]   - module.json:          {module_json.exists()}")

        if auth_spec.exists():
            log(f"HOSTAPP_ROOT detected via authorization.yaml: {path}")
            return True
        if module_json.exists():
            log(f"HOSTAPP_ROOT detected via module.json: {path}")
            return True
        return False
...
def _discover_modules_root(hostapp_root: Path) -> Path:
    env_root = os.getenv("MODULES_ROOT")
    if env_root:
        candidate = Path(env_root).expanduser().resolve()
        if candidate.exists():
            return candidate

    deployment_modules_root = Path("/modules")
    if deployment_modules_root.exists():
        return deployment_modules_root

    return hostapp_root.parent


MODULES_ROOT = _discover_modules_root(HOSTAPP_ROOT)
...
def _build_default_login_css(app_name: str, background_image_url: str, fill_color: str) -> str:
    ...
    return f"""
ak-flow-executor {{
  /* ── Force light-mode native form controls (inherits through Shadow DOM) ── */
  color-scheme: light;
...
def load_authorization_contracts(
    ...
    if effective_hostapp_auth.exists():
        ...
        log(f"  ⚠ HostApp authorization spec not found at {effective_hostapp_auth} — continuing with module contracts only")
...
def build_authorization_plan(contracts):
    ...
    hostapp_tenant_claim = "hostapp.tenant_ids"
    ...
    for contract in contracts:
        ...
        if contract.get("is_hostapp"):
            ...
            hostapp_tenant_claim = contract.get("tenant_claim", hostapp_tenant_claim)
    ...
    return {
        ...
        "hostapp_tenant_claim": hostapp_tenant_claim,
...
def render_scope_mapping_expression(plan: dict) -> str:
    ...
    tenant_claim_json = json.dumps(plan.get("hostapp_tenant_claim") or "hostapp.tenant_ids")
    ...
    profile_roles_json = json.dumps(plan.get("profile_roles") or {})
    roles_mapping_json = json.dumps(plan.get("roles_mapping") or {})
    available_permissions_json = json.dumps(plan.get("available_permissions") or [])
    template = textwrap.dedent(
        """
        # Ideable Permissions Claims Mapping
        # This expression collects permissions based on the active profile.

        SUPERADMIN_VALUES = __SUPERADMIN_VALUES__
        HOSTAPP_PERMISSION_CLAIM = __PERMISSION_CLAIM__
        TENANT_CLAIM = __TENANT_CLAIM__
        ...
        # Inline fallbacks: ak_group_by is not available in Authentik property-mapping
        # context, so these pre-baked plan values are the primary data source.
                ...
                for r_name in role_names:
                    ...
                    for p_name in perms:
                        ...
                        if action == "menu_access" and resource:
                            res.setdefault(p_claim, set()).add(f"{resource}:menu_access")
                        else:
        ...
        # Add tenant IDs
        tenant_ids = set()
        for attr in (TENANT_CLAIM, "user_companies", "company_ids"):
            val = u_attrs.get(attr)
            if val:
                if isinstance(val, (list, tuple, set)):
                    tenant_ids.update(str(x).strip() for x in val if x)
                elif isinstance(val, str):
                    tenant_ids.update(x.strip() for x in val.split(",") if x.strip())
                else:
                    tenant_ids.add(str(val).strip())
        if tenant_ids:
            claims.setdefault(TENANT_CLAIM, set()).update(tenant_ids)
    ...
    return (
        ...
        .replace("__TENANT_CLAIM__", tenant_claim_json)
        ...
        .replace("__FALLBACK_PROFILE_ROLES__", profile_roles_json)
        .replace("__FALLBACK_ROLES_MAPPING__", roles_mapping_json)
        .replace("__FALLBACK_PERMISSION_META__", available_permissions_json)
    )
...
def api_request(endpoint, method="GET", data=None):
    ...
    headers = {
        ...
        "User-Agent": f"Ideable-Auth-Bootstrap/1.0 ({BOOTSTRAP_EVENT_QUERY_PARAM})",
    }
...
def _set_user_attributes(user_pk, attributes: dict[str, object]) -> bool:
    """Merge the supplied attributes into the Authentik user object."""
    try:
        api_request(f"core/users/{user_pk}/", method="PATCH", data={"attributes": attributes})
        return True
    except Exception as exc:
        log(f"  WARNING: could not update attributes for user {user_pk}: {exc}")
        return False


def _wait_for_authentik_user(username: str, *, max_attempts: int = 60, delay: int = 5):
    """Wait until an Authentik user exists before continuing bootstrap token seeding."""
    for attempt in range(1, max_attempts + 1):
        user = _find_user_by_username(username)
        if user:
            return user
        log(f"  akadmin not ready yet (attempt {attempt}/{max_attempts}) — retrying in {delay}s...")
        time.sleep(delay)
    raise RuntimeError(f"Timed out waiting for Authentik user '{username}' to exist")
...
def ensure_sadmin_user():
    ...
    desired = {
        ...
        "email": SADMIN_EMAIL,
        "attributes": {
            "hostapp.active_profile": SADMIN_ACTIVE_PROFILE,
        },
    }
    ...
    _ensure_superadmin_group_membership(user_pk)


def _ensure_superadmin_group_membership(user_pk):
    """Add or remove user from the Authentik superadmin group.

    The user-level is_superuser flag is silently ignored by the Authentik
    REST API in Authentik 2024+. A group with is_superuser=True is the
    reliable alternative to grant or revoke Authentik admin UI access.
    """
    group = _find_group_by_name(SUPERADMIN_GROUP_NAME)
    if SADMIN_IS_SUPERUSER:
        if not group:
            result = api_request(
                "core/groups/",
                method="POST",
                data={"name": SUPERADMIN_GROUP_NAME, "is_superuser": True},
            )
            group_pk = result.get("pk")
            log(f"✓ Created superadmin group '{SUPERADMIN_GROUP_NAME}' (pk: {group_pk})")
        else:
            group_pk = group.get("pk")
            if not group.get("is_superuser"):
                api_request(
                    f"core/groups/{group_pk}/",
                    method="PATCH",
                    data={"is_superuser": True},
                )
                log(f"✓ Updated group '{SUPERADMIN_GROUP_NAME}' is_superuser=True")
            else:
                log(f"✓ Superadmin group '{SUPERADMIN_GROUP_NAME}' ready")
        try:
            api_request(
            ...
            log(f"✓ Added '{SADMIN_USERNAME}' to '{SUPERADMIN_GROUP_NAME}'")
        except Exception as exc:
            log(f"  ℹ '{SADMIN_USERNAME}' already in '{SUPERADMIN_GROUP_NAME}' or could not add: {exc}")
    else:
        if group:
            group_pk = group.get("pk")
            try:
                ...
                log(f"✓ Removed '{SADMIN_USERNAME}' from '{SUPERADMIN_GROUP_NAME}'")
            except Exception as exc:
                log(f"  ℹ Could not remove '{SADMIN_USERNAME}' from '{SUPERADMIN_GROUP_NAME}': {exc}")


def get_default_authorization_flow():
    """Get the default authorization flow UUID."""
    # Try by designation first
    try:
        ...
        log(f"Warning: Could not fetch default authorization flow by designation: {e}")
    
    # Fallback: look up the canonical slug directly
    for slug in ("default-provider-authorization-implicit-consent",
                 "default-provider-authorization-explicit-consent"):
        try:
            flow = api_request(f"flows/instances/{slug}/")
            if flow and flow.get("pk"):
                log(f"Using authorization flow: {slug}")
                return flow["pk"]
        except Exception:
            pass
    
    raise RuntimeError(
        "Unable to locate a valid authorization flow. "
        "Ensure Authentik has finished its internal bootstrap."
    )
...
def get_default_invalidation_flow():
    ...
    try:
        ...
        log(f"Warning: Could not fetch default invalidation flow by designation: {e}")
    
    # Fallback: look up the canonical slug directly
    try:
        flow = api_request("flows/instances/default-provider-invalidation-flow/")
        if flow and flow.get("pk"):
            log(f"Using invalidation flow: default-provider-invalidation-flow")
            return flow["pk"]
    except Exception:
        pass
    
    raise RuntimeError(
        "Unable to locate a valid invalidation flow. "
        "Ensure Authentik has finished its internal bootstrap."
    )
...
def ensure_login_branding():
    ...
    for brand in brands:
        ...
        if attrs_field:
            ...
            existing_attrs["settings"]["theme"] = {"base": "light"}
...
def ensure_bootstrap_token():
    ...
    for attempt in range(1, 61):
        try:
            ...
            log(f"  DB not ready (attempt {attempt}/60): {e} — retrying in 5s...")
            time.sleep(5)
    else:
        raise RuntimeError("Could not connect to DB for token upsert")
        ...
        with conn.cursor() as cur:
            ...
            for attempt in range(1, 61):
                ...
                if row:
                    akadmin_id = row[0]
                    break
                log(f"  akadmin not ready yet (attempt {attempt}/60) — retrying in 5s...")
                time.sleep(5)
            if not akadmin_id:
                log("  akadmin not found — fresh DB, relying on Authentik seeding")
                return
            log("  akadmin ready — proceeding with bootstrap token upsert")
...
def ensure_user_group_memberships(plan):
    ...
    for username, profiles in user_profiles.items():
        ...
        # Sync available_profiles and active_profile attributes for UI profile-switching
        try:
            active_profile = next((p for p in profiles if p), None)
            attrs: dict[str, object] = {"hostapp.available_profiles": profiles}
            if active_profile:
                attrs["hostapp.active_profile"] = active_profile
            api_request(
                f"core/users/{user_pk}/",
                method="PATCH",
                data={"attributes": attrs},
            )
            log(f"  ✓ Synced available_profiles for '{username}'")
        except Exception as exc:
            log(f"  ℹ Could not sync available_profiles for '{username}': {exc}")
...
def ensure_authorization_entities():
    ...
    # Sync sadmin available_profiles and active_profile from plan
    users_meta = plan.get("users") or {}
    sadmin_meta = users_meta.get(SADMIN_USERNAME) or {}
    sadmin_profiles = sadmin_meta.get("profiles") or []
    if sadmin_profiles:
        user = _find_user_by_username(SADMIN_USERNAME)
        if user:
            try:
                active_profile = next((p for p in sadmin_profiles if p), None) or SADMIN_ACTIVE_PROFILE
                api_request(
                    f"core/users/{user['pk']}/",
                    method="PATCH",
                    data={"attributes": {"hostapp.available_profiles": sadmin_profiles, "hostapp.active_profile": active_profile}},
                )
                log(f"  ✓ Synced available_profiles for '{SADMIN_USERNAME}'")
            except Exception as exc:
                log(f"  ℹ Could not sync available_profiles for '{SADMIN_USERNAME}': {exc}")
...
def main():
    ...
    for attempt in range(1, 61):
        try:
            ...
            last_validation_error = f"HTTP {exc.code}: {exc.read().decode() if hasattr(exc, 'read') else exc}"
        except Exception as exc:
            last_validation_error = str(exc)
        log(f"Token not yet valid (attempt {attempt}/60), retrying in 5s...")
        time.sleep(5)
    if not token_ready:
        message = "Bootstrap token was never accepted by Authentik."
        if last_validation_error:
            message += f" Last validation error: {last_validation_error}."
        message += " Timed out waiting for validity."
