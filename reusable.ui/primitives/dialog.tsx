...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      // Popups dismiss only via the close (X) button (or Esc for a11y) — a click on the
      // backdrop/outside must NOT close, so a stray click never discards the dialog and its
      // unsaved input. Callers can re-enable click-away by passing their own
      // onPointerDownOutside / onInteractOutside (spread below overrides these defaults).
      onPointerDownOutside={(e) => e.preventDefault()}
      onInteractOutside={(e) => e.preventDefault()}
      className={cn(
        "ideable:fixed ideable:left-[50%] ideable:top-[50%] ideable:z-50 ideable:grid ideable:w-full ideable:max-w-lg ideable:max-h-[calc(100vh-2rem)] ideable:overflow-y-auto ideable:translate-x-[-50%] ideable:translate-y-[-50%] ideable:gap-4 ideable:border ideable:bg-popover ideable:text-popover-foreground ideable:p-6 ideable:shadow-lg ideable:duration-200 ideable:data-[state=open]:animate-in ideable:data-[state=closed]:animate-out ideable:data-[state=closed]:fade-out-0 ideable:data-[state=open]:fade-in-0 ideable:data-[state=closed]:zoom-out-95 ideable:data-[state=open]:zoom-in-95 ideable:data-[state=closed]:slide-out-to-left-1/2 ideable:data-[state=closed]:slide-out-to-top-[48%] ideable:data-[state=open]:slide-in-from-left-1/2 ideable:data-[state=open]:slide-in-from-top-[48%] ideable:sm:rounded-lg",
        className
      )}
      {...props}
    >
