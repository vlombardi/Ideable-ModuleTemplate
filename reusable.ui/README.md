...
**Step 5 — i18n.** Your page strings come from **your module's** `useTranslation`, whose bundle must hold every key your own code resolves (it has no library fallback). The widgets carry their own `table.*`/`auditTrail.*`/`chart.*`/`common.*` strings, in `reusable.ui/i18n/` — you hold none of them and still get a complete, translated widget. **To override one, put that key in your own `en.json`/`it.json`**: inclusion *is* the override (your module's hook registers its bundles with `registerModuleMessages`, and the shared hook reads yours first). For the same reason, never copy a library string you do not mean to change — an identical copy overrides nothing and pins that wording the day the library improves it. `TESTS/test_i18n_contract.py` reports what you override, and lists verbatim copies separately. Keep `en.json`/`it.json` in sync.
