...
| i18n | `registerModuleMessages({ en, it })` — hands the consuming module's bundles to the widgets, so a key the module holds overrides the library's (Step 5 below) |
...
./scripts/dev/module/sync-template-updates.sh --selective
...
**Step 5 — i18n.** Your page strings come from **your module's** `useTranslation`, whose bundle must hold every key your own code resolves (it has no library fallback). The widgets carry their own `table.*`/`auditTrail.*`/`chart.*`/`common.*` strings, in `reusable.ui/i18n/` — you hold none of them and still get a complete, translated widget. **To override one, put that key in your own `en.json`/`it.json`**: inclusion *is* the override (your module's hook registers its bundles with `registerModuleMessages`, and the shared hook reads yours first). For the same reason, never copy a library string you do not mean to change — an identical copy overrides nothing and pins that wording the day the library improves it. Keep `en.json`/`it.json` in sync: they hold the same keys, so overriding a string in one language means holding the key in the other too, at the library's own wording — that second row is required, not a copy to delete. `TESTS/test_i18n_contract.py` reports what you override **per language**, and lists separately the keys you copy verbatim in **every** language; those, and only those, are the ones you can remove.
