...
- **Consistent look & feel, for free.** Widgets inherit host_app's design tokens automatically, so your module looks native inside the host with zero styling work.
...
Rebrand by overriding **token values, never class names** — widgets reference token names, so a rebrand touches only values:
- Inside host_app, widgets inherit host tokens automatically (`.ideable-scope[data-lf='hostapp']`).
- For a module‑specific look, set the `--ideable-module-*` overrides and render under `data-lf="module"` (see `reusable.ui/styles/tokens.css`).
