...
validate_module() {
  ...
  # HostApp in remote module repos is a runtime-only skeleton (no SPECS/ or sub-module SOURCES/).
  # Skip source-level validation and only validate runtime artifacts.
  if [[ "${module_name}" == "HostApp" && ! -d "${module_dir}/SPECS" ]]; then
    echo "  [${module_name}] Runtime-only HostApp detected; skipping full module validation."
...
PY
  ...
  if [[ "${module_role}" != "side" && ! -f "${config_dir}/authorization.yaml" ]]; then
    # Runtime-only HostApp uses pre-built images; authorization is baked in
    if [[ "${module_name}" == "HostApp" && ! -d "${module_dir}/SPECS" ]]; then
      :
    else
