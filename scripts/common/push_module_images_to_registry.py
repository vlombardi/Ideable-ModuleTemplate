...
def local_image_tag() -> str:
    """Return the tag the local images this script pushes were actually built with.

    Compose references built images as `…:${IMAGE_TAG}`, so this script has to resolve the same
    variable that `build_and_deploy.py` set — otherwise it looks for `hostapp.backend:${IMAGE_TAG}`
    in the local store and reports every image as missing.

    The authority is `deployment_root/.env.config`, because that is the file the deploy *wrote*:
    it names the build that is on this machine, which is the build there is anything to push. An
    `IMAGE_TAG` in the environment overrides it, for pushing a build other than the deployed one.

    Note this is a different question from `-t/--tag`. That option names the tag to push *to* in
    the registry (a release name). This is the tag the local image is *found under*.
    """
    override = os.environ.get("IMAGE_TAG", "").strip()
    if override:
        return override

    deployed_env = PROJECT_ROOT / "deployment_root" / ".env.config"
    if deployed_env.is_file():
    ...
    print("ERROR: cannot determine which local images to push.")
    print("  deployment_root/.env.config does not record an IMAGE_TAG, so the tag the local")
    print("  images were built with is unknown. Run ./redeploy.sh first, or state it:")
    print("    IMAGE_TAG=<tag> python3 scripts/common/push_module_images_to_registry.py -a")
    sys.exit(1)
...
def read_local_images(module_name: str, module_slug: str, compose_path: Path, app_slug: str = "") -> list[str]:
    ...
    for raw_line in compose_path.read_text(encoding="utf-8").splitlines():
        ...
        # Compose uses: image: ${MODULE_DOCKER_REGISTRY_PREFIX}/${MODULE_SLUG}.backend:${IMAGE_TAG}
        if registry_prefix:
...
def main() -> int:
    ...
    for module_name in requested_modules:
        ...
        for local_ref in report.local_images:
            ...
            report.expected_images.append(target_ref)
            ...
            if not args.single_arch:
                ...
                ok, output = push_image_multiarch(
                    target_ref, sources_dir, args.platform, multiarch_builder_name,
