...
def _resolve_docker_registry_prefix(content, module_env):
    ...
    Reads the prefix from the provided module env dict so each module
    uses its own registry setting (prevents leakage from merged .env).
    """
...
def image_name(module_name, submodule_name, module_slug=None, project_slug=None):
    ...
    """
    module_slug_value = module_slug or module_name.lower()
...
def build_submodule(module_name, submodule_name, submodule_path, module_slug=None, compose_image=None, project_slug=None):
    ...
    if has_dockerfile:
        ...
        if "${" in img:
            ...
            img = _resolve_docker_registry_prefix(img, module_env)
        if compose_image:
...
def deploy_module_root(module_name, module_path, module_meta, project_slug=None, is_remote=False):
    ...
    if os.path.exists(src_compose):
        ...
        # each module uses its own registry setting.
        module_env = read_module_env_map(module_path)
        content = _resolve_docker_registry_prefix(content, module_env)
...
def generate_traefik_dynamic_template(enabled_module_data):
    ...
    remote_modules = [
        (name, meta)
        for name, path, meta, _mode in enabled_module_data
    ...
    {slug}-frontend:
      rule: "Host(`{BASE_HOST}`) && PathPrefix(`/remotes/{slug}`)"
      priority: 130
      entryPoints:
        - web
        - websecure
    ...
    {slug}-backend:
      rule: "Host(`{BASE_HOST}`) && PathPrefix(`/module/{slug}`)"
      priority: 110
      entryPoints:
        - web
        - websecure
...
def generate_modules_menu_mapping(enabled_module_data=None):
    ...
    If modules/HostApp/config/modules_menu_mapping.json exists, it is used directly
    ...
    if os.path.isfile(source_path):
        ...
        if os.path.isdir(src_config_dir):
            ...
            with open(src_config_path, "w", encoding="utf-8") as f:
                json.dump(payload, f, indent=2)
                f.write("\n")
            print("  [HostApp] Mirrored explicit modules_menu_mapping.json → src/config and deployment_root")
        else:
            print("  [HostApp] Mirrored explicit modules_menu_mapping.json → deployment_root")
        return

    # Auto-merge from enabled modules when no explicit HostApp mapping exists
    merged = {"menu_mapping": []}
    merged_count = 0
    ...
    if merged["menu_mapping"]:
        ...
        if os.path.isdir(src_config_dir):
            ...
            with open(src_config_path, "w", encoding="utf-8") as f:
                json.dump(merged, f, indent=2)
                f.write("\n")
            print(f"  [HostApp] Generated merged modules_menu_mapping.json from {merged_count} module(s) → src/config and deployment_root")
        else:
            print(f"  [HostApp] Generated merged modules_menu_mapping.json from {merged_count} module(s) → deployment_root")
    else:
        print("  [HostApp] WARNING: No modules_menu_mapping.json found in HostApp or any enabled module")


def _normalize_authentik_tokens(env_path: str) -> None:
    """Resolve AUTHENTIK token placeholders in an env file.

    - If AUTHENTIK_BOOTSTRAP_TOKEN is ${SOME_KEY}, replace with SOME_KEY's value.
    - If AUTHENTIK_API_TOKEN is ${AUTHENTIK_BOOTSTRAP_TOKEN} or empty,
      replace with the resolved AUTHENTIK_BOOTSTRAP_TOKEN value.
    """
    ...
    # Resolve AUTHENTIK_BOOTSTRAP_TOKEN reference
    bootstrap_token = env.get("AUTHENTIK_BOOTSTRAP_TOKEN", "")
    if bootstrap_token.startswith("${") and bootstrap_token.endswith("}"):
        ref_key = bootstrap_token[2:-1]
        if ref_key in env:
            bootstrap_token = env[ref_key]
            env["AUTHENTIK_BOOTSTRAP_TOKEN"] = bootstrap_token

    # Resolve AUTHENTIK_API_TOKEN
    api_token = env.get("AUTHENTIK_API_TOKEN", "")
    if not api_token or api_token == "${AUTHENTIK_BOOTSTRAP_TOKEN}":
        if bootstrap_token:
...
def classify_env_var(key: str, value: str, all_vars: dict[str, str] | None = None) -> str:
...
def merge_env_files(enabled_module_data, remote_hostapp=False, project_env_path="", remote_module_names=None):
    ...
    # Process HostApp env first.
    if remote_hostapp or hostapp_module_entry:
        _append_env_section([hostapp_config, hostapp_secrets], "HostApp")
    ...
    if remote_hostapp:
        ...
        _write_env_split(hostapp_deploy_dir, project_env_sources + [hostapp_config, hostapp_secrets], "HostApp deployed env")
    ...
    for module_name, _, module_meta, mode in enabled_module_data:
        module_slug = module_meta.get("slug", module_name.lower())
        module_log_level = 'DEBUG' if mode == 'build' else 'INFO'
...
def generate_module_registry(enabled_module_data, remote_hostapp=False):
    ...
    if remote_hostapp:
        print("  [HostApp] Remote mode — skipping in-place module-registry.json copy to local SOURCES")
        ...
        print(f"  [HostApp] frontend SOURCES not present locally; skipping in-place module-registry.json copy")
    else:
...
if __name__ == "__main__":
    ...
    for name, _, _, _ in enabled_module_data:
        if name == 'HostApp' and remote_hostapp_compose:
            enabled_names.append('HostApp (remote)')
        else:
    ...
    for module_name, module_path, module_meta, mode in enabled_module_data:
        ...
        module_slug = module_meta.get("slug") or module_name.lower()
        if not module_slug:
    ...
    if remote_hostapp_compose:
        print("PROGRESS:Deploying remote HostApp configuration")
        ...
        # Resolve slug placeholders in remote HostApp compose as well.
        ...
        hostapp_content = _resolve_docker_registry_prefix(hostapp_content, hostapp_env)
        ...
        hostapp_content = hostapp_content.replace(
            "./modules/HostApp/modules:/modules",
            "./modules:/modules",
        )
        hostapp_content = re.sub(
            r'(source:\s+)(\.\/[^\s\n]+)',
        ...
        # Deploy HostApp sub-module DIST/ folders (database, traefik, etc.).
        ...
        print("PROGRESS_HEADER:Deploying module HostApp")
        ...
        if os.path.isdir(hostapp_authentik_blueprints_dir):
            print(f"  [HostApp] Ensured authentik blueprints dir → {hostapp_authentik_blueprints_dir}")

    for module_name, module_path, module_meta, mode in enabled_module_data:
        # Remote HostApp is handled by the special path above; skip here.
        if module_name == 'HostApp' and remote_hostapp_compose:
            continue
