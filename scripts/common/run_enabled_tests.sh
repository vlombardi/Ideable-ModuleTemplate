#!/bin/bash
# Run tests for all enabled modules listed in modules/enabled.md.
...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  echo "Usage: $0 [-h|--help]"
  echo ""
  echo "Runs pytest for every enabled module listed in modules/enabled.md."
  echo "Test reports are written to TEST_REPORTS/<timestamp>-<module>/test-report.md."
  echo ""
  echo "Options:"
