...
- **Purpose**: Adopts a framework release in this project — writes the version into `IDEABLE_FRAMEWORK_VERSION` and then runs the sync that brings that release's files, so the project's choice and its facts move together or not at all.
- **Usage**:
  ```bash
...
- **Notes**:
  ...
  - **It restores `framework.env` if the sync fails.** A project naming a release whose files it does not have resolves nothing, and that half-state is what this command exists to remove.
...
- **Usage**:
  ...
  ./scripts/dev/master/publish_framework.sh              # publish what framework.env already names
  ./scripts/dev/master/publish_framework.sh --dry-run    # show the plan, touch nothing
  ```
- **Notes**:
  - **A release ends in a commit and a tag.** The two files the publish writes — `framework.env` and the regenerated `framework.lock.json` — are committed as `chore(release): framework <version>`, and the tag is put on that commit, so the tagged tree contains the lock naming its own release. The template repository is tagged in step 4 by the push itself.
  - **The release is named as a bare argument**, not a flag, and the script writes it into `IDEABLE_FRAMEWORK_VERSION` after the images and before the toolbox publish — so the argument is an input to the run, not a second place a version is stated. A failed run restores the previous value. Omit it to republish what the file already says, which is how the `latest` channel is published. A release also moves `latest` and tags both git repositories.
