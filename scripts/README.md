...
(`scripts/TESTS/test_sync_lands_a_layout_move.py`). **A path whose KIND changed is part of that
move**: a root command the template holds as a symlink and a project still holds as a regular file
differs by type, which git reports as `T` and never as `M`, and the sync transfers it on that letter
like any other difference (`status_wants_a_transfer`, `scripts/TESTS/test_sync_handles_symlinks.py`).
Without it the project kept a stale command that named the folder the move had just emptied.
...
- **Notes**:
  ...
  - Runs a **static analysis gate** before the suites — `ruff` over `modules scripts`, `mypy` per backend `app/`, `npx tsc --noEmit` per frontend — and fails the run on a finding, so a type or lint error is reported as itself rather than as a downstream test failure.
  - **Installs the frontend dependencies the gate and the UI suite need, and reinstalls them when the shared widget library has moved on.** `--install-links` COPIES `@ideable/ui` into `node_modules` (`modules/host_app/SPECS/dependencies.md` § *npm dependencies are pinned by lockfile*), and inside the dev tools container that tree persists between runs — so the copy would otherwise outlive the library and `tsc`, plus the dev server Playwright starts, would read a snapshot while the developer's host reads a symlink. Any file under `reusable.ui/` newer than the installed copy triggers `npm ci` for that tree.
...
- **Notes**:
  ...
  - `names_a_local_build()` is the single definition of *this deploy built it*, asked by both entry points: the repository must be `<slug>.<submodule>`. A sub-module whose `SPECS/build.sh` produces files runs a third-party image, which carries no revision from this checkout and is not checked; the decision is on the repository name and never on the tag, so an own image whose tag holds an unresolved variable is still checked.
...
- **Notes**:
  ...
  - **It polices WHETHER a table is converted to a hypertable, not HOW.** A conversion counts in any of three spellings: the table name as a literal inside `create_hypertable(`; a string member of a module-level tuple or list constant, annotated (`AUDIT_HYPERTABLES: tuple[str, ...] = (…)`) or bare; or a literal passed to a function of the migration's own whose body performs the conversion, which is the safer spelling a module reaches for when it binds the name as a query parameter. The last two are deliberately generous heuristics: the comparison is *required ⊆ found*, so a name they add that is no table cannot hide an unconverted one, while a shape they miss reports a module's correct implementation as a defect it has no way to fix.
  - **Uses**: no other repo scripts.
...
- **Notes**:
  ...
  - `DEV_CYCLE_AGENT_PERMISSIONS` sets the policy: `ask` (default on a TTY) · `deny` (default without one) · `accept-edits` · `skip`. `deny` names what an unattended run must do with a **permission** nobody allowed: refuse it, never grant it. The policy governs **permissions only** — a QUESTION is a decision the maintainer owes, and a policy cannot take it for them, so a question is put to the terminal under every policy.
  - **A question waits for the person, and blocks the plan rather than being lost.** The prompt rings the terminal's bell — a node can run for twenty minutes needing nobody and then need someone at once — offers `0. I'll answer later` beside the real answers, and counts down `DEV_CYCLE_ASK_TIMEOUT` seconds (default 120; `0` waits indefinitely) for a FIRST keystroke, which cancels the countdown for good: someone who has started typing is demonstrably there, and a timer still running would cut a long answer off mid-thought.
  - **Four ways of not answering take one exit**: `I'll answer later`, a countdown that ran out, no terminal at all, and a read that failed (EOF, a blank line). Each denies the set carrying *which* of them happened — "the maintainer chose to answer later" and "nobody was there" are different facts about who decided what — and hands the question back to the router, which moves the plan to `Asking human direction`. A permission has no `later` option and blocks nothing: an unsettled permission is a thing the run could not do.
  - **The terminal is `/dev/tty`, and there is never a fallback to stdin.** A process with no controlling terminal has nobody to ask, and reading fd 0 there either hits EOF at once or — on the pipe an agent's shell tool holds open — waits forever; the prompt answers `LATER` immediately instead. The countdown reads a character at a time (cbreak, `TCSANOW` both ways so a terminal nobody is draining cannot hang the router), and the mode is restored on **every** exit path — a normal answer, expiry, Ctrl-C, and SIGTERM/SIGHUP, where a handler restores the mode and re-raises the signal with its default disposition so the process still dies of what it was sent. The failure that guards against is leaving the maintainer's shell unusable after the router exits.
  - **An unanswered question is handed back to the router, not merely denied.** The agent is still told the question is open — it must report it rather than choose — and the whole set is handed to `dev_cycle.py` through the `unanswered` out-parameter, answered entries included, because a set is denied as a unit and an answer already given would otherwise be thrown away. The router records them in the plan and blocks there. `unanswered` reaches `answer_question` only and deliberately **not** `decide_permission`: an unsettled permission is a thing the run could not do, while an unanswered question is a decision still owed.
  ...
  - **The dev tools image carries the agent, and `--doctor` asserts both halves of it** — `claude_agent_sdk` in `REQUIRED_PYLIBS`, the `claude` binary in `REQUIRED_TOOLS`. The Python package is a thin wrapper that locates a Node CLI on PATH and spawns it, so an image with only the package passes its import check and fails at the first node. On a machine running the host toolchain (`IDEABLE_NO_CONTAINER=1`) neither half is guaranteed, and there the router still falls back to suggesting the skill when the import fails; `--deterministic` drives every deterministic node with no agent at all.
  - **`missing_prerequisite` refuses a node it cannot drive, and names the remedy.** Asked before the SDK is imported, because the import proves the least: it checks that a `claude` binary exists and — inside the container only, where neither the host's credential store nor any mounted file is reachable — that `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY` is set, pointing at `claude setup-token`. It refuses; it never quietly runs somewhere else, which would be the two-toolchain drift the container exists to end.
...
- **Notes**:
  - It performs an LLM node by running the node's skill through the Claude Agent SDK (`scripts/dev/common/agent_conversation.py`), and `--deterministic` opts out — the router then suggests the skill and stops. The skills still own what happens inside a node; the router owns where the plan stands.
  - **`run` is the only advance**, and `Asking human direction` is the router's to assign: a question the maintainer did not answer is recorded in the plan with the node the plan stood on, rather than lost. `resume_node` reads that record when every question is answered and returns it, or `Implementing` when the answer left the executing sub-set holding an unimplemented thing. See `rules/implementation-plan.md` § *Git integration*.
  - Canonical graph: `rules/implementation-plan.md` § *Overall view*.
  - **Uses**: `scripts/dev/common/run_enabled_tests.sh`, `redeploy.sh`, `scripts/dev/common/agent_conversation.py`.
...
- **Purpose**: Runs any dev-cycle tool inside the dev tools container instead of on this machine — the project's single toolchain. Every shipped script that runs a dev tool re-execs through it first, `scripts/dev/common/dev-cycle.sh` included: the router drives the agent, which is a dev tool like `pytest` or `ruff`, so it runs from the image too.
- **Usage**:
  ```bash
...
- **Notes**:
  ...
  - **What crosses into the container is a named set, never the caller's whole environment**: every `IDEABLE_*` and `DEV_CYCLE_*` variable by prefix, plus a short list of names that have none. The two prefixes carry a *set-but-empty* value, which the name list deliberately does not — `DEV_CYCLE_LOG=` is how logging is turned off, and a switch that silently failed to cross would turn it back on inside.
  - **The agent's credential crosses as a variable and in no other way.** `CLAUDE_CODE_OAUTH_TOKEN` and `ANTHROPIC_API_KEY` are forwarded; no credentials file is mounted and none is baked into the image, because this container mounts the Docker socket and is therefore root-equivalent on the host's Docker. It is also the only route that works everywhere — on macOS the credential is a login-Keychain item, so there would be nothing to mount.
...
- **Usage**:
  ...
  ./scripts/dev/common/dev-cycle.sh pause [<desc>]      # park a plan on its branch
  ...
  ```
- **Notes**:
  - **The command describes itself**: `./scripts/dev/common/dev-cycle.sh --help` is the full, current
    account of every action, flag and environment variable. The notes below are the catalogue's
    summary of what the script is *for*; where the two ever disagree, the help is the one that ships
    with the code (`scripts/TESTS/test_the_router_is_described_once.py`).
  - Canonical graph and status legend: `rules/implementation-plan.md` § *Overall view*.
  - **It runs in the dev tools container, like every other shipped script.** The whole wrapper
    re-execs through `scripts/dev/common/tool.sh` before it tees, so `tool.sh` still sees a terminal
    and allocates one — the question prompt reads `/dev/tty`, and a router with no controlling
    terminal would block a plan on a question nobody could be asked. Driving a plan therefore needs
    nothing installed on this machine, and the agent's credential reaches the container as a
    forwarded variable (`claude setup-token`). The opt-out and the variables are in
    `dev-cycle.sh --help`, with the rest of them.
  - **Landing a plan is `deliver`, never `run`.** `run` commits on the plan branch and, at `Done`,
    prints the delivery commands instead of running them; `deliver` squashes the branch onto the
    target as one commit whose message is the plan's abstract (or opens a PR with `--pr`), moves the
    kanban card to `kanban/done/` and renames the plan `(Merged)`. See `rules/version-control.md`
    § *Delivering a plan*.
  - **A delivery certifies the tree it lands.** `deliver` compares HEAD with the commit the newest recorded run names — chosen by filename, as `.githooks/pre-push` chooses it — and when they differ in any path that is not run bookkeeping it **runs the suite itself** and refuses on red, so delivering needs no manual run and no `SKIP_TEST_GATE`. Two orderings are forced: the tree is committed **before** the run, because a dirty tree is refused and a report must certify a commit rather than a working tree tied to nothing; and the report is committed **after** it, so the tree reaching the target carries its own certificate instead of being dirtied by the run that proved it. The check runs before any of the delivery's own bookkeeping, so a refusal leaves the plan at `Done`, the kanban card where it was and the target untouched — the whole point of running it here rather than meeting the gate's refusal with the commit already on the shared branch. `deliver` forgives fewer paths than the gate does and never more: stricter costs one unneeded run, looser lands a commit the push then refuses.
  - `pause` commits the tree on the plan branch, renames the plan `(Paused)` with the resume node still highlighted, drops the `ACTIVE PLAN` symlink and checks the target out; `resume` finds the plan by asking the `plan/*` branches — a paused plan's file lives on its own branch, never in the working tree — and renames it back onto its node. Both take an optional `<description>` so the remedy the two-plans refusal names reaches the plan the reader means. Bare `resume` when exactly one plan is paused; it lists them and stops when more than one is. Bare `pause` is never refused — it parks the plan the router resolves as active and reports which, printing the command that names the other. It also writes a light card to `kanban/paused/<description>.md` on the target and `resume` removes it, because a parked plan otherwise exists only on its own branch and the board — not `git branch --list` — is what a maintainer reads.
  - A sub-set still holding a thing whose `Impl` is 🔲/🔄 sends the plan back to `Implementing`, because prose cannot close unwritten work. Where that scope grew from an **answer**, the route is taken at `Asking human direction` — the first exit its panel names in the canonical graph, because that is where the cause is. The docs gate's own route-back is the **safety net** for a scope that grew with no question behind it, and is deliberately not a second drawn path. `Documenting` never routes into `BuildDeploy`/`Testing`/`Fixing`.
  - The plan's Status summary carries the run's own `**Overall:**` verdict, not a verdict derived from the test counts, so a run whose every test passed while the static-analysis gate failed reads `❌ FAILED — static analysis`. A SUMMARY with no verdict line falls back to the counts rather than inventing one.
  - `NotStarted` and `Branching` are performed by the router itself (the second creates the plan branch); deterministic nodes run here; LLM nodes are performed by a headless agent, whoever ran the command.
  - **`run` is the only advance.** It writes everything a transition must write — the graph, `Current step`, the executing sub-set's label, the sub-set advance, the `Commit` cells and the scope gate — so nothing else may write a plan's state; a plan that must genuinely be moved by hand is renamed by hand, deliberately.
  - **A question only the maintainer can answer blocks the plan rather than being lost.** When the agent performing a node asks one and nobody answers, `run` moves the plan to `Asking human direction`, carrying the question, its options and the node to come back to. The next `run` puts it again, or reads an answer written on the plan's `**Answer**:` line, and resumes once every question is answered — at the recorded node, or at `Implementing` when the answer left the executing sub-set holding an unimplemented thing, which is decided at resume time because the answer is what grows the scope. An unsettled *permission* does not block a plan — it is a thing the run could not do, not a decision still owed.
  - The prompt the router gives an LLM node asks it to **record every decision it took** — when, the node and sub-set, the question, the decision, who took it and why, a question nobody answered included — as a row in the plan's **`Decisions and answers`** chapter (`rules/implementation-plan.md` § *Decisions and answers*). A decision that stays in a terminal is one the next node takes again, differently: an agent's memory is never something the next node may rely on, and at a sub-set boundary the next node holds nothing but the plan.
  - **One chapter holds every decision, and it comes before chapter 1.** What a node settled, what the router decided and what the maintainer answered are one table — `When | Node | Sub-set | Question | Decision | Who | Why`, newest last — written by `record_decision()`, the router's only writer of it. `Who` is `maintainer` · `agent` · `router`, because a maintainer's decision may not be revisited without asking them again; a `Node` cell marked ⛔ came from an `Asking human direction` block, so the questions the plan was stopped for stay readable as a subset. A plan written before the merge is folded into the chapter by `migrate_decisions()` the first time anything records a decision on it; a delivered plan is a record of what was true then and is never rewritten.
  - **One agent per sub-set, not per node.** The agent's session is resumed across the LLM nodes of one sub-set, so `Documenting` and `Committing` judge the diff `Implementing` wrote instead of rebuilding what it means; a **new sub-set starts a fresh agent**, which is where reading only the plan earns its keep. A session that cannot be resumed starts a fresh one and says so. Each node is also handed a **brief** — the node, the executing sub-set's rows, what is already decided, the last recorded verdict and the sub-set's diff — rather than being told to read the plan; the brief does not replace the plan and says so in its own last line. How long a session lives is a setting, in `dev-cycle.sh --help` with the rest of them.
  - **A node whose input the diff cannot touch is skipped, and the skip is recorded with its reason.** A sub-set whose changed paths are all rules, skills or plan bookkeeping runs no `BuildDeploy` — building it would rebuild the *previous* sub-set and the run that follows would certify that — and the router writes the skip into the plan's decisions record rather than only printing it, because a step that silently did not happen reads exactly like one that did. An unknown path counts as a build input, so the skip is provable. **The tests are never skipped and never scoped** (`rules/testing-guidelines.md`).
  - `run`, `pause`, `resume` and `deliver` hold a lock on the plan (`.ideable-work/dev-cycle-locks/<description>.lock`) for their whole duration: a second worker on the same plan is refused with exit `3` and told who holds it, whatever started it. `status` takes no lock.
  - The active plan is the one an `ACTIVE PLAN - <description>.md` link names; `start` creates that link, so a plan file with no link is not active and a parked plan never resolves as active, even on its own branch. Because links are counted rather than files, one plan is one plan however many files `--keep-history` leaves behind. Two plans in flight (any state before `Done`) is **refused** by `run`, `status` and `start` with a non-zero exit code, naming both; `pause` and `deliver` are not refused, because they are the way out. See `rules/implementation-plan.md` § *Active plan resolution*.
  - Every checkpoint commit is followed by a bookkeeping-only commit folding the plan's Repos `Commit` cells from git.
  - **The sub-set table says where the plan's time went.** Its `Exec time` column is accrued at **every** transition: an execution clock starts once `run` is past the states that execute nothing, and `_write_plan` — the one function every transition passes through — adds the span to the sub-set that was executing when the clock started. It is therefore **not wall-clock since the sub-set began**: a pause, a wait at `Asking human direction` and a session that died are all outside the span by construction rather than subtracted afterwards, and `Branching` is charged to nothing because no sub-set is executing yet. Seconds are kept at every magnitude (`2h 3m 12s`), because the value is read-add-written and a cell rounded to the minute leaks once per transition. A plan in flight gains the column at its next transition; a delivered plan is never rewritten.
  - **Every invocation is watchable while it runs.** The wrapper tees the whole invocation — stdout *and* stderr, so the agent's tool calls, the runners' output and the router's own decisions all land in it — to `.ideable-work/dev-cycle.log`, and `tail -f` on that file follows a headless run instead of waiting for the driver's account of it afterwards. Appended, never truncated, with a header line per invocation, and rolled over to `dev-cycle.log.1` past 5 MB. `DEV_CYCLE_LOG` moves it; `DEV_CYCLE_LOG=` turns it off. Teeing costs the child processes their tty-ness (`redeploy.sh` and the test runner lose colour) and costs the caller nothing else: the question prompt reads and writes `/dev/tty` directly, and the router's own exit code is preserved through the pipeline.
...
- **Usage**:
  ...
  ```
- **Notes**:
  - **It runs from a copy of itself.** It is one of the files it syncs, and bash reads a script incrementally by byte offset — overwriting the running file makes the shell resume at that offset in different content. So it copies itself to a temporary file and hands over before doing any work; the file in the working tree is then just another file the sync may replace.
  - **The ref follows the framework version**: `main` on the `latest` channel, the version itself (`1.6.3`, not `v1.6.3`) when `framework.env` pins one. It asks `scripts/runtime/framework_version.py template-ref-name`, which reads `framework.env` alone — a project whose lock is missing or corrupt must still be able to sync, since the sync is what repairs it.
