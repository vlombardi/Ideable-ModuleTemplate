...
#     scripts/dev/common/tool.sh --stop            # remove the container
#
...
#
# WHICH IMAGE. `framework.env` names the framework version (one line, `latest` or `X.Y.Z`) and
# `framework.lock.json` records the digest the publish made for it; `scripts/runtime/framework_version.py`
# is the one reader of both, asked through `devtools_version.sh`. No shell override: the file is the
# only source, and an edit to it shows in `git diff` where a variable never would.
#
# HOW IT IS OBTAINED. Pulled from the registry BY DIGEST, never built. A pinned version must run
# exactly the published toolbox, so a local image that differs from the lock is a hard failure; on
# `latest` the lock moves with each sync and the image is pulled and retagged to follow it.
set -uo pipefail
...
# recurses or fails with something that looks unrelated to the cause. It comes before anything is
# resolved: inside the container there is nothing to resolve.
...
#
# Every `IDEABLE_*` and `DEV_CYCLE_*` variable is forwarded automatically; the rest are named
# because they have no common prefix. Forwarding the whole environment instead would carry the
# developer's shell into a container that is supposed to be identical for everyone.
#
# `DEV_CYCLE_*` joined the prefixes when `dev-cycle.sh` started re-execing through here: every one
# of the router's knobs — the log path, the permission policy, the question countdown, the session
# granularity — is read by a process that now runs INSIDE, so leaving them behind would have made
# the container silently ignore what the caller asked for. A prefix rather than seven names, for the
# reason the `IDEABLE_*` prefix already exists: the eighth knob should not need remembering.
#
# It also has to be a prefix to be CORRECT, not just tidy. The named list below forwards a variable
# only when it is non-empty, and `DEV_CYCLE_LOG=` — empty, deliberately — is the documented way to
# turn logging off. The prefix loop reads `env`, which lists a set-but-empty variable, so the off
# switch survives the crossing; in the named list it would have been indistinguishable from unset
# and logging would have come back on inside the container.
FORWARDED_ENV=(
  ...
  TEST_AUTH_TOKEN
  CI GITHUB_ACTIONS
  # THE AGENT'S CREDENTIAL, and the ONLY route it has into this container.
  #
  # The dev-cycle router runs in here like every other shipped script, so its LLM nodes
  # authenticate from in here — where there is no login Keychain and, deliberately, no mounted
  # credentials file. A maintainer ruling of 2026-09-14, on two grounds. This container mounts the
  # Docker socket, so anything reaching it is root-equivalent on the host's Docker, and a long-lived
  # credentials file is the wrong thing to put there. And a mount could not have been the rule
  # anyway: on macOS the credential IS the Keychain item (measured — `Claude Code-credentials`
  # exists, `~/.claude/.credentials.json` does not), so mounting the file is Linux-only by
  # construction, and one mechanism with no platform branch is the whole point.
  #
  # Mint one once with `claude setup-token` and export it. When neither is set the ROUTER refuses
  # the node and says so by name — `missing_prerequisite` in agent_conversation.py — rather than
  # letting the CLI fail with an auth error two layers from the cause.
...
forwarded_env_args() {
  ...
  while IFS='=' read -r name _; do
    [[ "$name" == IDEABLE_* || "$name" == DEV_CYCLE_* ]] && out+=(-e "$name")
  done < <(env)
...
# Three facts from the one resolver. Its own message on stderr names what is wrong with framework.env
# or framework.lock.json; there is nothing to add here but where the run stopped.
VERSION="$(devtools_version)" || die "could not read the framework version (framework.env) — see above."
IMAGE="$(devtools_image_ref)" || die "could not resolve the dev tools image — see above."
DIGEST="$(devtools_locked_digest)" || die "could not read the dev tools digest (framework.lock.json) — see above."
...
REQUIRED_TOOLS=(python3 pip ruff mypy pytest node npm npx git gh jq docker alembic rsync claude)
# `alembic` is here as a LIBRARY as well as a binary: `schema-workflow.md` mandates
# `scripts/dev/common/schema.sh` for all four of its phases, and that script drives alembic. It was absent
...
#
# `claude` and `claude_agent_sdk` are the same lesson, one cycle later and in BOTH lists on purpose.
# The dev-cycle router drives every LLM node through the Agent SDK, and the SDK is a thin wrapper
# that locates the Node CLI on PATH and spawns it (measured: it references `claude-code`, `node`,
# `which(` and `executable`). So asserting only the Python half would certify an image whose import
# check passes and whose first LLM node fails — the silently-weakened check this doctor exists to
# catch. Both are installed by the devtools Dockerfile; neither is optional in the image.
REQUIRED_PYLIBS=(pydantic sqlalchemy sqlalchemy_continuum yaml psycopg2 requests dotenv alembic claude_agent_sdk)
...
# --- Image -----------------------------------------------------------------------------------
# Pull by digest, never build. Nothing here can produce a toolbox: a project that could build its
# own would have a second toolbox, free to differ from the published one, which is the opposite of
# what this container is for — and that now includes the main repository, whose toolbox is published
# by `scripts/dev/master/push_devtools_image_to_registry.sh` and recorded in the lock like anyone
# else's. `devtools_ensure_locked_image` holds the rule: absent → pull the locked digest; present and
# different → a hard failure on a pinned version, a pull-and-retag on `latest`.
ensure_image() {
  devtools_ensure_locked_image "$IMAGE" "$DIGEST" "$VERSION" && return 0
  die "could not obtain $IMAGE.
The dev tools image is pulled from the registry by the digest framework.lock.json records; nothing builds one here.
  - if the package is private or you are not logged in:  docker login ghcr.io
  - if the version is wrong, edit the one line in framework.env (IDEABLE_FRAMEWORK_VERSION, currently
    '$VERSION') and run scripts/dev/module/sync-template-updates.sh so the lock follows it"
}
