...
validate_consumed_image_tag() {
  ...
  # host_app is not a third-party module a project happens to consume: it IS the framework, and
  # which release of it a project runs is what `framework.env` already says. A `consumedImageTag`
  # there would be a second answer to one question, free to disagree with the lock — the shape of
  # defect the framework-version work exists to remove. So for host_app the field is forbidden
  # rather than required, and the release comes from framework.lock.json.
  if [[ "${module_name}" == "host_app" ]]; then
    if python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); sys.exit(0 if (d.get("consumedImageTag") or "").strip() else 1)' \
         "${module_dir}/module.json" 2>/dev/null; then
      echo "ERROR: [host_app] declares 'consumedImageTag' in module.json." >&2
      echo "  Which host_app release this project runs comes from framework.env + framework.lock.json," >&2
      echo "  so a declaration here is a second answer to the same question and can disagree with it." >&2
      echo "  ACTION: remove 'consumedImageTag' from modules/host_app/module.json; set the release in" >&2
      echo "          framework.env (IDEABLE_FRAMEWORK_VERSION) and run sync-template-updates.sh." >&2
      return 1
    fi
    return 0
  fi
...
PY
  ...
  # gates live in scripts/dev/common/schema.sh verify. Both matter — this one catches the rule being
  ...
  if [[ -d "${module_dir}/backend/SOURCES/alembic" ]]; then
    ...
    if [[ -f "${module_dir}/docker-compose.yml" ]] &&
      ...
      echo "       Move it to a migration: scripts/dev/common/schema.sh migration ${module_name} -m '...'" >&2
      return 1
    fi
    ...
    while IFS= read -r sql_file; do
      ...
      if [[ "${base}" == "schema.sql" ]]; then
        if ! head -1 "${sql_file}" | grep -qE "GENERATED|RETIRED"; then
          ...
          echo "       Regenerate it: scripts/dev/common/schema.sh schema-sql ${module_name}" >&2
          return 1
        fi
        continue
      fi
      ...
      if grep -qiE "${ddl}" "${sql_file}"; then
        ...
        echo "       migration: scripts/dev/common/schema.sh migration ${module_name} -m '...'" >&2
        return 1
      fi
...
PYSEED
  ...
  # --- the framework DDL an entity's declarations promise --------------------------------------
  # REPORTED HERE, ENFORCED IN THE SUITE — and the split is deliberate rather than timid.
  #
  # `check_entity_ddl.py` had no caller at all: it was a checker nobody ran, which is how a folder
  # fills with scripts that answer questions no pipeline asks. Running it here puts its findings in
  # front of the person validating the module, at the moment they are validating it.
  #
  # It does NOT fail this script. The binding assertion is
  # `scripts/TESTS/test_entity_ddl_is_complete.py`, which carries a `KNOWN_NONCOMPLIANT` baseline
  # that can be pruned as the gaps close. Failing here instead would either block every deploy on
  # a baselined finding or force that baseline into this script, where nothing shrinks it.
  #
  # Guarded on `models.py` because the checker's own discovery is: asked for a module that has no
  # backend models it reports "the discovery is broken" and exits 1, which is true of the question
  # and not of the module.
  if [[ -f "${module_dir}/backend/SOURCES/app/models.py" ]]; then
    if ! python3 "${SCRIPT_DIR}/check_entity_ddl.py" "${module_name}"; then
      echo "[${module_name}] entity-DDL findings reported above — they do NOT fail validation;" \
           "scripts/TESTS/test_entity_ddl_is_complete.py is where they are enforced"
    fi
  fi
