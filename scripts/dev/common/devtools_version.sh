...
# EVERY registry digest the local image was pulled under, for the image's own repository, one per
# line — empty when it was never pulled (a hand-built image has no RepoDigests).
#
# AN IMAGE LEGITIMATELY CARRIES SEVERAL, and a release is when it happens. Publishing a release
# whose content is identical to the channel it came from produces a second manifest digest over the
# same bytes; pull both and docker records both `RepoDigests` on one local image. Asking for the
# first one and comparing it to the lock then fails on an image that IS the locked one — which is
# how the toolbox came to refuse to start on a freshly published, correctly pinned version, telling
# the operator to replace an image that needed no replacing.
devtools_local_digests() {  # <image:tag>
  local image="$1" repo="${1%:*}"
  docker image inspect "$image" --format '{{join .RepoDigests "\n"}}' 2>/dev/null \
    | sed -n "s#^${repo}@##p"
}

# One of them, for a diagnostic that has to print something. Never for the comparison: see above.
devtools_local_digest() {  # <image:tag>
  devtools_local_digests "$1" | head -1
}
...
devtools_ensure_locked_image() {  # <image:tag> <digest> <version>
  ...
  # Membership, not equality: the locked digest has to be AMONG the ones this image carries.
  devtools_local_digests "$image" | grep -qxF "$digest" && return 0
  actual="$(devtools_local_digests "$image" | paste -sd, -)"
