...
# DEV IMPORTS RUNTIME, AND NEVER THE REVERSE.
#
# `compose_merge` and `framework_version` live in `scripts/runtime/` because they have to be at a
# deployed site: the site-time merge runs one and the site-time puller runs the other. There is one
# copy of each, imported from there by this deploy — when the merge existed as two copies they
# drifted, and the deploy-site copy overwrote the correct output with a stale result. Runtime never
# imports anything from here, because none of this exists at a site.
sys.path.insert(0, os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "runtime"))
import module_deps  # noqa: E402  (sibling module in scripts/dev/common/)
import compose_deps  # noqa: E402  (sibling module in scripts/dev/common/)
import verify_build_identity  # noqa: E402  (sibling; the same check redeploy.sh runs at the end)
import framework_version  # noqa: E402  (scripts/runtime/; the one reader of framework.env + framework.lock.json)
import compose_merge  # noqa: E402  (scripts/runtime/; the same merge the deployed site runs)
...
#: The tag every image built HERE carries. A local build is a moving target by nature — it is
#: whatever the working tree was when `redeploy.sh` ran — and Docker's own default name says so.
#: What makes it traceable is not the name but the labels stamped at build time (`build_identity`):
#: the commit, the build time, and whether the tree was dirty. A published image gets a real name
#: at publish time (`push_module_images_to_registry.py`), and a deployed site consumes published
#: versions and never builds, so nothing built here is a rollback target — rollback is a version
#: change, not a tag change. `rules/general-guidelines.md` § *One version names the framework*.
LOCAL_IMAGE_TAG = "latest"

#: The one spare each locally built repository keeps beside `latest`, so "the previous build worked
#: and this one does not" still has something to reach for. See `rotate_previous_image`.
PREVIOUS_IMAGE_TAG = "previous"

#: OCI label keys stamped on every image this script builds. `verify_build_identity.py` reads the
#: first one back after the build and `status.sh` prints all three per container.
...
def local_image_tag():
    """The tag every image this run builds carries — always `LOCAL_IMAGE_TAG`."""
    return LOCAL_IMAGE_TAG


def build_identity():
    """`{revision, created, dirty}` for the images this run builds, read once from git.

    `revision` is the short commit, `created` the build time in UTC, `dirty` "true" when the
    working tree had uncommitted changes — so an image can always say what it was built from, even
    though its name cannot. A tree that is not a git checkout aborts: an image nobody can trace to
    a commit is the thing the labels exist to prevent, and quietly labelling it `unknown` would
    restore that for exactly the case that needs it most.
    """
    global _BUILD_IDENTITY_CACHE
    if _BUILD_IDENTITY_CACHE:
        return _BUILD_IDENTITY_CACHE

    def _git(*args):
        try:
            out = subprocess.run(["git", "-C", PROJECT_ROOT, *args],
    ...
    if not sha:
        print("ERROR: cannot record what this build is made from.")
        print("  This is not a git checkout (or git is unavailable), so the commit cannot be")
        print("  stamped on the images. Build from a checkout of the repository.")
        sys.exit(1)
...
def image_label_args():
    """The `--label` arguments every `docker build` in this run receives."""
    ident = build_identity()
    return [
        "--label", f"{LABEL_REVISION}={ident['revision']}",
        "--label", f"{LABEL_CREATED}={ident['created']}",
        "--label", f"{LABEL_DIRTY}={ident['dirty']}",
    ]


def export_build_identity_env():
    """Hand the identity to a sub-module's own `SPECS/build.sh`, which owns its `docker build -t`.

    Passing it through the environment keeps the scripts' signatures and makes an omission loud:
    each one reads `${LOCAL_IMAGE_TAG:?}` and the three `IDEABLE_IMAGE_*` values with `:?`, so a
    build.sh added later cannot quietly produce an unlabelled image.
    """
    ident = build_identity()
    os.environ["LOCAL_IMAGE_TAG"] = local_image_tag()
    os.environ["IDEABLE_IMAGE_REVISION"] = ident["revision"]
    os.environ["IDEABLE_IMAGE_CREATED"] = ident["created"]
    os.environ["IDEABLE_IMAGE_DIRTY"] = ident["dirty"]


#: `image:` lines that name one of a module's OWN images (registry prefix + module slug + sub-module),
#: with the local tag or the retired `${IMAGE_TAG}` placeholder a compose file from an older
#: framework version may still carry. Third-party images (`postgres:16-alpine`) never match.
_MODULE_IMAGE_REF = re.compile(
    r'(image:\s*\$\{MODULE_DOCKER_REGISTRY_PREFIX\}/[^\s:"\']+):(?:' + LOCAL_IMAGE_TAG + r'|\$\{IMAGE_TAG\})([ \t]*)$',
    re.MULTILINE,
)


def bake_consumed_tag(compose_content, tag):
    """Replace the tag on a module's own image references with the release this project consumes."""
    return _MODULE_IMAGE_REF.sub(lambda m: f"{m.group(1)}:{tag}{m.group(2)}", compose_content)
...
def consumed_image_tag(module_name, module_meta):
    ...
    `local_image_tag()` answers *"what did this run build"* (always `latest`, traced by its labels). A module enabled as `remote` in
    ...
    `pull-hostapp-images.sh` fetched `latest` while the deployed compose resolved the tag to
    ...
    is what `rules/general-guidelines.md` § *A local build is `latest`, and its identity is on the image* exists to prevent, and a
...
def image_name(module_name, submodule_name, module_slug=None, project_slug=None):
    ...
    (e.g. hostapp.backend:latest, template.frontend:latest) — see `LOCAL_IMAGE_TAG` for why a
    local build is named this way and traced by its labels. The MODULE_SLUG comes from module.json. The optional
    ...
    return f"{module_slug_value}.{submodule_name.lower()}:{local_image_tag()}"
...
#: Every image repository (name without tag) this run built. Populated by the two build paths,
#: read by `prune_superseded_images`, which must never touch a repository this run does not own:
#: a developer machine hosts other projects' images, and a blanket prune would take them.
_BUILT_REPOSITORIES: set = set()

#: Image ids that `rotate_previous_image` displaced this run — the build before the spare. Removed
#: after the deploy unless a container still uses them.
_SUPERSEDED_IMAGE_IDS: set = set()


def _record_built_repository(image_reference):
    """Remember `repo` out of `repo:tag`, ignoring a registry port in the host part."""
    reference = str(image_reference or "")
    if not reference:
        return
    last_segment = reference.rsplit("/", 1)[-1]
    repo = reference[: -(len(last_segment) - last_segment.index(":"))] if ":" in last_segment else reference
    _BUILT_REPOSITORIES.add(repo.rstrip(":"))


def _image_id(reference):
    """The image id `reference` names locally, or '' when it names nothing."""
    out = subprocess.run(["docker", "image", "inspect", "--format", "{{.Id}}", reference],
                         capture_output=True, text=True)
    return out.stdout.strip() if out.returncode == 0 else ""


def rotate_previous_image(repo):
    """Before `repo:latest` is rebuilt, keep the build it replaces as `repo:previous`.

    Every local build is `latest`, so without this the image a rebuild replaces becomes an
    anonymous dangling layer — un-nameable, and yet exactly what a developer reaches for when "the
    previous build worked and this one does not". One spare bounds the growth without taking that
    away; the build the spare itself replaces is remembered and pruned after the deploy.

    Measured before this existed: 171 superseded images accumulated over one day of iteration,
    filled the daemon filesystem, and made every `apt-get update` in a later build fail with "at
    least one invalid signature was encountered" — a truncated download reported as a signature
    error. That is why the displaced ids are pruned rather than left for `docker image prune`.
    """
    current = _image_id(f"{repo}:{LOCAL_IMAGE_TAG}")
    previous = _image_id(f"{repo}:{PREVIOUS_IMAGE_TAG}")
    if previous and previous != current:
        _SUPERSEDED_IMAGE_IDS.add(previous)
    if current:
        subprocess.run(["docker", "tag", f"{repo}:{LOCAL_IMAGE_TAG}", f"{repo}:{PREVIOUS_IMAGE_TAG}"],
                       capture_output=True, text=True)


def prune_superseded_images():
    """Delete the image ids this run's rotation displaced, keeping `latest` and `previous`.

    Only ids recorded by `rotate_previous_image` for repositories THIS run built are candidates —
    never a pattern, never another project's images, never a volume (databases and backups live
    there). An image a container still uses is skipped: docker refuses, and that is the correct
    outcome rather than an error.
    """
    if os.environ.get("IDEABLE_KEEP_SUPERSEDED_IMAGES") == "1":
        print("  keeping superseded images (IDEABLE_KEEP_SUPERSEDED_IMAGES=1)")
        return
    still_named = {_image_id(f"{repo}:{tag}") for repo in _BUILT_REPOSITORIES
                   for tag in (LOCAL_IMAGE_TAG, PREVIOUS_IMAGE_TAG)}
    removed = 0
    for image_id in sorted(_SUPERSEDED_IMAGE_IDS - still_named):
        if not image_id:
            continue
        gone = subprocess.run(["docker", "rmi", image_id], capture_output=True, text=True)
        if gone.returncode == 0:
            removed += 1
    if removed:
        print(f"  removed {removed} superseded image(s); kept {LOCAL_IMAGE_TAG} and {PREVIOUS_IMAGE_TAG} "
              f"for {len(_BUILT_REPOSITORIES)} repositor{'y' if len(_BUILT_REPOSITORIES) == 1 else 'ies'}")
...
def build_submodule(module_name, submodule_name, submodule_path, module_slug=None, compose_image=None, project_slug=None):
    ...
    if has_dockerfile:
        ...
        if "${" in img:
            ...
            img = (
                img.replace("${APP_SLUG}", resolved_project_slug)
                   ...
                   .replace("${IMAGE_TAG}", local_image_tag())
            )
        ...
        rotate_previous_image(img.rsplit(":", 1)[0])
        build_cmd = ["docker", "build", "--no-cache", "-t", img, *image_label_args()]
        ...
        # The build is the thing that runs: a failed `docker build` leaves the PREVIOUS image under
        # `latest`, so the label is read back and compared with HEAD before going any further.
        ok, message = verify_build_identity.check_image(img, build_identity()["revision"])
        print(f"  [{submodule_name}] {message}")
        if not ok:
            sys.exit(1)
...
def deploy_module_root(module_name, module_path, module_meta, project_slug=None, is_remote=False):
    ...
    if os.path.exists(src_compose):
        ...
        # A CONSUMED module's tag is resolved here, into its own compose file. A module's source
        # compose names its own images `:latest` — the tag a local build produces — and that is the
        # right answer for a module built here and the wrong answer for one pulled from a registry.
        # Baking the consumed release into the module's compose is the same treatment ${MODULE_SLUG}
        # already gets, and for the same reason: the value is per module, and the merged env
        # namespace is not.
        if is_remote:
            content = bake_consumed_tag(content, consumed_image_tag(module_name, module_meta))
...
def merge_env_files(enabled_module_data, remote_hostapp=False, project_env_path="", remote_module_names=None):
    ...
    with open(dst_config, "w", encoding="utf-8") as f:
        ...
        # No build identity is written here. Which build a container runs is a fact about the
        # image — its labels, stamped at build time — not a value in this file; and which release a
        # consumed module runs is resolved into that module's compose from the lock. An env key for
        # either is what once had to be preserved across two writers of this file, and lost.
        f.write("\n")
...
def _resolve_compose_image_for_submodule(module_name, module_slug, submodule_name, service_images):
    ...
    # A sub-module's service may carry a suffix of its own: host_app's `authentik` sub-module builds
    # the `authentik-bootstrap` service's image. The prefix is the sub-module; the suffix is the role.
    # Only the module's OWN images qualify here (`${MODULE_SLUG}.<name>`): `authentik-database` also
    # starts with the sub-module's name and runs a stock postgres image nobody here builds.
    for svc, image in service_images.items():
        svc_lower = svc.lower()
        if (svc_lower.startswith(f"{sub_lower}-") or svc_lower.startswith(f"{sub_lower}_")) \
                and "${MODULE_SLUG}." in image:
            return image
...
def copy_runtime_scripts():
    """Copy `scripts/runtime/` into `deployment_root/scripts/` — the whole folder, no exception list.

    THE FOLDER IS THE DEFINITION OF WHAT A SITE GETS. This step used to take `runtime/config/*` and
    then add `common/compose_merge.py`, `common/framework_version.py` and
    `module_only/pull-module-deployable-from-git.sh` by hand, so membership of the deployed set was
    decided here in Python rather than by where a file sat. It cost exactly what that arrangement
    costs: `runtime/authz.sh` was in `runtime/` and outside `runtime/config/`, so it never reached a
    site at all — while its own header says it exists because at production a devops has only the
    runtime scripts. There is now nothing to keep in sync: add a file to `scripts/runtime/` and it
    is deployed; that equality is asserted by `test_runtime_is_the_deployed_folder.py`.

    `start.sh`, `stop.sh` and `status.sh` are ADDITIONALLY placed at the deployment root, which is
    where a devops runs them. They resolve the deployment root beside-or-above themselves, so the
    copy inside `scripts/` works too rather than being an inert duplicate.

    Also placed here: the two things a site needs to name the release it runs — `framework.lock.json`
    at the deployment root, written through the resolver, and the resolver itself, which travels
    with the folder like everything else. Not `framework.env`: a site inherits its release rather
    than choosing one. See the comment at that step.
    """
    runtime_dir = os.path.join(PROJECT_ROOT, "scripts", "runtime")
    if not os.path.isdir(runtime_dir):
        print(f"ERROR: runtime scripts folder not found: {runtime_dir}")
        print("       It IS the deployed scripts folder; there is nothing to deploy without it.")
        sys.exit(1)
    ...
    # ONE directory copy. `__pycache__` is not a file anyone wrote, so it is not part of the folder.
    copied = 0
    for entry in sorted(os.listdir(runtime_dir)):
        src = os.path.join(runtime_dir, entry)
        if not os.path.isfile(src) or entry == "__pycache__":
            continue
    ...
    if not copied:
        print(f"ERROR: {runtime_dir} holds no files to deploy.")
        sys.exit(1)

    # The three a devops runs from the deployment root, additionally placed there.
    for script_name in ("start.sh", "stop.sh", "status.sh"):
        src = os.path.join(runtime_dir, script_name)
        if not os.path.isfile(src):
            print(f"ERROR: runtime wrapper not found: {src}")
            print("       The project root's ./start.sh, ./stop.sh and ./status.sh exec these.")
            sys.exit(1)
    ...
    # THE RELEASE TRAVELS TO THE SITE; THE CHOICE STAYS IN THE CHECKOUT.
    #
    # `pull-hostapp-images.sh` has to answer "which host_app release do I run?" at a deployed site,
    # and a deployable bundle is `cp -r deployment_root/.` — so the files that answer it have to be
    # inside that bundle. The resolver is one of them, and it needs no special handling here: it
    # lives in `scripts/runtime/` and was copied with the folder above.
    #
    # `framework.env` is deliberately NOT copied. It is the one line a module maintainer edits, and
    # a deployment has no business re-deciding at deploy side what was decided at coding time: a
    # site INHERITS its release. A choice file at a site is precisely the thing that could diverge
    # from it, so the site gets the facts and no choice to contradict them.
    #
    # The lock is written THROUGH the resolver rather than copied byte-for-byte, because reading is
    # what enforces that it was published for the version `framework.env` asks for. A disagreeing
    # pair fails the deploy here, instead of being carried to a site as though it were the answer.
    wanted, lock = framework_version.deployed_release(Path(PROJECT_ROOT))
    lock_dst = framework_version.write_lock(lock, Path(DEPLOYMENT_ROOT_DIR))
    print(f"Copied framework lock → {lock_dst} (release {wanted})")


def run_module_validation(module_names=None):
    validator = os.path.join(PROJECT_ROOT, "scripts", "dev", "common", "validate_modules.sh")
...
def deploy_remote_hostapp(remote_hostapp_compose, project_slug):
    """Produce `deployment_root/modules/host_app/` for a project that CONSUMES host_app.

    This is the deploy path of every real module project and of none of this repository's own runs:
    here host_app is enabled `local` and built from source, so the branch below is never entered.
    Both defects that made framework 1.6.5 undeployable for a consuming project lived on these
    lines — a `str` handed to the release resolver on the first of them, and an identity check
    applied to an image nobody here builds just before it — and neither could be seen by a test,
    because this code was inline in `if __name__ == "__main__":` where nothing can call it. It is a
    function so that a staged project can be deployed by a test; see
    `scripts/TESTS/test_the_consumed_hostapp_deploy_lands_a_site.py`.

    What it does, in the order a site needs it: read the module's own compose and rewrite it so the
    relative bind mounts resolve under `deployment_root/modules/host_app/` rather than the
    deployment root; point the `/modules` mount at every enabled module's deployed config; resolve
    the slug placeholders; bake the consumed release and registry, both from the framework lock, so
    compose names images the publish actually produced; make the networks creatable rather than
    external; then copy `config/`, `module.json`, the env examples and any sub-module `DIST/`.

    Returns the deployed compose path for the caller's merge list.
    """
    ...
    # And its consumed release, which for host_app comes from the FRAMEWORK LOCK rather than
    # from a field in its module.json. host_app is not a third-party module a project happens to
    # consume: it is the framework, and which release of it a project runs is the one thing
    # `framework.env` already says. A second declaration would be a second answer to that
    # question, free to disagree — which is the shape of the defect this whole plan removes.
    #
    # The registry comes from the lock too, so `MODULE_DOCKER_REGISTRY_PREFIX` in host_app's
    # `.env.config` cannot redirect images the project does not own.
    hostapp_registry, hostapp_version = framework_version.hostapp_consumed_ref(Path(PROJECT_ROOT))
    hostapp_content = bake_consumed_tag(hostapp_content, hostapp_version)

    # Resolve MODULE_DOCKER_REGISTRY_PREFIX at compose generation time.
    # Compose files keep the slash: image: ${MODULE_DOCKER_REGISTRY_PREFIX}/${MODULE_SLUG}.backend:latest
    hostapp_content = _resolve_docker_registry_prefix(
        hostapp_content, {"MODULE_DOCKER_REGISTRY_PREFIX": hostapp_registry},
        keep_registry_prefix=True,
    )
    ...
    return "modules/host_app/docker-compose.yml"
...
if __name__ == "__main__":
    ...
    for module_name, module_path, module_meta, mode in enabled_module_data:
        ...
        for submodule_name in submodules:
            ...
            if custom_script:
                ...
                # A SPECS/build.sh owns its own `docker build -t`, so it is told the tag and the
                # identity labels through the environment (see export_build_identity_env). Its
                # image is rotated and verified like a generic build's — and WHICH image that is
                # comes from the compose file, not from the sub-module's name: `authentik` builds
                # `hostapp.authentik-bootstrap`, and guessing `hostapp.authentik` failed the first
                # BuildDeploy of the label scheme.
                compose_image = _resolve_compose_image_for_submodule(
                    module_name, module_slug, submodule_name, compose_images,
                )
                if compose_image is None:
                    ...
                    continue
                export_build_identity_env()
                own_image = (
                    compose_image.replace("${APP_SLUG}", project_slug or get_app_slug())
                                 .replace("${MODULE_SLUG}", module_slug)
                                 .replace("${IMAGE_TAG}", local_image_tag())
                )
                own_image = _resolve_docker_registry_prefix(
                    own_image, read_module_env_map(module_path), keep_registry_prefix=False
                )
                own_repo = verify_build_identity.image_repository(own_image)
                # A `SPECS/build.sh` need not produce an image: a database sub-module running a
                # third-party engine materializes SQL into DIST/ and its service names, say,
                # `timescale/timescaledb:${VERSION}`. Rotation, the built-repository record and the
                # identity check are all statements about an image THIS deploy built, so they are
                # made only when it did — see verify_build_identity.names_a_local_build.
                builds_own_image = verify_build_identity.names_a_local_build(own_image, module_slug)
                if builds_own_image:
                    _record_built_repository(own_image)
                    rotate_previous_image(own_repo)
                run(
                    ["bash", custom_script],
                    failure_context=(
                ...
                if builds_own_image:
                    ok, message = verify_build_identity.check_image(own_image, build_identity()["revision"])
                    print(f"  [{submodule_name}] {message}")
                    if not ok:
                        sys.exit(1)
                else:
                    print(
                        f"  [{submodule_name}] build identity not verified: the compose service runs "
                        f"{own_image}, which is not an image this deploy builds "
                        f"(`{module_slug}.<sub-module>`). A third-party or consumed image carries the "
                        f"publisher's revision, not this checkout's."
                    )
            else:
                ...
                # PULLED from the registry by `scripts/dev/common/tool.sh` and, as its own spec says, "is
    ...
    if remote_hostapp_compose:
        deployed_compose_files.append(
            deploy_remote_hostapp(remote_hostapp_compose, project_slug)
        )
