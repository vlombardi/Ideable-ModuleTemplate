...
#
# Usage:
#   scripts/dev/common/schema.sh design    [module]   # materialise <db>_design; print the import recipe
#   scripts/dev/common/schema.sh model     [module]   # sqlacodegen <db>_design → candidate models.py
#   scripts/dev/common/schema.sh migration [module] -m "message"   # autogenerate against <db>_head
#   scripts/dev/common/schema.sh verify    [module]   # the acceptance gates (what CI runs)
#   scripts/dev/common/schema.sh schema-sql[module]   # regenerate the derived full-schema SQL
#   scripts/dev/common/schema.sh squash    [module]   # maintainer: collapse versions/ into a baseline
set -euo pipefail
...
if [[ "${IDEABLE_IN_TOOL_CONTAINER:-0}" != "1" && "${IDEABLE_NO_CONTAINER:-0}" != "1" ]]; then
  if [[ ! -x "${REPO_ROOT}/scripts/dev/common/tool.sh" ]]; then
    echo "scripts/dev/common/tool.sh is missing — it is how this project obtains its toolchain." >&2
    exit 1
  fi
  exec "${REPO_ROOT}/scripts/dev/common/tool.sh" bash "${REPO_ROOT}/scripts/dev/common/schema.sh" "$@"
fi
...
design)
  ...
  ${BOLD}Then apply your exported DDL to the design database${NC}, and continue with:
        scripts/dev/common/schema.sh model ${MODULE}
...
model)
  ...
  Then:  scripts/dev/common/schema.sh migration ${MODULE} -m "your message"
EOF
  ;;
...
migration)
  ...
  Then:                      scripts/dev/common/schema.sh verify ${MODULE}
EOF
  ;;
...
schema-sql)
  ...
  in_backend "${DB_NAME}_schemagen" alembic upgrade head >/dev/null
  {
    ...
    echo "--"
    echo "-- Regenerate:  scripts/dev/common/schema.sh schema-sql ${MODULE}"
    echo ""
    docker exec -e PGPASSWORD="${DB_PASS}" "${DB_CONTAINER}" \
...
squash)
  ...
  1. Build the end state:      scripts/dev/common/schema.sh design ${MODULE}
  ...
  4. Prove it:                 scripts/dev/common/schema.sh verify ${MODULE}
