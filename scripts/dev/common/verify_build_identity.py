...
revision, not this checkout's, so only images of modules enabled `local` are compared. A
THIRD-PARTY image is the same case and was not excluded: a sub-module whose `SPECS/build.sh`
produces files rather than an image runs something like `timescale/timescaledb:<version>`, which
carries no revision from this checkout either. `names_a_local_build` is the one place that decides
whether a reference is this deploy's own build, and both the per-image and the set-wide entry
point ask it.
...
#: An `image:` line's reference, quoted or bare. Which of them this checkout BUILT is decided by
#: `names_a_local_build`, so the rule lives in one place instead of inside this pattern.
_IMAGE_LINE = re.compile(r"^\s*image:\s*[\"']?([^\s\"']+)[\"']?\s*$", re.MULTILINE)


def image_repository(reference):
    """`repo` out of `repo:tag`, leaving a registry port alone (`localhost:5000/x` has no tag)."""
    return reference.rsplit(":", 1)[0] if ":" in reference.rsplit("/", 1)[-1] else reference


def names_a_local_build(reference, slug):
    """Whether `reference` names an image THIS deploy builds for the module whose slug is `slug`.

    Every image built here is `<slug>.<submodule>` with no registry — the deploy strips the prefix
    — so the repository part answers the question on its own, and it answers it without expanding
    anything: `timescale/timescaledb:${TIMESCALEDB_VERSION}` is not this checkout's build whether or
    not the variable resolves.

    Deciding on the repository and never on the tag is deliberate. A predicate that gave up at the
    first `${` would also let go of an image this deploy DID build whose tag carries a variable, and
    silently switching the identity gate off for those would be worse than the failure it fixes.

    A third-party image is the case that matters and the one that was missing: `postgres:16-alpine`
    and `timescale/timescaledb` carry the publisher's revision, not this checkout's — the same
    reason this module's docstring gives for excluding consumed images.
    """
    return image_repository(reference).startswith(f"{slug}.")
...
def local_module_images(repo_root=_PROJECT_ROOT):
    ...
    with open(compose, encoding="utf-8") as handle:
        for match in _IMAGE_LINE.finditer(handle.read()):
            reference = match.group(1)
            if not reference.endswith(f":{LOCAL_IMAGE_TAG}"):
                continue
            if not any(names_a_local_build(reference, slug) for slug in slugs):
                continue
