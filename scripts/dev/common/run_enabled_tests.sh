...
#
# Without the exclusion the field was `dirty` on EVERY run that had a plan active, for a reason no
# amount of committing could fix: `implementation-plans/ACTIVE PLAN - <description>.md` is a symlink,
# and inside the dev tools container — where this script runs — the bind mount answers
# `readlink()` with EINVAL, so git cannot hash it and reports it modified forever. The one run on
# 2026-09-13 that recorded `clean` was the one taken after a delivery, when no such link existed.
# `deliver` refuses a dirty certificate, so a plan could not be landed while it was active — which
# is the only time anyone delivers.
tree_at_start="clean"
