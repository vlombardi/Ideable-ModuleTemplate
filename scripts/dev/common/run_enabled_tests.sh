...
# Usage: ./scripts/dev/common/run_enabled_tests.sh [-h|--help]
#
...
if [[ "${IDEABLE_IN_TOOL_CONTAINER:-0}" != "1" && "${IDEABLE_NO_CONTAINER:-0}" != "1" ]]; then
  if [ ! -x scripts/dev/common/tool.sh ]; then
    echo "scripts/dev/common/tool.sh is missing — it is how this project obtains its toolchain." >&2
    exit 1
  fi
  exec scripts/dev/common/tool.sh bash scripts/dev/common/run_enabled_tests.sh "$@"
fi

# Where the running stack is, seen from inside the container. One definition, shared with
# scripts/dev/module/run_tests.sh — see that file for the whole reasoning.
# shellcheck source=scripts/dev/common/container_stack_env.sh
...
#
# BOOKKEEPING PATHS DO NOT COUNT, and the reason is not tidiness. The question is whether the suite
# ran against committed CODE; `implementation-plans/`, `kanban/`, `TEST_REPORTS/` and the framework
# version files are what a run and a delivery rewrite by construction, and `.githooks/pre-push`
# already excludes exactly this set when it compares a tested tree with HEAD.
#
# Without the exclusion the field is `dirty` on EVERY run that has a plan active, for a reason no
# amount of committing can fix. `implementation-plans/ACTIVE PLAN - <description>.md` is a symlink
# the router REPOINTS at every transition, and each state name has its own length. Docker Desktop's
# bind mount keeps serving the size it cached for that link, so once the host has repointed it the
# cached length no longer matches the target the mount returns: inside the dev tools container —
# where this script runs — `readlink()` fails with EINVAL, git cannot hash the entry, and it reads
# modified forever while the same tree is clean on the host. `deliver` refuses a dirty certificate,
# so a plan could not be landed while it was active — which is the only time anyone delivers.
#
# THE CONTAINER READS SYMLINKS, and EINVAL here does not mean the path is not one — `lstat` says it
# is. Two earlier explanations of this exclusion claimed each of those in turn, and the repoint is
# what both were missing. Reproduced 2026-09-13 in four commands: read a host-made link inside the
# container (fine, as are four others with spaces and parens in their names), repoint it from the
# host to a target of a different length, read it again (EINVAL), and `git status` inside the
# container then reports ` M` on a tree the host calls clean.
tree_at_start="clean"
if [ -n "$(git status --porcelain 2>/dev/null | grep -Ev '^(..) "?(implementation-plans/|kanban/|TEST_REPORTS/|framework\.env|framework\.lock\.json)')" ]; then
  tree_at_start="dirty"
fi
...
# A persisted node_modules holds a SNAPSHOT of @ideable/ui, and a snapshot goes stale.
#
# `--install-links` is mandatory for the frontend images (`modules/*/frontend/SOURCES/Dockerfile`,
# `modules/host_app/SPECS/dependencies.md` § *npm dependencies are pinned by lockfile*): it COPIES
# `@ideable/ui` instead of symlinking it, which is what makes the parent lock pin the shared
# library's own dependencies and the image reproducible. In an image that is harmless — it is built
# from the tree it copies, once.
#
# The dev tools container's node_modules is a host-cache directory that PERSISTS between runs, and
# the install below is skipped whenever it is non-empty. So the copy outlives the library: every
# later change to `reusable.ui/` is invisible to everything that resolves `@ideable/ui` in that
# container — `tsc`, and the dev server Playwright starts. On the host npm symlinks the same
# `file:` dependency, so the developer's own checkout is live and the container is a month behind.
#
# Measured: a widget-library export added in this repository failed `tsc` in the container with
# "Module '@ideable/ui' has no exported member" while the same command passed on the host. The
# UI suite was the quieter half of it — it had been building pages against a stale widget library
# and reporting them green.
#
# So the library's own files decide, not the directory's existence: anything under `reusable.ui/`
# newer than the installed snapshot means the snapshot is not the library any more.
#
# Absolute, because the second caller below runs inside a subshell that has `cd`-ed into the
# Playwright harness: a relative `reusable.ui` there names nothing and the check would answer
# "not stale" for every tree, which is the failure it exists to catch.
_REUSABLE_UI="$(pwd)/reusable.ui"
_shared_ui_snapshot_is_stale() {  # <frontend SOURCES dir>
  local installed="$1/node_modules/@ideable/ui"
  # No shared library here (a project may not vendor one), nothing installed yet (the install is
  # the answer, not this), or a SYMLINK — which is npm's default for a `file:` dependency on the
  # host, and is the library rather than a copy of it, so it cannot go stale.
  if [ ! -d "$_REUSABLE_UI" ] || [ ! -d "$installed" ] || [ -L "$installed" ]; then
    return 1
  fi
  # FILES, and only the ones npm actually copies. A directory's mtime moves for reasons that are
  # not the library changing — a `.DS_Store`, an editor's temp file, a `dist/` build — and each one
  # would buy a full `npm ci` on a run that needed none. `node_modules/` and `dist/` are excluded
  # from the package for the same reason they are git-ignored (infrastructure-file-list.md), so a
  # change inside them is not a change to what a consumer resolves.
  [ -n "$(find "$_REUSABLE_UI" \( -path "$_REUSABLE_UI/node_modules" -o -path "$_REUSABLE_UI/dist" \) -prune -o \
            -type f -newer "$installed/package.json" -print -quit 2>/dev/null)" ]
}
...
if [[ "${IDEABLE_IN_TOOL_CONTAINER:-0}" == "1" ]]; then
  for _fe in modules/*/frontend/SOURCES; do
    ...
    if _shared_ui_snapshot_is_stale "$_fe"; then
      echo "  · reinstalling $_fe dependencies (reusable.ui changed since this environment installed it)"
      (cd "$_fe" && npm ci --install-links --legacy-peer-deps) || {
        echo "  ✗ could not reinstall $_fe dependencies — the gate and the UI suite would run" >&2
        echo "    against a stale copy of @ideable/ui" >&2
        exit 1
      }
    elif [ ! -d "$_fe/node_modules" ] || [ -z "$(ls -A "$_fe/node_modules" 2>/dev/null)" ]; then
...
_require_tool() {
  ...
  echo "    incomplete: run scripts/dev/common/tool.sh --doctor."
  gate_rc=1
  return 1
}
    ...
    if ! command -v npm >/dev/null 2>&1; then
      ...
      (
        ...
        if [ -n "$_fe_src" ] && [ -f "$_fe_src/package.json" ]; then
          # Stale as well as absent: `--install-links` COPIES @ideable/ui into this tree, and this
          # tree persists, so the dev server builds the pages the UI suite asserts on against
          # whatever the library was when it was installed. That failure is silent — the suite goes
          # green against the old widgets — which is why it is checked here and not only at the
          # static gate above.
          if _shared_ui_snapshot_is_stale "$_fe_src"; then
            echo "  · reinstalling frontend dependencies for the dev server (reusable.ui changed)"
            (cd "$_fe_src" && npm ci --install-links --legacy-peer-deps)
          elif [ ! -d "$_fe_src/node_modules" ] || [ -z "$(ls -A "$_fe_src/node_modules" 2>/dev/null)" ]; then
