...
Usage:
  python3 scripts/dev/common/push_module_images_to_registry.py -a
  python3 scripts/dev/common/push_module_images_to_registry.py -a -t v1.0.0-latest
  python3 scripts/dev/common/push_module_images_to_registry.py host_app module_template
  python3 scripts/dev/common/push_module_images_to_registry.py -a -t v1.0.0-latest --single-arch
  python3 scripts/dev/common/push_module_images_to_registry.py -a -t v1.0.0-latest --platform linux/amd64,linux/arm64
...
def local_image_tag() -> str:
    """The tag the local images this script pushes are found under.

    Every image `build_and_deploy.py` builds is `<slug>.<submodule>:latest`; what it was built from
    is in its labels, not its name. So the local name is always the same, and this is a different
    question from `-t/--tag`, which names the release to push *to* in the registry.
    """
    return "latest"
...
class ModulePushReport:
    ...
    #: Every target ref this module was SUPPOSED to publish, recorded before any push is attempted.
    #: `pushed_images` records what the push loop believed it did; this records what was owed. The
    #: two are compared after the run, because a publish that lands some of a module's images and
    #: not others leaves a stack that cannot start — and, when the tag is a moving one, leaves the
    #: previous build's images still answering, which reads as success from the registry's side.
    ...
    #: Refs that were not found in the registry after the push. A separate list from `push_errors`
    #: because they are a different claim: not "the push reported a failure" but "the push
    #: reported nothing and the image is not there".
    missing_after_push: list[str] = field(default_factory=list)
    registry: str = ""

    @property
    def has_errors(self) -> bool:
        return bool(self.validation_errors or self.push_errors or self.missing_after_push)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="tag",
        default="",
        help="Publish this release (e.g. 1.7.0). Every image is pushed under it AND under `latest`, "
             "which a release moves because a release is the newest thing published. Refused on a "
             "dirty tree, and refused when the tag already resolves in the registry. Without it, "
...
def read_local_images(module_name: str, module_slug: str, compose_path: Path, app_slug: str = "") -> list[str]:
    ...
    for raw_line in compose_path.read_text(encoding="utf-8").splitlines():
        ...
        resolved = resolved.replace("${IMAGE_TAG}", local_image_tag())
...
def registry_manifest_exists(image_ref: str) -> bool:
    """Is this ref actually resolvable in the registry?

    The one question that was never asked. `docker manifest inspect` is the same call
    `pull-hostapp-images.sh` already makes to compare digests, and it answers from the registry
    rather than from what this process believes it did.
    """
    result = subprocess.run(
...
def verify_published(reports: list[ModulePushReport]) -> None:
    """Check every ref that was owed actually resolves in the registry, and record what does not.

    A partial publish is a failed publish, and until this ran nothing said so. The reported case:
    `hostapp.backend` and `hostapp.frontend` pulled cleanly while `hostapp.database`,
    `hostapp.traefik` and `hostapp.authentik-bootstrap` returned `manifest unknown` — so a machine
    that had never deployed the stack could not bring it up, because `authentik-bootstrap`
    provisions the identity plane and `traefik` routes to the module at all.

    Why the exit code was not enough. It is correct and it is not sufficient: it reports whether
    *this* run's pushes failed, and the failure mode here is a run whose result was never checked,
    or one that did not finish. Verification answers the durable question instead — is the release
    complete *now* — which is the question the next machine to deploy will ask.
    """
    for report in reports:
        if report.skipped_reason or not report.expected_images:
            continue
        for target_ref in report.expected_images:
            if not registry_manifest_exists(target_ref):
                report.missing_after_push.append(target_ref)
...
def push_image_multiarch(
    ...
    """Build and push a multi-arch manifest under every ref, in ONE build.

    A release publishes two names for the same bytes — `X.Y.Z` and `latest` — and buildx takes
    repeated `-t`. Building twice would produce two builds and make `latest` a different image from
    the version it is supposed to alias, which is the whole defect a release is meant not to have.
    """
    ...
    for report in reports:
        ...
        if report.push_errors:
            ...
            for error in report.push_errors:
                print(f"    - {error}")
        if report.missing_after_push:
            print("  NOT IN THE REGISTRY after the push:")
            for ref in report.missing_after_push:
                print(f"    - {ref}")
            print("    A stack cannot start from an incomplete release: authentik-bootstrap")
            print("    provisions the identity plane and traefik routes to the modules at all.")
        print()

    # The last line, because it is the one a person reads.
    #
    # A summary whose result had to be assembled from the sections above is a summary that gets
    # skimmed — three of five host_app tags were missing from the registry for a day while every
    # section of this output was individually correct.
    expected = sum(len(r.expected_images) for r in reports)
    published = expected - sum(len(r.missing_after_push) for r in reports)
    if expected == 0:
        # Nothing was owed, and WHY matters: a run that never reached the push loop (a validation
        # error, a missing registry prefix) is a failure, and reporting it as "no images to
        # publish" states a reason that is not the reason.
        blocked = [r for r in reports if r.has_errors]
        if blocked:
            print(f"Published: FAILED — 0/0. Nothing was attempted: "
                  f"{', '.join(r.name for r in blocked)} did not get as far as pushing. See the "
                  f"validation / push errors above.")
        else:
            print("Published: nothing to publish (no locally built images for the selected modules)")
    elif published == expected:
        print(f"Published: {published}/{expected} — complete")
    else:
        print(f"Published: FAILED — {published}/{expected}. A partial publish is a failed publish.")
...
def working_tree_is_dirty(root: Path = PROJECT_ROOT) -> bool:
    """True when the checkout has uncommitted changes, False when clean or not a checkout."""
    result = subprocess.run(["git", "-C", str(root), "status", "--porcelain"],
                            capture_output=True, text=True)
    return result.returncode == 0 and bool(result.stdout.strip())


def refuse_unpublishable_release(tag: str, refs: list[str], exists=registry_manifest_exists,
                                 is_dirty=None) -> str:
    """The reason `--tag` must not proceed, or '' when it may.

    Two refusals, and both are about a release being a promise that its name means one thing:

    - **A dirty tree.** The bytes came from a working tree that no commit describes, so nothing can
      ever reproduce them. `latest` moves to them too, which spreads the problem to every consumer.
    - **A tag the registry already holds.** Re-pushing a published version silently changes what
      that version means for anyone who already resolved it. Immutability used to be a property of
      the tag NAME (a commit hash); with a chosen release name it has to be a property of the
      publish, and this is where it lives.
    """
    if (is_dirty or working_tree_is_dirty)():
        return (f"refusing to publish {tag} from a dirty working tree.\n"
                f"  The images would contain changes no commit describes, and `latest` would move to\n"
                f"  them. Commit or stash first.")
    already = [ref for ref in refs if exists(ref)]
    if already:
        listed = "\n    ".join(already[:5])
        return (f"refusing to publish {tag}: the registry already holds it.\n"
                f"    {listed}\n"
                f"  Re-publishing a released version changes what that version means for everyone who\n"
                f"  already has it. Publish a new one.")
    return ""
...
def main() -> int:
    ...
    # A release is refused from a dirty tree before anything is built, because a multi-arch publish
    # REBUILDS from SOURCES: the bytes would come from a working tree no commit describes, and
    # `latest` would move to them.
    if args.tag and working_tree_is_dirty():
        print(f"ERROR: {refuse_unpublishable_release(args.tag, [])}")
        return 1

    # The release name comes from `--tag` and nowhere else. It used to fall back to
    # `IDEABLE_VERSION` in project.env.config — a third place a version could be stated, and one a
    # publisher could forget was set.
    ...
    for module_name in requested_modules:
        ...
        for local_ref in report.local_images:
            # `local_ref` is `<slug>.<submodule>:latest` — the only name a local build has. The
            # published names are that same `latest` plus, for a release, the version: one build,
            # two names, so `latest` IS the release rather than a separate image that resembles it.
            latest_ref = f"{module_registry}/{local_ref}".replace("//", "/")
            base_ref = latest_ref.rsplit(":", 1)[0]
            target_refs = [f"{base_ref}:{args.tag}", latest_ref] if args.tag else [latest_ref]
            target_ref = target_refs[0]

            # Recorded before the attempt, so a ref that is never reached — a `continue` below, an
            # interrupted run — is still owed and still verified.
            report.expected_images.extend(target_refs)

            # Checked here, before this image is built or pushed: a published release must not be
            # silently replaced. The check is per image rather than once for the whole run because
            # the registry is the authority and each ref is a separate question to it.
            if args.tag:
                refusal = refuse_unpublishable_release(args.tag, [target_refs[0]])
                if refusal:
                    report.push_errors.append(refusal)
                    errors_found = True
                    break
            ...
            if not args.single_arch:
                ...
                if sources_dir is None:
                    ...
                    errors_found = True
                    continue
                # Frontend images consume the shared @ideable/ui package (repo-root
                # reusable.ui/), which sits outside the SOURCES build context. Provide it
                # as the `ideable_ui` named build context (same mechanism as the frontend
                # SPECS/build.sh) and exclude the dev-only Widget Examples gallery from
                # published images.
                extra_build_args: list[str] = []
                if sources_dir.name == "SOURCES" and sources_dir.parent.name == "frontend":
                    reusable_ui = sources_dir.parents[3] / "reusable.ui"
                    if reusable_ui.is_dir():
                        extra_build_args += ["--build-context", f"ideable_ui={reusable_ui}"]
                    extra_build_args += ["--build-arg", "WIDGET_EXAMPLES=false"]
                print(f"[buildx] {module_name}: {local_ref} -> {', '.join(target_refs)} ({args.platform})")
                ok, output = push_image_multiarch(
                    target_refs, sources_dir, args.platform, multiarch_builder_name,
                    no_cache=args.no_cache, extra_build_args=extra_build_args,
                )
            else:
                ...
                ok, output = True, ""
                for ref in target_refs:
                    ok, output = push_image(local_ref, ref)
                    if not ok:
                        break
    ...
    # The publish is not finished until the registry says so. Runs before the summary, so what the
    # summary prints is what the registry actually holds rather than what this process attempted.
    verify_published(reports)
    if any(r.missing_after_push for r in reports):
        errors_found = True
