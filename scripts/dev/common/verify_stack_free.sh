...
if [[ "${IDEABLE_IN_TOOL_CONTAINER:-0}" != "1" && "${IDEABLE_NO_CONTAINER:-0}" != "1" ]]; then
  if [[ ! -x "$REPO/scripts/dev/common/tool.sh" ]]; then
    echo "[verify] scripts/dev/common/tool.sh is missing — it is how this project obtains its toolchain." >&2
    exit 1
  fi
  exec "$REPO/scripts/dev/common/tool.sh" bash "$REPO/scripts/dev/common/verify_stack_free.sh" "$@"
fi
...
"$REPO/scripts/dev/common/stack_free_tests.py" --by-suite > "$WORK/.suites" || exit 1
...
if [ "$rc" -ne 0 ]; then
  ...
  echo "[verify] half — teach scripts/dev/common/stack_free_tests.py what they depend on — or they read a"
