...
#: The two variables that carry a credential INTO the dev tools container, and the only route there
#: is one. The toolbox holds the Docker socket, so anything reaching it is root-equivalent on the
#: host's Docker, and a long-lived credentials FILE is the wrong thing to put there — a maintainer
#: ruling of 2026-09-14, taken over mounting `~/.claude/.credentials.json` the way `tool.sh` mounts
#: `~/.gitconfig`. It is also the only route that works everywhere: on macOS the credential lives in
#: the login Keychain and there is no file to mount at all (measured — the keychain item
#: `Claude Code-credentials` exists, `~/.claude/.credentials.json` does not), so a mount-the-file
#: design is Linux-only by construction. `scripts/dev/common/tool.sh` forwards both.
...
def missing_prerequisite(extra_options: dict | None = None, env=None, which=None) -> str | None:
    """What stops this process driving an LLM node, named — or None when nothing does.

    Both inputs are injectable so the contract test needs neither a container nor a credential.

    WHY THIS IS A CHECK AND NOT A TRY/EXCEPT. The router now runs inside the dev tools container
    (`dev-cycle.sh` re-execs through `tool.sh` like every other shipped script), and both ways it
    can be unable to drive a node in there fail LATE and OBSCURELY:

    - **No `claude` binary.** The Agent SDK is a thin wrapper that locates a Node CLI on PATH and
      spawns it, so an image without one imports fine and dies at the first node with a message
      about an executable, naming nothing a reader would connect to the framework release. Measured
      2026-09-14 against the toolbox this project currently pulls: `claude_agent_sdk` present,
      `claude` ABSENT — because the Dockerfile that adds it has not been published yet. That is the
      ordinary lag between a Dockerfile change and the publish that realises it, and it is exactly
      the case a reader needs told plainly rather than discovered.
    - **No credential.** Inside the container there is no Keychain and — by the 2026-09-14 ruling —
      no mounted credentials file, so one of `CREDENTIAL_ENV` is the whole route. Unset, the CLI
      fails with an auth error two layers from the cause.

    The credential is only required INSIDE the container, and that is not a special case for
    convenience: it is where the credential STORE differs. On the host the CLI reads the platform's
    own store (the login Keychain on macOS, `~/.claude/.credentials.json` on Linux), so an unset
    variable there says nothing about whether a credential exists. In the container neither store is
    reachable, so an unset variable is dispositive.

    This never chooses a different toolchain, and must not: `ideable-spec-driven-edit` forbids
    fallbacks, and a router quietly running on the host because the container looked unready is the
    two-environments problem the container exists to end. It refuses, and says what to do.
    """
    env = os.environ if env is None else env
    which = shutil.which if which is None else which
    in_container = (env.get("IDEABLE_IN_TOOL_CONTAINER") or "").strip() == "1"

    # An explicitly named binary is the caller's answer to this question already
    # (`DEV_CYCLE_AGENT_BIN` → `cli_path`), so it is not asked again.
    if not (extra_options or {}).get("cli_path") and not which("claude"):
        if in_container:
            return (
                "the dev tools image has no `claude` binary, which the Agent SDK locates and spawns "
                "— so this toolbox cannot drive an LLM node. The image is pulled by digest at the "
                "framework version this project is pinned to, so the fix is a framework release "
                "that contains it: publish one (scripts/dev/master/publish_framework.sh <version>) "
                "or adopt a newer one (scripts/dev/module/adopt_framework.sh <version>). To "
                "proceed on this machine "
                "meanwhile, run with IDEABLE_NO_CONTAINER=1 — which gives up the toolchain parity, "
                "so the result proves less — or drive this node yourself with --deterministic"
            )
        return (
            "no `claude` binary is on PATH, and the Agent SDK is a thin wrapper that locates and "
            "spawns one. Install it (npm install -g @anthropic-ai/claude-code), or drive this node "
            "yourself with --deterministic"
        )

    if in_container and not any((env.get(name) or "").strip() for name in CREDENTIAL_ENV):
        return (
            f"no agent credential reached the dev tools container: neither "
            f"{' nor '.join(CREDENTIAL_ENV)} is set. The container has no access to the host's "
            f"credential store (the login Keychain on macOS) and mounts no credentials file, so one "
            f"of those variables is the whole route. Mint one once with `claude setup-token` and "
            f"export CLAUDE_CODE_OAUTH_TOKEN — scripts/dev/common/tool.sh forwards it. Or drive "
            f"this node yourself with --deterministic"
        )
    return None
...
def run_agent(prompt: str, cwd, policy: str, extra_options: dict | None = None,
    ...
    """
    # Asked BEFORE the SDK is imported, because the import succeeding proves the least of the three:
    # the Python package is present in the toolbox today and the CLI it spawns is not, so an import
    # check alone reports a node ready that cannot start. Same shape as the `--doctor` probe that
    # said every lib was importable while alembic was absent.
    unmet = missing_prerequisite(extra_options)
    if unmet:
        return False, unmet
