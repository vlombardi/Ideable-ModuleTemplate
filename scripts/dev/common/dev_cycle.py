...
NODES = ["NotStarted", "Branching", "Implementing", "BuildDeploy", "Testing", "Fixing",
         "Documenting", "Committing", "Done", "Merged", "Blocked"]
...
def agent_driven_run() -> str | None:
    ...
    mystery diffs appear. There is deliberately NO override: two agents on one plan is never a
    thing to want, and the flag that used to permit it was the tempting escape from this refusal —
    so whoever hit the refusal reached for the one action the rules forbid.
    """
...
def invoke_skill_headless(node: str, skill: str) -> tuple[bool, str | None]:
    ...
    Returns `(ok, reason_if_not)`. Every way this can fail to start — an agent caller, a missing
    SDK, a bad policy, the run itself — is a reason string, so the caller falls back to suggesting
    the skill rather than dying.
    ...
    if marker:
        nxt = NEXT_SINGLE.get(node, "the next node")
        return False, (
            ...
            f"'{skill}' yourself, then advance with `./scripts/dev/common/dev-cycle.sh set {nxt}`. "
            f"Running this again refuses identically — the refusal is about WHO is calling, not "
            f"about what has been done, so `run` cannot be the way out of it. That asymmetry is "
            f"kanban/todo/the-router-accepts-a-node-performed-by-hand.md; until it lands, `set` is "
            f"the advance."
        )
    ...
    if policy == "ask":
        ...
        print(f"[dev-cycle] Policy `{policy}`: nothing will be asked. Anything it does not cover is "
              f"refused, and the agent is told to report it as not done.")
    ...
    return conversation.run_agent(prompt, REPO_ROOT, policy, extra_options=extra,
                                  quiet=quiet, label=f"{node} / {skill}")
...
def active_plan() -> Path | None:
    if not PLANS_DIR.is_dir():
        return None
    # Symlinks are excluded EXPLICITLY, not left to the sort. `ACTIVE PLAN - <description>.md` is a
    # pointer at a plan, and resolving it as one would make the router read and rewrite a plan
    # through its alias — renaming the target out from under the link on the first transition.
    plans = sorted((p for p in PLANS_DIR.glob("*.md") if not p.is_symlink()),
                   key=_plan_order, reverse=True)
...
def active_link_path(plan: Path) -> Path:
    ...
    """
    parts = _plan_parts(plan.stem)
    description = parts[2] if parts else re.sub(r"\s*\([^()]*\)\s*$", "", plan.stem).strip()
    return PLANS_DIR / f"{ACTIVE_LINK_PREFIX}{description}.md"


def sync_active_link(target: Path, node: str) -> None:
    """Point the stable name at `target`, or remove it once the plan is `Merged`.
    ...
    Removed at `Merged` rather than left pointing at the delivered file: the absence IS the signal
    that no plan is in flight. A link that survived delivery would say "a plan is active" forever,
    which is the same class of lying signal as a status line that reports success regardless.
    """
    link = active_link_path(target)
    if node == "Merged":
        if link.is_symlink() or link.exists():
            link.unlink()
            print(f"[dev-cycle] removed '{link.name}' — the plan is delivered, none is active.")
        return
    ...
    if _summary is not None:
        ...
        if _recorded and _recorded.group(1) == "dirty":
            ...
            print("[deliver] Re-run the suite on a committed tree — "
                  "`./scripts/dev/common/dev-cycle.sh set Testing && ./scripts/dev/common/dev-cycle.sh run` — "
...
def _run_one(plan: Path, auto_invoke: bool, keep_history: bool) -> tuple[bool, int]:
    ...
    if auto_invoke:
        ...
        if ok:
            ...
            # here, and a move BACKWARDS had the agent gone further (to Testing, or to Blocked). So
...
def main() -> int:
    ap = argparse.ArgumentParser(
        ...
        description="Thin, deterministic router over the Ideable dev-cycle skill graph.\n"
                    ...
                    "(Documenting | Fixing) → Committing → Done → Merged; Blocked is a human gate.\n"
        ...
        epilog=(
            ...
            "                    (NODE ∈ NotStarted, Branching, Implementing, BuildDeploy, Testing,\n"
            "                     Fixing, Documenting, Committing, Done, Merged, Blocked)\n"
            ...
            "  ./scripts/dev/common/dev-cycle.sh set Testing          # move the highlight to Testing\n"
            ...
            "                         AND its questions to you in this terminal.\n"
    ...
    ap.add_argument(
        ...
        help="run only: advance ONLY deterministic nodes (BuildDeploy/Testing); at an LLM node "
             ...
             "nesting is refused — the caller performs the node itself.",
    )
    ap.add_argument(
    ...
    if args.action == "status":
        ...
        if not args.node:
            sys.exit("`set` requires a node, e.g. `scripts/dev/common/dev-cycle.sh set Documenting`")
        ...
        # A DELIVERED plan is finished, and its file is the only surviving record of per-thing
        # detail once the branch is gone (rules/version-control.md § *Delivering a plan*). Moving it
        # backwards rewrites that record AND resurrects the branch `deliver` deleted: the next `run`
        # calls `ensure_plan_branch`, which recreates `plan/<description>` at today's HEAD. Measured
        # on 2026-09-08 — `set Documenting` on a merged plan renamed the (Merged) artifact and put
        # the checkout on a branch that had been deleted an hour earlier.
        #
        # Refused rather than warned, unlike the unfinished-scope case above: that one leaves a
        # truthful file behind, this one destroys it. The escape is deliberate and manual, which is
        # the point — renaming the file by hand cannot be done by accident.
        current = current_node(plan.read_text(encoding="utf-8"))
        if current == "Merged" and args.node != "Merged":
            print(f"[dev-cycle] '{plan.name}' is at Merged — the work is delivered and its branch is "
                  f"gone, so this file is the only record of it.")
            print(f"[dev-cycle] Refusing to move it back to {args.node}: that rewrites the record, "
                  f"and the next `run` would recreate the deleted plan branch at today's HEAD.")
            print("[dev-cycle] Start new work with a new plan. To reopen this one anyway, rename the "
                  "file by hand — deliberately, because nothing else will.")
            return 1
