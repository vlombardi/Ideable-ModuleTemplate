...
# Usage: ./scripts/dev/module/run_tests.sh [module_name] [-h|--help]
...
if [[ "${IDEABLE_IN_TOOL_CONTAINER:-0}" != "1" && "${IDEABLE_NO_CONTAINER:-0}" != "1" ]]; then
  if [[ ! -x "${PROJECT_ROOT}/scripts/dev/common/tool.sh" ]]; then
    echo "scripts/dev/common/tool.sh is missing — it is how this project obtains its toolchain." >&2
    exit 1
  fi
  exec "${PROJECT_ROOT}/scripts/dev/common/tool.sh" bash "${SCRIPT_DIR}/run_tests.sh" "$@"
fi

# This is a QUICK, UNRECORDED run for one module. The official path — the only one that writes
# TEST_REPORTS/ — is `scripts/dev/common/run_enabled_tests.sh` (rules/testing-guidelines.md § "How tests
...
source "${PROJECT_ROOT}/scripts/dev/common/container_stack_env.sh"
