...
# Usage: ./scripts/dev/module/sync-template-updates.sh [OPTIONS]

set -euo pipefail

# THIS SCRIPT IS ONE OF THE FILES IT SYNCS, so it runs from a COPY.
#
# bash reads a script incrementally, by byte offset. Overwriting the running file means the shell
# resumes at that offset in different content — measured on a real adoption of `adopt_framework.sh`:
# the sync succeeded, then the caller landed mid-heredoc of its own replacement and reported
# `se: command not found`, `EOF: command not found`, and a syntax error on a stray `}`. Whether the
# same thing happens to THIS file has only ever been a question of where its write lands relative
# to how far bash has read, which is luck rather than design.
#
# So the first thing it does is copy itself to a temporary file and hand over to that. The copy is
# what runs; the file in the working tree is then just another file the sync may replace. The
# original location travels in the environment because `SCRIPT_DIR` is how this script finds the
# project — derived from a path in /tmp it would find nothing.
#
# `--file` used to carry a "Self-update requires special handling" note and then copy over the
# running file anyway. That note is gone: the handling is here, and it is not special.
SCRIPT_DIR="${IDEABLE_SYNC_SCRIPT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"

if [[ "${IDEABLE_SYNC_FROM_COPY:-0}" != "1" ]]; then
    _sync_copy="$(mktemp -t sync-template-updates.XXXXXX)" || {
        echo "ERROR: could not create the temporary copy this script runs from." >&2; exit 1; }
    trap 'rm -f "$_sync_copy"' EXIT
    cp "${BASH_SOURCE[0]}" "$_sync_copy"
    IDEABLE_SYNC_FROM_COPY=1 \
    IDEABLE_SYNC_SCRIPT_DIR="$SCRIPT_DIR" \
    IDEABLE_SYNC_INVOKED_AS="${BASH_SOURCE[0]}" \
        bash "$_sync_copy" "$@"
    exit $?
fi
...
TEMPLATE_URL="$IDEABLE_TEMPLATE_URL"
...
# Live env files this run OVERWROTE, so the report can say so in words at the end.
#
# Overwriting `modules/host_app/.env.config` is the intended behaviour — it is framework-owned and
# it is the only route a structural default has to an existing project (see `is_infrastructure`).
# But a deployer who edited that file has just had their values replaced, and `[updated]` in a list
# of two hundred lines does not tell anyone that. It reads as one more file being brought up to
# date, which is exactly what it is not.
#
# So the overwrite is announced twice: where it happens, and again at the end of the report where a
# person actually looks. The notice NAMES the file and names the remedy — `project.env.config`, which
# the deploy merges first and which therefore wins every key it declares — because a notice that
# reports a loss without saying how to avoid it just moves the surprise later.
LIVE_ENV_OVERWRITES=()

announce_live_env_overwrite() {
    local file="$1"
    is_live_env_file "$file" || return 0
    LIVE_ENV_OVERWRITES+=("$file")
    echo "             ^ framework-owned env file REPLACED — your own values belong in project.env.config"
}

print_live_env_overwrite_notice() {
    (( ${#LIVE_ENV_OVERWRITES[@]} )) || return 0
    local file
    echo ""
    echo "  ⚠ A framework-owned env file was REPLACED, not merged"
    echo "    ------------------------------------------------------------------"
    for file in "${LIVE_ENV_OVERWRITES[@]}"; do
        echo "      $file"
    done
    echo ""
    echo "    These files belong to the framework, and the sync overwrites them whole. That is"
    echo "    deliberate: it is the only way a default that MOVED — a service name, a port, the"
    echo "    database authentik connects to — can reach a project that already exists. A merge"
    echo "    would preserve the old value, which is the very thing that has to change."
    echo ""
    echo "    So if you had edited one of them, those edits are gone. Put them in"
    echo "    project.env.config instead: it is merged FIRST and the first writer of a key wins,"
    echo "    in the merged deployment_root/.env.config and in each per-module one alike. A value"
    echo "    declared there overrides the framework's default and survives every future sync."
    echo ""
    echo "    Check now whether anything you relied on has changed:"
    for file in "${LIVE_ENV_OVERWRITES[@]}"; do
        echo "      git diff -- $file"
    done
}
...
print_sync_report() {
    ...
    # Said last because it is the one line of this report a deployer may have to ACT on.
    print_live_env_overwrite_notice
...
usage() {
    cat << EOF
Usage: ${IDEABLE_SYNC_INVOKED_AS:-$0} [OPTIONS]
...
EXAMPLES:
    ${IDEABLE_SYNC_INVOKED_AS:-$0} --list-changes
    ${IDEABLE_SYNC_INVOKED_AS:-$0} --file scripts/dev/module/module-init.sh
    ${IDEABLE_SYNC_INVOKED_AS:-$0} --selective
    ${IDEABLE_SYNC_INVOKED_AS:-$0} --all
EOF
    exit 0
}
...
# WHICH template ref to fetch is the framework version's decision, not `main` unconditionally.
#
# This used to fetch `main` always, so a project that pinned `IDEABLE_FRAMEWORK_VERSION=1.6.0` pinned
# its TOOLBOX and got framework files from whatever the template held that morning. Pinning a
# framework version was not actually possible: the files moved underneath the pin.
#
#   latest  → main       the channel: newest framework files
#   X.Y.Z   → vX.Y.Z     the release the framework tagged when it published that version
#
# A pinned version whose tag does not exist is an ERROR, never a fall-through to `main`. Falling
# back would deliver files from a version the project did not ask for while reporting success,
# which is the whole defect in a different costume.
# ASKED OF `template-ref-name`, WHICH READS ONLY `framework.env`. It used to ask `template-ref`,
# which also reads the lock — so a project whose lock was missing or corrupt could not sync, and the
# sync is what delivers the lock. Worse, the resolver's diagnosis went to /dev/null and `set -e`
# aborted this script on the assignment, so the whole failure was a blank exit: the guarded
# `[[ -n "$_resolved" ]]` fallback written for exactly this case could never run. Measured on a
# project predating the lock: `adopt_framework.sh` reported only "the sync failed".
TEMPLATE_REF="main"
if [[ -f "$PROJECT_ROOT/framework.env" ]] && [[ -f "$PROJECT_ROOT/scripts/runtime/framework_version.py" ]]; then
    if _resolved="$(python3 "$PROJECT_ROOT/scripts/runtime/framework_version.py" template-ref-name 2>&1)"; then
        TEMPLATE_REF="$_resolved"
    else
        echo "ERROR: could not read the framework version from framework.env:" >&2
        printf '%s\n' "$_resolved" >&2
        echo "       The version decides which framework files this project gets, so the sync stops" >&2
        echo "       rather than guessing at 'main' and reporting success." >&2
        exit 1
    fi
fi

echo "Fetching template changes at ${TEMPLATE_REF}..."
if ! git fetch template "$TEMPLATE_REF" 2>/dev/null; then
    if [[ "$TEMPLATE_REF" == "main" ]]; then
        echo "ERROR: could not fetch 'main' from the template remote ($TEMPLATE_URL)." >&2
        exit 1
    fi
    echo "ERROR: the template repository has no '${TEMPLATE_REF}'." >&2
    echo "       framework.env asks for a framework version the framework never published, so there" >&2
    echo "       are no files to sync. There is deliberately no fall-back to 'main': that would" >&2
    echo "       deliver a different version's files and report success." >&2
    echo "       ACTION: set IDEABLE_FRAMEWORK_VERSION in framework.env to a published version" >&2
    echo "               (or to 'latest'), then run this again." >&2
    exit 1
fi
...
# Which `git diff --name-status` statuses mean "take the template's version of this path".
#
# `D` — in the template, absent from the working tree. `M` — present in both and different.
# `T` — present in both as DIFFERENT KINDS of entry, which is the status a symlink↔regular-file
# difference gets: `M` is never reported for one. That last letter was missing, and because the
# dispatch fell through to a silent `continue`, a project that adopted the `scripts/` layout move
# kept four stale root commands: the template's `redeploy.sh`, `start.sh`, `stop.sh` and
# `status.sh` are symlinks into `scripts/dev/common/`, theirs were regular files from the old
# layout, so `./redeploy.sh` still looked for `scripts/common/build_and_deploy.py` and died.
# `dev-cycle.sh` converted in that same sync only because it did not exist locally at all — absent
# is `D`, and `D` was handled.
#
# It is a function so a test can ask it, and it names no path: a file whose kind changes in the
# template is the general case, and enumerating today's instances leaves the next one to be found
# in a remote project again. The transfer itself already knew how to land a link
# (`sync_regular_file_from_template` asks `template_entry_is_symlink` before anything else).
status_wants_a_transfer() {
    [[ "$1" == "D" || "$1" == "M" || "$1" == "T" ]]
}
...
# What the framework OWNS, and therefore what a sync overwrites with the template's version rather
# than merging into what is already there.
#
# `modules/host_app/.env.config` is in this list, and it is the entry that is easy to read as a
# mistake — a `.env.config` is a deployer's file everywhere else in the tree. host_app's is not:
# `AGENTS.md` declares `modules/host_app/` framework-owned in its entirety, and the file holds
# STRUCTURAL defaults — which container authentik's database is, which port a service listens on —
# that change when the framework changes. Only the example shipped before, and an example is copied
# once at `module-init.sh`: a default that moved afterwards could reach an existing project by no
# route at all. `AUTHENTIK_POSTGRES_HOST` moved from `database` to `authentik-database` when
# authentik got its own Postgres, and every project created before that split still names the wrong
# host — invisible until a volume is recreated, when authentik loops on `database "authentik" does
# not exist`. The env merge is not that route either: it adds missing keys and preserves existing
# values, which is right for a deployer's port and wrong for a default that moved.
#
# Overwriting it loses nothing a deployer needs to keep, PROVIDED their values are in
# `project.env.config` — which the deploy merges first and which therefore wins every key it names
# (`build_and_deploy.py`, `merge_env_files` / `_write_env_split`: first writer wins, in the merged
# file and in the per-module one alike). That is the lever, and it is the same one a project used as
# a workaround while this file could not move.
is_infrastructure() {
    local file="$1"
    [[ "$file" == scripts/* ]] || \
    [[ "$file" == "AGENTS.md" ]] || \
...
# The `scripts/` layout move, landed as a WHOLE — never half.
#
# `scripts/` answers when a script runs (`dev/` or `runtime/`) and then who runs it (`common/`,
# `module/`, `master/`). Projects created before that move hold the old folders, and the per-file
# loop below would eventually empty them: every old path is absent from the template, so it takes
# the `A` branch and is removed one file at a time.
#
# That is not enough, and the reason is the hazard this function exists for. A project mid-sync
# would hold BOTH layouts — the emptied old directories still there, and the root commands, which
# are themselves synced, resolving through one of them. `./redeploy.sh` in the old layout is a real
# file that runs `scripts/common/build_and_deploy.py`; in the new one it is a symlink into
# `scripts/dev/common/`. A tree with both is a tree where the answer to "which deploy runs?"
# depends on which file the shell reached first, and nothing in it looks wrong.
#
# So the old locations go in the same run that creates the new ones, before any file is compared.
migrate_legacy_scripts_layout() {
    local legacy removed_any=0

    # Folders that became folders somewhere else. `runtime/config` is here as well as the three
    # role folders: `runtime/` was flattened (it IS the deployed `scripts/` folder, copied whole),
    # so a project could be on either of two earlier shapes.
    for legacy in scripts/common scripts/master_only scripts/module_only scripts/runtime/config; do
        if [[ -d "${PROJECT_ROOT}/${legacy}" ]]; then
            rm -rf "${PROJECT_ROOT}/${legacy}"
            echo "  [migrated]  ${legacy}/ removed — its scripts now live under the folder that says when they run"
            record_sync_result "removed" "${legacy}/"
            removed_any=1
        fi
    done

    # Files that used to sit at the top of `scripts/` or directly inside `scripts/dev/`. Nothing
    # sits at either place now — `scripts/` holds only `README.md`, and `dev/` holds only the three
    # role folders — so this is expressed as "any regular file there" rather than as a list of
    # basenames. Computed, not enumerated: it covers every file that was ever there, and it does
    # not go stale.
    for legacy in "${PROJECT_ROOT}/scripts"/*.sh "${PROJECT_ROOT}/scripts"/*.py "${PROJECT_ROOT}/scripts/dev"/*; do
        [[ -f "$legacy" ]] || continue
        rm -f "$legacy"
        echo "  [migrated]  ${legacy#"${PROJECT_ROOT}/"} removed — it moved into a folder that answers who runs it"
        record_sync_result "removed" "${legacy#"${PROJECT_ROOT}/"}"
        removed_any=1
    done

    # `scripts/specs` → `scripts/SPECS` is a CASE-ONLY rename, and it must not be done by removing
    # the old name: on a case-insensitive filesystem — every default macOS install — `scripts/specs`
    # and `scripts/SPECS` are the SAME directory, so `rm -rf scripts/specs` deletes the new one.
    # The framework itself was caught by this: a `git mv` recorded the new name in the index while
    # the on-disk directory kept the old one, and every `[[ -d ]]` check reported success.
    #
    # So the on-disk name is read from the directory LISTING, and the rename goes through a
    # temporary name, which is the only sequence that works on both kinds of filesystem.
    if [[ -d "${PROJECT_ROOT}/scripts" ]]; then
        local on_disk
        on_disk="$(ls "${PROJECT_ROOT}/scripts" 2>/dev/null | grep -x 'specs' || true)"
        if [[ -n "$on_disk" ]] && ! ls "${PROJECT_ROOT}/scripts" 2>/dev/null | grep -qx 'SPECS'; then
            mv "${PROJECT_ROOT}/scripts/specs" "${PROJECT_ROOT}/scripts/.specs.renaming.$$"
            mv "${PROJECT_ROOT}/scripts/.specs.renaming.$$" "${PROJECT_ROOT}/scripts/SPECS"
            echo "  [migrated]  scripts/specs/ → scripts/SPECS/ (case-only rename, done through a temporary name)"
            record_sync_result "removed" "scripts/specs/"
            removed_any=1
        fi
    fi

    if [[ "$removed_any" -eq 1 ]]; then
        echo "  [migrated]  the scripts/ layout move landed as a whole — no project is left holding both layouts"
    fi
}
...
# A LIVE env file — the `.env.config` / `.env.secrets` a stack actually reads — as opposed to the
# `.example` beside it. The distinction did not need a name while no live env file was force-synced;
# host_app's `.env.config` is (see `is_infrastructure`), and force-syncing one puts it in reach of
# two code paths that are correct for a doc or a script and destructive for a configuration:
#
#   * the removal branches below, which delete an infrastructure path the template does not hold —
#     so a project syncing against an older template ref would have its configuration DELETED, not
#     merely reverted;
#   * `should_create_missing_infrastructure_file`, which would write an EMPTY one in its place.
#
# Both are refused for these files. Reverting a deployer's configuration to the framework's default
# is the intended consequence of framework ownership; removing or blanking it is not, and neither
# failure announces itself — the stack comes up wrong rather than not coming up.
is_live_env_file() {
    local file="$1"
    [[ "$file" == *.env.config || "$file" == *.env.secrets ]]
}
...
should_create_missing_infrastructure_file() {
    ...
    # An env file is never created as an EMPTY placeholder, however infrastructure it is — see
    # `is_live_env_file`. A push that failed to carry host_app's `.env.config` would otherwise blank
    # the deployer's copy on the next sync.
    if is_live_env_file "$file"; then
        return 1
    fi
...
sync_regular_file_from_template() {
    ...
    if [[ -f "$local_path" ]]; then
        ...
        record_sync_result "updated" "$local_path"
        announce_live_env_overwrite "$local_path"
    else
        mkdir -p "$(dirname "$local_path")"
        cp "$template_tmp" "$local_path"
        chmod +x "$local_path" 2>/dev/null || true
...
if ! $SELECTIVE_MODE && [[ -z "$FILES_TO_SYNC" ]]; then
    ...
    while IFS= read -r line; do
        ...
        if [[ "$status" == R* || "$status" == C* ]]; then
            ...
            if is_infrastructure "$local_path" && ! is_template_module_file "$local_path" && ! is_branding_file "$local_path" && ! is_hostapp_config_file "$local_path" && ! is_live_env_file "$local_path"; then
            ...
            if is_infrastructure "$file" && ! is_template_module_file "$file" && ! is_branding_file "$file" && ! is_hostapp_config_file "$file" && ! is_env_example_file "$file" && ! is_live_env_file "$file"; then
            ...
            continue
        elif status_wants_a_transfer "$status"; then
            # In the template and not in the working tree, different there, or a different KIND of
            # entry there — see status_wants_a_transfer for what each letter means and which one
            # was missing.
            file="$rest"
    ...
    SHARED_TESTS=(
        ...
        # Its companion, and it covers the two steps that one assumes: that the token a module
        # sends is the CURRENT session's (the key is built exactly as oidc-client-ts wrote it,
        # never guessed from whatever `oidc.user:` entry storage yields first), and that a failed
        # /me is not rendered as a denial. Both are framework-owned: every remote reads the
        # host's session store through the same two slug-free service files, so a remote holding
        # a fork of this check would keep a defect the framework had already fixed.
        "frontend/TESTS/test_oidc_token_source_contract.py"
        ...
        # The E2E suites must be able to clean up after themselves. Force-synced because the
        # leak it catches is a framework one: the harness specs it scans are force-synced, and
        # it reads the cap from the module's OWN backend rather than restating a number.
        "frontend/TESTS/test_e2e_cleanup_pages_within_the_api_cap.py"
