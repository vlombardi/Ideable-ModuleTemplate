...
DEFAULT_TEMPLATE_REMOTE_URL="$IDEABLE_TEMPLATE_URL"
ORIGINAL_ORIGIN_URL=""
...
# A new project starts on the `latest` channel, and owns that choice from here on.
#
# `framework.env` holds the ONE line that decides which framework this project runs. Sync never
# writes it — a sync that reset it would take away the only control a module maintainer has over
# their framework version — so a generated project has to start with one, and this is where.
FRAMEWORK_ENV="${PROJECT_ROOT}/framework.env"
if [[ ! -f "$FRAMEWORK_ENV" ]]; then
    cat > "$FRAMEWORK_ENV" <<'FWENV'
# The framework version this project runs. ONE line, and it is yours to change.
#
#   latest   follow the newest published framework (the channel)
#   X.Y.Z    pin a published release
#
# Everything else follows from it: the dev tools image, the host_app images, and which framework
# files `scripts/dev/module/sync-template-updates.sh` fetches. The digests published for this
# version are recorded in `framework.lock.json`, which IS force-synced — that file is generated by
# the framework and read by `scripts/runtime/framework_version.py`.
#
# After changing this line, run: scripts/dev/module/sync-template-updates.sh
IDEABLE_FRAMEWORK_VERSION=latest
FWENV
    echo "Created framework.env (IDEABLE_FRAMEWORK_VERSION=latest)"
fi
...
# Update all source files AND the module's own specs for name/slug replacements.
#
# `*.md` is included, and the exclusion below is why it can be. The two kinds of spec need
# OPPOSITE treatment:
#
#   - MODULE-OWNED specs (SPECS/module-specs.md, SPECS/general_bug_avoider.md, dependencies.md)
#     are copied once, here, and never force-synced again. They describe THIS module, so they must
#     be substituted. They were not, on the assumption recorded in the old comment that "SPECS are
#     generic" — so a project called acme_assets was told by its own backend spec that its entity
#     was `template_items`, its permission `template.audit_trail:view`, and its database variables
#     `TEMPLATE_ENTITIES_DB_*`. Followed literally, that builds the wrong module.
#
#   - FRAMEWORK specs (SPECS/ideable-framework-specs/**) are force-synced verbatim on every
#     template sync, so a substitution here would be silently undone the first time the maintainer
#     syncs. They are therefore EXCLUDED, and are written slug-agnostically instead — placeholders
#     like `<slug>.<entity>:view` and `<SLUG>_ENTITIES_DB_*` rather than template values.
#
# Getting this backwards in either direction produces churn: a renamed file that reverts, or a
# framework contract rewritten per module.
find "$MODULE_DIR" -type f \( -name "*.py" -o -name "*.tsx" -o -name "*.ts" -o -name "*.js" -o -name "*.css" -o -name "*.json" -o -name "*.yml" -o -name "*.yaml" -o -name "*.sh" -o -name "*.md" \) \
     -not -path "*/ideable-framework-specs/*" | while read -r file; do
    # Skip binary files
...
# Parity guard: the exposed moduleManifest must import the module stylesheet so host_app
# loads the module's CSS on mount (see module-integration-specs.md §2 CSS loading contract).
# A re-runnable check (no in-place edit), safe on existing modules.
MANIFEST_TS="${MODULE_DIR}/frontend/SOURCES/src/moduleManifest.ts"
if [[ -f "$MANIFEST_TS" ]] && ! grep -qE "import[[:space:]]+['\"]\./index\.css['\"]" "$MANIFEST_TS"; then
    echo ""
    echo "Warning: ${MANIFEST_TS#${PROJECT_ROOT}/} does not import the module stylesheet."
    echo "         Add \`import './index.css'\` at the top of moduleManifest.ts, or host_app"
    echo "         will render this module unstyled (module-integration-specs.md §2)."
fi
