...
# Usage: ./scripts/runtime/restore.sh --from <backup-dir> [--yes] [--only <label>]
#
...
# The build this stack runs: the revision label the deploy stamps on every image it builds, read
# off the running containers (the value most of them carry, when replicas disagree). Empty when no
# container carries one — a stack of consumed images has digests, not this checkout's revision.
running_revision() {
  docker ps --filter "label=com.docker.compose.project=${APP_SLUG:-}" --format '{{.ID}}' 2>/dev/null \
    | while read -r cid; do
        docker inspect --format '{{index .Config.Labels "org.opencontainers.image.revision"}}' "$cid" 2>/dev/null
      done | grep -v '^$' | sort | uniq -c | sort -rn | awk 'NR==1{print $2}'
}
...
if [[ ! -f "$PROV" ]]; then
  ...
  B_REV="$(prov_get revision)"
  R_REV="$(running_revision)"
  log "backup was produced by revision=${B_REV:-unknown}; this deployment runs ${R_REV:-unknown}"
  if [[ -n "$B_REV" && -n "$R_REV" && "$B_REV" != "$R_REV" ]]; then
