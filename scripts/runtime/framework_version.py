...
a second parser is how two readers stop agreeing.
    ...
    python3 scripts/runtime/framework_version.py template-ref-name      # <ref>, needs no lock
...
def _root(root: Path | str | None) -> Path:
    ...
    An argument is COERCED, never trusted to be a `Path` already. Every entry point here funnels
    through this function and then does path arithmetic on what it returns, so a `str` arriving
    intact fails several frames later on `root / ENV_FILE` with a `TypeError` naming neither the
    caller nor the file being resolved. `build_and_deploy.py` passed `PROJECT_ROOT` — an `os.path`
    string — and killed every deploy of a project that consumes host_app; the public signatures
    still ask for a `Path`, so a type checker keeps reporting such a caller, and this line keeps
    the runtime from turning it into a dead deploy.
    """
    if root is not None:
        return Path(root)
...
def deployed_release(root: Path | str) -> tuple[str, dict]:
    ...
    tree, and inferring it wrongly is the defect this function was written for. It is still
    coerced: this is one of the two entry points that does not funnel through `_root()`, because
    passing `None` here must stay an error rather than resolve to the checkout, and a `str` would
    otherwise fail on the line below with a `TypeError` about `/`.
    """
    root = Path(root)
...
def deployed_hostapp_image_refs(root: Path | str) -> dict[str, ImageRef]:
...
def template_ref_name(root: Path | None = None) -> str:
    """Which ref the framework files come from: `main` on `latest`, the version itself when pinned.

    READS `framework.env` AND NOTHING ELSE, which is why it exists apart from `template_ref` below.
    The sync and `adopt_framework.sh` both need this answer at a moment when the lock may be
    missing, truncated, or published for another version — adoption is the step that repairs
    exactly that — and the ref never depended on the lock in the first place. Keeping the rule here
    rather than letting each caller write `main`-or-the-version is what stops the two drifting.

    Before this, the sync asked `template-ref`, which reads the lock: a project whose lock was
    absent or corrupt could not sync, and the sync is what delivers the lock. The resolver's own
    error said to run the sync.
    """
    wanted = version(_root(root))
    return wanted if is_pinned(wanted) else "main"


def template_ref(root: Path | None = None) -> TemplateRef:
    """Where sync fetches the framework files: the repository and the ref.
    ...
    root = _root(root)
    lock = read_lock(root)
    return TemplateRef(repo=lock["template"]["repo"], ref=template_ref_name(root))
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument("command", choices=[
        ...
        "deployed-hostapp-image-refs", "template-ref", "template-ref-name", "check",
    ])
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        elif args.command == "template-ref-name":
            print(template_ref_name(root))
