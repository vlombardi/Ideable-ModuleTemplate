...
# Detect context: source repo (script in scripts/runtime/) vs deployed (script in scripts/)
...
# Minimum length requirements for specific secrets. Authentik's Django SECRET_KEY
# must be at least 50 characters or the server refuses to start.
MIN_LENGTH_RULES: dict[str, int] = {
    "HOSTAPP_AUTHENTIK_SECRET_KEY": 50,
...
def collect_targets() -> list[Path]:
    ...
    for module_name in read_enabled_modules():
        ...
        example_path = module_secrets.parent / f"{module_secrets.name}.example"
        if not module_secrets.exists() and example_path.exists():
            module_secrets.write_text(example_path.read_text(encoding="utf-8"), encoding="utf-8")
            print(f"Created {module_secrets.relative_to(PROJECT_ROOT)} from {example_path.relative_to(PROJECT_ROOT)}")
...
def validate_secret_length(key: str, value: str) -> tuple[bool, str]:
    min_len = MIN_LENGTH_RULES.get(key)
    if min_len and len(value) < min_len:
        return False, f"{key} must be at least {min_len} characters (got {len(value)})."
    return True, ""
...
def prompt_for_values(occurrences):
    ...
    for key, data in occurrences.items():
        ...
        min_len = MIN_LENGTH_RULES.get(key)
        if min_len and len(current) < min_len:
            print(f"  WARNING: current value is too short (must be ≥{min_len} chars).")

        while True:
            new_value = prompt_input(f"  new value [{current}]: ").strip()
            chosen = current if new_value == "" else new_value
            valid, error = validate_secret_length(key, chosen)
            if valid:
                updates[key] = chosen
                break
            print(f"  ERROR: {error}")
...
def ensure_root_secrets_complete(root_secrets: Path, occurrences: OrderedDict) -> None:
    """If a secret key only exists in a module file, make sure it is also in the
    root .env.secrets so the user prompt updates the merged root file too."""
    if not root_secrets.exists() or not occurrences:
        return
    existing_keys = set()
    for raw_line in root_secrets.read_text(encoding="utf-8").splitlines():
    ...
    for key, data in occurrences.items():
        if key not in existing_keys and is_secret_key(key):
            missing.append(f"{key}={data['current']}")

    if missing:
        with open(root_secrets, "a", encoding="utf-8") as f:
            content = root_secrets.read_text(encoding="utf-8")
            if content and not content.endswith("\n"):
                f.write("\n")
            f.write("\n".join(missing) + "\n")
        print(f"Added {len(missing)} missing secret(s) to {root_secrets.relative_to(PROJECT_ROOT)}")
...
def main() -> int:
    ...
    root_secrets = PROJECT_ROOT / PROJECT_ENV_NAME
    root_secrets_example = PROJECT_ROOT / (PROJECT_ENV_NAME + ".example")
    if not root_secrets.exists() and root_secrets_example.exists():
        root_secrets.write_text(root_secrets_example.read_text(encoding="utf-8"), encoding="utf-8")
        print(f"Created {root_secrets.relative_to(PROJECT_ROOT)} from {root_secrets_example.relative_to(PROJECT_ROOT)}")

    targets = collect_targets()
    occurrences = collect_secret_occurrences(targets)
    ensure_root_secrets_complete(PROJECT_ROOT / PROJECT_ENV_NAME, occurrences)
    # Re-collect after augmenting root, so occurrences reflect the now-complete root file.
    occurrences = collect_secret_occurrences(targets)
