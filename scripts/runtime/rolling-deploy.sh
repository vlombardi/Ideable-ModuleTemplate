...
# The deployment root, resolved the way every other runtime script resolves it: beside the script
# when this is the deployed copy (`deployment_root/scripts/`), the project's `deployment_root/` when
# it is the source copy (`scripts/runtime/`). It used to be a bare `../../..`, which from the old
# three-deep `runtime/config/` location was the project root — a directory with no `.env.config` and
# no `docker-compose.yml`, so the default only ever worked when `--dir` overrode it.
if [[ -f "${SCRIPT_DIR}/../.env.config" ]]; then
  DEPLOY_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
else
  DEPLOY_DIR="$(cd "${SCRIPT_DIR}/../../deployment_root" && pwd)"
fi
PROJECT=""
READY_TIMEOUT=120
