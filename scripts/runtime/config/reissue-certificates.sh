...
Deletes modules/HostApp/traefik/acme.json (after confirmation) and recreates
...
for _ in $(seq 1 6); do
  if [[ -f "$CURRENT_DIR/modules/HostApp/traefik/acme.json" ]]; then
...
if [[ -z "$PROJECT_ROOT" ]]; then
  echo "ERROR: Could not locate modules/HostApp/traefik/acme.json relative to $SCRIPT_DIR" >&2
  exit 1
fi

ACME_PATH="${PROJECT_ROOT}/modules/HostApp/traefik/acme.json"
if [[ ! -e "$ACME_PATH" ]]; then
