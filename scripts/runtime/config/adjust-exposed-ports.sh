#!/bin/bash
#
# adjust-exposed-ports.sh
#
# Scans the exposed ports declared in project.env.config, checks which are already
# in use on the host, reports conflicts, and interactively reassigns them.
# Updates project.env.config in place and also updates the corresponding fallback
# port values in module-side .env.config files. Prints a final recap.
#
# Usage:
#   ./scripts/runtime/adjust-exposed-ports.sh [-h|--help]
#
...
for f in "$F_COMPOSE_VAR" "$F_MODULE_VAR" "$F_PROJECT_VAR" "$F_SERVICES" "$F_MODULES" "$F_DEFAULTS" "$F_COMPOSE_VAR_FILE"; do
    touch "$f"
done
...
get_env_value() {
    local key="$1" file="$2"
    if [[ -f "$file" ]]; then
        grep -E "^${key}=" "$file" 2>/dev/null | tail -1 | sed "s/^${key}=//" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//'
    fi
}
...
if [[ -n "$ENABLED_MD" && -f "$ENABLED_MD" ]]; then
    while IFS= read -r line; do
        ...
        [[ -z "$line" ]] && continue
        if [[ "$line" =~ ^([A-Za-z0-9_-]+):[[:space:]]*enabled ]]; then
