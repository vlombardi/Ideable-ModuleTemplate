...
#
# Options:
#   [TAG]         Image tag to pull (default: consumedImageTag from modules/host_app/module.json)
...
Arguments:
  TAG                 Image tag to pull. Default: the 'consumedImageTag' declared in
                      modules/host_app/module.json — the release this project runs.
                      There is no fallback to "latest".
EOF
}

# Left empty here and resolved from modules/host_app/module.json below, once PROJECT_ROOT is known.
#
# This used to default to `latest`, and that default was one half of a contradiction the framework
# had with itself: this script pulled `latest` while the deployed compose addressed the same images
# by `${IMAGE_TAG}` — the consuming project's own commit hash. One image, two names, and a remote
# project could satisfy neither. `consumedImageTag` in module.json is now the single answer to
# "which host_app release does this project run", and every tool reads it from there.
TAG=""
REGISTRY_OVERRIDE=""
POSITIONAL_TAG=""
...
MODULE_JSON="$PROJECT_ROOT/modules/host_app/module.json"
if [[ -z "$TAG" ]]; then
  if [[ ! -f "$MODULE_JSON" ]]; then
    echo "ERROR: $MODULE_JSON not found, and no tag was given." >&2
    echo "       Pass one explicitly: $0 --tag <release>" >&2
    exit 1
  fi
  TAG=$(python3 -c 'import json,sys; print((json.load(open(sys.argv[1])).get("consumedImageTag") or "").strip())' "$MODULE_JSON")
  if [[ -z "$TAG" ]]; then
    echo "ERROR: modules/host_app/module.json declares no 'consumedImageTag', and no tag was given." >&2
    echo "       There is deliberately no fallback to 'latest': a moving tag makes 'which build is" >&2
    echo "       running?' unanswerable and lets a pull replace a working image." >&2
    echo "       ACTION: declare it in module.json, or pass one: $0 --tag <release>" >&2
    exit 1
  fi
fi
