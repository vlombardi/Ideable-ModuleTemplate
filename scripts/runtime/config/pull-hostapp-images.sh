#!/usr/bin/env bash
# Check and pull HostApp images referenced in modules/HostApp/docker-compose.yml.
...
Options:
  --registry <value>  Override registry prefix (default: value from modules/HostApp/.env.config)
...
for _ in $(seq 1 6); do
  if [[ -d "$CURRENT_DIR/modules/HostApp" ]]; then
...
if [[ -z "$PROJECT_ROOT" ]]; then
  echo "ERROR: Could not locate modules/HostApp relative to $SCRIPT_DIR" >&2
  exit 1
fi

ENV_FILE="$PROJECT_ROOT/modules/HostApp/.env.config"
if [[ ! -f "$ENV_FILE" ]]; then
...
printf 'Checking HostApp images with tag "%s"...\n\n' "$TAG"
