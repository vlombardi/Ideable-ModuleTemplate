...
is_env_file() {
    local file="$1"
    [[ "$file" == modules/*/.env.config ]] || [[ "$file" == modules/*/.env.config.example ]] || [[ "$file" == modules/*/.env.secrets ]] || [[ "$file" == modules/*/.env.secrets.example ]] || [[ "$file" == "project.env.config.example" ]] || [[ "$file" == "project.env.secrets.example" ]]
...
if ! $SELECTIVE_MODE && [[ -z "$FILES_TO_SYNC" ]]; then
    ...
    while IFS= read -r line; do
        ...
        if [[ "$status" == R* || "$status" == C* ]]; then
            ...
            # File exists in the working tree but not in the template — remove locally if managed
            file="$rest"
