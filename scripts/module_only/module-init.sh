...
# Update all source files for name/slug replacements
find "$MODULE_DIR" -type f \( -name "*.py" -o -name "*.tsx" -o -name "*.ts" -o -name "*.json" -o -name "*.yml" -o -name "*.yaml" -o -name "*.md" -o -name "*.sh" \) | while read -r file; do
    # Skip binary files
...
if [[ -f "$ENV_EXAMPLE" ]]; then
    ...
    # Update VITE_* variables and APP_NAME/APP_SLUG in .env
    ...
    rm -f "${ENV_FILE}.bak"
    echo "Updated .env with ${UPPER_SLUG}_* variables and APP_NAME=${NEW_NAME}"
fi
...
if [[ -f "$HOSTAPP_ENV_EXAMPLE" && ! -f "$HOSTAPP_ENV" ]]; then
...
echo ""
echo "Next steps:"
echo "1. Edit modules/HostApp/.env — point to your running HostApp instance"
