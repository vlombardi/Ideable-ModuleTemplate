...
#   --include-all       Pull the whole repo content (modules/HostApp, scripts/,
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      echo "  --include-all           Pull entire repo (HostApp, scripts, root files)"
...
if [[ -n "$INCLUDE_ALL" ]]; then
  ...
  mkdir -p "${DEPLOYMENT_ROOT}/modules"
