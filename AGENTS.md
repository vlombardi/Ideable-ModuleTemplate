...
- `SPECS/dependencies.md` is the single source of truth for **component** versions (libraries, frameworks, images) — update it when any dependency changes.
- The **framework** version is a different question with a different answer: one line, `IDEABLE_FRAMEWORK_VERSION` in `framework.env`, with the digests it resolved to in the generated `framework.lock.json`. Never state a framework version anywhere else — see `rules/general-guidelines.md` § *One version names the framework*.
...
- `redeploy.sh`, `start.sh`, `stop.sh`, `status.sh`, `dev-cycle.sh`, `update_backend.sh`, `update_frontend.sh` (all relative symlinks into `scripts/dev/common/`)
- Any file under `scripts/` (including `scripts/dev/common/build_and_deploy.py`)
...
- `modules/module_template/database/SPECS/ideable-framework-specs/schema-workflow.md` — **read before changing any table**: the model is the schema and only Alembic writes it; the design → model → migration → verify phases (`scripts/dev/common/schema.sh`); baselines, squashing, and expand → migrate → contract.

**Implementation plan (status tracking across the workflow skills):**
- `rules/implementation-plan.md` — canonical format, status-symbol legend, naming and location (`implementation-plans/`), active-plan resolution, the kanban card's move from `todo/` to `doing/` when a plan is created, and the **Purpose** + **Overall view** chapters (Created at / Last updated / Current step + the canonical dev-cycle Mermaid graph with the current node highlighted yellow), and the **mandatory decisions record** (§ *Decisions and answers* — one chapter before chapter 1, one table, `When | Node | Sub-set | Question | Decision | Who | Why`, one row wherever the plan deferred a choice, an open question included; `Who` is `maintainer` · `agent` · `router`, and a `Node` cell marked ⛔ came from an `Asking human direction` block). The plan artifact is created and kept current by `ideable-implement-specs`, `ideable-test-and-fix`, `ideable-build-and-deploy`, `ideable-align-docs`, `ideable-bugfixing-and-changes`, and `ideable-commit-changes`. Load when creating or updating an implementation plan.

**Dev-cycle orchestration (nodes = dev states, arcs = skills):**
- `scripts/dev/common/dev-cycle.sh` — the thin deterministic router over the skill graph, and **it describes itself**: run `./scripts/dev/common/dev-cycle.sh --help` for the actions (`status`, `start`, `run`, `pause`, `resume`, `deliver`), the flags, the environment variables and what the router does at each node. Nothing is restated here, deliberately — a second copy of a command's behaviour is a copy that goes stale, and this file is the index, not the manual. Three things a reader needs before they get there:
  - **`run` is the only advance.** One command writes everything a transition must write — the graph, `Current step`, the executing sub-set's label, the sub-set advance, the `Commit` cells and the scope gate — so a plan can never record its state somewhere `run` does not look. No skill, no agent and no other command writes a plan's node.
  - **Who calls `run` makes no difference**: the router performs an LLM node for an agent caller exactly as for a human one. So an agent that reaches a node runs the router; it never performs the node and then moves the plan itself. What is refused is a *second worker on the same plan*, by a lock the router holds for the duration.
  - **Changes you did not make are still yours.** Two workers on one plan edit the tree concurrently and each then sees the other's diffs with no idea where they came from — so if unexplained changes appear in `git status` after you ran this router, they are the consequence of *your* command. Never report them as an outside event.

  The plan format, the canonical graph, the `Asking human direction` contract and how a run's results are folded back are `rules/implementation-plan.md`; the catalogue entry is `scripts/README.md`; landing a plan is `rules/version-control.md` § *Delivering a plan*.
- `ideable-align-docs` skill — drives the **`Documenting`** node, between a green `Testing` and `Committing`: the specs and docs governing the sub-set's diff are brought into line with what is now true (present tense only, nothing removed still named as live), it records a `Docs` cell per thing, and it ends on a green docs gate. It **reconciles, it does not legislate** — a change that would require deciding something the plan never decided goes to `Asking human direction`, and framework-owned files in a remote module project are reported, never edited. What it never does is write code or advance the plan: a sub-set found holding unwritten work goes back to `Implementing`, and the router is what takes it there. See `rules/implementation-plan.md` § *Documenting* for the node's routes and the graph that draws them.
...
- `scripts/dev/common/spec_workset.py` — incremental work-set for `ideable-implement-specs` (reference graph rebuilt each run, git as change oracle, `changed ∪ transitive-dependents`, `--force` full run). A cache hint only — `Done ⇐ contract tests pass`.
...
- `rules/general-guidelines.md` § *One version names the framework* — the framework version contract: `framework.env` (the one line a person edits), `framework.lock.json` (generated, force-synced), and `scripts/runtime/framework_version.py` as their only reader. A local build is `<slug>.<submodule>:latest` and carries its commit as an OCI label; a production site consumes published versions and never builds.
- `modules/<MODULE>/SPECS/dependencies.md` — pinned component versions for all sub-modules.
- `modules/module_template/SPECS/ideable-framework-specs/infrastructure-file-list.md` — canonical file inventory.

**Adding, moving or invoking a script:**
- `rules/general-guidelines.md` § *Project structure* (the `scripts/` entry) — where a script goes is answered by the folder, not by a list: `scripts/dev/` is development time, split by who uses it (`common/`, `module/`, `master/`); `scripts/runtime/` **is** the deployed `scripts/` folder; `scripts/SPECS/` and `scripts/TESTS/` are the specs and the suite; nothing sits at the top but `README.md`. Read it for the three rules that keep that true — `runtime/` is copied whole, `runtime/` never names a path under `scripts/dev/`, and everything ships except `dev/master/` and `TESTS/` — and for the convention that a root command is written bare (`./redeploy.sh`) while a deployed script is written with its path.
- `scripts/README.md` — the catalogue, grouped by those folders and naming every script, with the ones nobody invokes marked **manual-only**. Add an entry in the same change as the script; `scripts/TESTS/test_script_catalogue_is_complete.py` fails otherwise.
