...
2. Each module's `module.json` (`provides` / `dependsOn`) — the machine-readable inter-module contract that `build_and_deploy.py` resolves providers-first (via `scripts/common/module_deps.py`) to order the build; and `modules/<MODULE>/SPECS/dependencies.md` for pinned library/image versions. Canonical contract: `modules/module_template/SPECS/ideable-framework-specs/module-integration-specs.md` §5.1.
