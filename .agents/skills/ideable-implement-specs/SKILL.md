...
1. Create the plan file in `implementation-plans/` using the naming convention in that rule
   ...
   dev-cycle node — `Implementing` at creation). The filename's timestamp tracks the **latest
   execution**: `scripts/dev-cycle.sh` re-stamps it at every transition, while the plan's
   creation time stays in the Overall view's `Created at`. Then, per that rule's
