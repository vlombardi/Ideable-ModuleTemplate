...
1. Create the plan file in `implementation-plans/` using the naming convention in that rule
   (`<date> - <time> - <description> (<state>).md`, timestamp = now, state = the plan's current
   dev-cycle node — `NotStarted` at creation). The filename's timestamp tracks the **latest
   execution**: `scripts/dev/common/dev-cycle.sh` re-stamps it at every transition, while the plan's
   creation time stays in the Overall view's `Created at`. Do **not** create the plan branch by
   hand: per that rule's **Git integration** the `Branching` node does it. Once the plan file is
   complete (steps 2–3), make it the active plan and then drive it — two commands, in this order:
   ```bash
   ./scripts/dev/common/dev-cycle.sh start <description>       # creates its `ACTIVE PLAN` link
   ./scripts/dev/common/dev-cycle.sh run --auto-advance 2      # Branching, then Implementing
   ```
   `start` is what makes the plan the active one (`rules/implementation-plan.md` § *Active plan
   resolution*): a plan file with no link does not resolve, so `run` on its own would answer
   *"No active implementation plan"* and name this plan as one waiting to be started. `run` then
   advances `NotStarted → Branching` (nothing to do), performs `Branching` (creates and checks out
   `plan/<description>` from the current branch, starts sub-set 1) and leaves the plan at
   `Implementing`, which is where this skill's own work begins.
2. Write the **Purpose** chapter (a few sentences on what this run implements) and the
   ...
   **Current step** = `Not started (./scripts/dev/common/dev-cycle.sh start … then run …)`, every sub-set row `—`, and
   the canonical dev-cycle Mermaid graph with the highlight on the `NotStarted` node (the two
   `class` lines set so `NotStarted` is the only `current` node).
3. Fill the **Main implementation summary table** with the things to implement, derived from
   the specs read in Step 2 and the audit in Step 3 — one row per thing. The columns are the ones
   `rules/implementation-plan.md` §3 defines, and all of them are required:
   `Impl | BE test | FE test | Cfg test | Fw test | Docs`. Start `Impl` and `Docs` at 🔲, and set
   each test column to 🔲 or ➖ (`N/A`) depending on whether the thing has a part that kind of
   suite can exercise. **`Docs` is not optional**: it is the artifact the `Documenting` node
   fills, and a plan created without the column leaves that step with nothing to record and its
   gate with nothing to read.
4. Write the **Status summary**, the **Detailed summary** (with sub-task tables where a thing
   ...
   Then record every **decision this node took that the plan did not already contain** as a row in
   the plan's **`Decisions and answers`** chapter — `When | Node | Sub-set | Question | Decision |
   Who | Why`, `Who` being `agent` for a decision you took — per `rules/implementation-plan.md`
   § *Decisions and answers*, which is mandatory for every skill that acts. It is the plan's one
   decisions record and it sits **before chapter 1**, not inside § *Detailed summary*. A question
   put to the maintainer counts even when nobody answered it: an open question is recorded, not
   omitted. An agent's memory is never something the next node may rely on — the router resumes the
   session across the nodes of one sub-set, but a session can be gone and the sub-set after this one
   starts a fresh agent on purpose — so what is not written here is decided again, differently, by
   the next one.
...
router does not look. The `start` + `run --auto-advance 2` above is that same command doing the
first two transitions; past them, highlight **your own** node and stop there.
