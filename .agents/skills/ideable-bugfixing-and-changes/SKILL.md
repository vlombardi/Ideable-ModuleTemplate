...
- Resolve the active plan (`rules/implementation-plan.md` § *Active plan resolution* — the plan
  named by an `ACTIVE PLAN - <description>.md` link in `implementation-plans/`, its current file
  being the newest one bearing that description; an unlinked or parked plan is not active).
...
**This step performs the `Fixing` node; it never advances the plan.**
...
router does not look. Highlight **your own** node — that is the Overall view staying true about
where the plan is — and stop there.
