...
The canonical implementation of this workflow is the script `scripts/dev/common/build_and_deploy.py`. **Always run that script** rather than performing manual build/copy steps, unless the script does not yet support the module being built (see Step 4).

## Prerequisites

Before starting, verify:
1. `modules/enabled.md` — identify which modules are enabled. Only build enabled modules.
2. Each module's `module.json` (`provides` / `dependsOn`) — the machine-readable inter-module contract that `build_and_deploy.py` resolves providers-first (via `scripts/dev/common/module_deps.py`) to order the build; and `modules/<MODULE>/SPECS/dependencies.md` for pinned library/image versions. Canonical contract: `modules/module_template/SPECS/ideable-framework-specs/module-integration-specs.md` §5.1.
3. `rules/general-guidelines.md` — re-read the mandatory project rules before any action.
4. All `SOURCES/` folders for the target modules are present and consistent with their SPECS (run the `ideable-implement-specs` skill first if in doubt).
...
```bash
python scripts/dev/common/build_and_deploy.py
```
...
7. Copies per-module `docker-compose.yml`, `.env.config`, and `.env.secrets` into `deployment_root/modules/<MODULE>/`
8. Merges all module `.env.config` files into `deployment_root/.env.config` and all module `.env.secrets` files into `deployment_root/.env.secrets`
9. Generates the merged `deployment_root/docker-compose.yml`
10. Copies `scripts/runtime/` **whole** into `deployment_root/scripts/` — that folder is the definition of what a site gets, so there is no exception list to keep in sync: `create-merged-configuration.sh` and the `compose_merge.py` it uses, `authz.sh`, the backup scripts and `framework_version.py` are deployed because they are in it
11. Additionally places `start.sh`, `stop.sh` and `status.sh` at `deployment_root/`, where a devops runs them; each resolves the deployment root beside-or-above itself, so the copy inside `scripts/` works too
12. Writes `framework.lock.json` to `deployment_root/`, so the deployed `pull-hostapp-images.sh` can name the release the site runs. The lock is written through the resolver, so a lock disagreeing with the version `framework.env` asks for fails here. `framework.env` itself is **not** copied: a site inherits its release rather than choosing one

If registry publication is needed, run `scripts/dev/common/push_module_images_to_registry.sh` after the build step and pass the module names plus the tag you want to publish. That push script is the only step that tags and pushes images; it does not rebuild them.
...
2. Each enabled module has `deployment_root/modules/<MODULE>/docker-compose.yml` with `env_file: - ../../.env.config` and `- ../../.env.secrets`
3. `deployment_root/docker-compose.yml` (merged) exists and is up to date
4. `deployment_root/.env.config` and `deployment_root/.env.secrets` (merged from all modules) exist and are up to date — no slug-based env files (e.g. `.env.template`) should exist
5. `deployment_root/start.sh`, `deployment_root/stop.sh` and `deployment_root/status.sh` exist, are executable, and reference `modules/<MODULE>/docker-compose.yml` paths
6. The file set of `deployment_root/scripts/` equals the file set of `scripts/runtime/` — the folder is copied whole, so anything missing there is missing at every site. `create-merged-configuration.sh` is executable with `compose_merge.py` beside it, because it looks there and nowhere else
7. `deployment_root/framework.lock.json` and `deployment_root/scripts/framework_version.py` exist, and `deployment_root/framework.env` does **not**: a bundle is a copy of `deployment_root/`, so a site that cannot read its release from inside one cannot pull its images
8. No `Dockerfile` or `SOURCES/` path references exist anywhere inside `deployment_root/`
9. No hardcoded values in compose files where an env var is available
10. All Docker images referenced by `docker-compose.yml` exist in the local Docker registry (`docker images`)
...
| `docker-compose.yml`, `.env.config`, or `.env.secrets` changed | `docker compose down && docker compose up -d` from `deployment_root/` |
...
**Notes**:
- Always run compose commands from `deployment_root/` where `docker-compose.yml`, `.env.config`, and `.env.secrets` are located.
...
## Step 8 — Update the implementation plan

Resolve the active plan (the most-recently-modified `*.md` in `implementation-plans/`, per
`rules/implementation-plan.md`). If one exists:
- Set the Overall-view **Current step** to `Building & Deploying (ideable-build-and-deploy)`,
  move the graph highlight to the `BuildDeploy` node (rewrite the two `class` lines), and
  refresh **Last updated**.
- On a **failed** build or deploy, set the affected repo's Repos `Implementation` cell to
  `In error` and refresh the Status summary to say what failed.
- On success, leave `Implementation` as it is (build/deploy does not by itself make a thing
  `Done` — tests do) and refresh the Status summary if the deploy state is worth noting.

If no plan exists, skip silently and note it in the report — do not create one from this skill.
