...
- The **canonical palette values** (the single source of truth) live once in `reusable.ui/styles/base-tokens.css` as raw `:root` (light) + `.dark` tokens. The `.ideable-scope` cascade that maps them to `--ideable-*` (plus `data-lf` parity/branding) lives in `reusable.ui/styles/tokens.css`. Every prefix (`ideable:`, and each module's own) resolves to these same values — no consumer redefines them.
- **Brand a project by overriding token VALUES, never class names.** Widgets reference token names, so a rebrand touches only values:
  - **Build-time, per module:** set `[data-lf='module']` overrides (`--ideable-module-*` / `--<slug>-module-*`).
  - **Runtime, no rebuild:** edit `config/theme-override.css` in the deployed folder — an unlayered stylesheet loaded after the bundle (served `no-store`) that redefines `:root`/`.dark` values and wins the cascade for chrome and all widgets.
- Under `data-lf='hostapp'` (the default scope) the `--ideable-*` tokens resolve to the shared canonical `--*` tokens, so shared widgets match the surrounding app's L&F automatically.
...
- **Widget chrome** strings (`table.*`, `chart.*`, `auditTrail.*`, `common.*` used by widgets) belong to `@ideable/ui` and are resolved by the widget via `@ideable/ui`'s `useTranslation` (reads `localStorage['hostapp.language']` + the `hostapp:language-changed` event — works standalone and inside host). A module holds **none** of them and still renders a complete, translated widget.
- **Page-specific** strings belong to the module and are resolved by the **module's own** `useTranslation`. Pass page labels to widgets already translated (e.g. `ServerDataTable` column `header: t('templateItems.name')`). The module's bundle must carry every key its own code resolves — that hook has no library fallback.
- **Overriding a widget string: include that key in the module's own bundle.** Inclusion *is* the override, with nothing declared per key — the module's `useTranslation` calls `registerModuleMessages({ en, it })` once at import time, and the shared hook then reads the module's bundle before its own. So **never copy a library string you do not mean to change**: an identical copy overrides nothing and pins that wording the day the library improves it. `test_i18n_contract.py` reports every override **per language**, and names separately the keys copied verbatim in **every** language — the only ones that can be removed.
- Keep `en.json`/`it.json` in sync in every i18n bundle. They hold the same keys, so a string overridden in one language is still held in the other at the library's own wording: that row is required by key parity, which is why a copy is judged across all languages and not one at a time.
