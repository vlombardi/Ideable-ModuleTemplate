...
- **Overriding a widget string: include that key in the module's own bundle.** Inclusion *is* the override, with nothing declared per key — the module's `useTranslation` calls `registerModuleMessages({ en, it })` once at import time, and the shared hook then reads the module's bundle before its own. So **never copy a library string you do not mean to change**: an identical copy overrides nothing and pins that wording the day the library improves it. `test_i18n_contract.py` reports every override, and names verbatim copies separately.
