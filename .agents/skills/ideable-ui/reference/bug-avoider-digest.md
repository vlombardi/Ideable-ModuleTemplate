...
- **Never copy a library string into the module's bundle unless you are changing it.** Including a key IS overriding it (`conventions.md` § *i18n split*), so a verbatim copy overrides nothing today and pins the library's wording the day it improves. The module's bundle carries what the module's own code resolves — the widgets carry their own.
