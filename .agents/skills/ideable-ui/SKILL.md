...
The Ideable framework ships a single shared UI library, **`@ideable/ui`**, living in the top-level **`reusable.ui/`** folder (sibling of `modules/` and `deployment_root/`). It is the **single source of truth** for UI, Look & Feel (all CSS/design tokens), and widget definitions — host_app, `module_template`, and every remote module generated from it all consume the same widgets and the same canonical palette. **Reuse these widgets — never hand-roll a table, popup, dialog, chart, or primitive, and never redefine the shared palette locally.**
...
## Changing Look & Feel (branding) — never fork a widget

Rebrand by overriding design-token **values**, never class names (widgets reference token names). Two paths, neither of which edits framework-owned sources:
- **Build-time, per module:** set `--<slug>-module-*` (and `--ideable-module-*`) values and render under `data-lf="module"` (see `reference/conventions.md` and `reusable.ui/styles/tokens.css`).
- **Runtime, no rebuild:** edit `config/theme-override.css` in the deployed folder (`deployment_root/modules/<slug>/config/`). It is served `no-store` and loaded after the compiled bundle, so redefining `:root { --… }` / `.dark { --… }` there recolors your module and every `@ideable/ui` widget live. Logos/favicon/backgrounds are swapped by replacing files in the same `config/` folder. (Bundled icon glyphs need a rebuild.)

The canonical token values live once in `reusable.ui/styles/base-tokens.css`; the `.ideable-scope` cascade + `data-lf` parity/branding in `reusable.ui/styles/tokens.css`. Full role-based how-to (build time vs deploy time): `modules/module_template/frontend/SPECS/ideable-framework-specs/look-and-feel-branding.md`.
