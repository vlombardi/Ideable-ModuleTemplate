...
> `TEST_REPORTS/` entry — e.g. `IDEABLE_UNRECORDED_RUN=1 scripts/dev/common/tool.sh pytest modules/host_app/TESTS/`).
...
```bash
IDEABLE_UNRECORDED_RUN=1 scripts/dev/common/tool.sh pytest modules/host_app/TESTS/ -v --tb=short
```
...
Database suite only:
```bash
IDEABLE_UNRECORDED_RUN=1 scripts/dev/common/tool.sh pytest modules/host_app/TESTS/database/ -v --tb=short
```

Backend suite only:
```bash
IDEABLE_UNRECORDED_RUN=1 scripts/dev/common/tool.sh pytest modules/host_app/TESTS/backend/ -v --tb=short
```

Authentik + Traefik suite only:
```bash
IDEABLE_UNRECORDED_RUN=1 scripts/dev/common/tool.sh pytest modules/host_app/TESTS/authentik_traefik/ -v --tb=short
```

Authentik + Traefik endpoint-reference smoke tests only:
```bash
IDEABLE_UNRECORDED_RUN=1 scripts/dev/common/tool.sh pytest modules/host_app/TESTS/authentik_traefik/test_authentik_traefik.py -v --tb=short -k TestExternalEndpointReference
```
...
```bash
IDEABLE_UNRECORDED_RUN=1 scripts/dev/common/tool.sh bash -c 'cd modules/module_template/frontend/TESTS/playwright && npm ci && npm test'
```
...
```bash
IDEABLE_UNRECORDED_RUN=1 scripts/dev/common/tool.sh pytest modules/host_app/TESTS/frontend/ -v --tb=short
```
...
**Update the implementation plan.** Resolve the active plan (`rules/implementation-plan.md`
§ *Active plan resolution* — the plan named by an `ACTIVE PLAN - <description>.md` link in
`implementation-plans/`, its current file being the newest one bearing that description; an
unlinked or parked plan is not active). If one exists, set
...
`Fixing`. The router takes whichever arc the run's exit code selects — `./scripts/dev/common/dev-cycle.sh run`
is the only advance, and nothing in this skill moves the plan: a green run handed straight to
`Committing` would skip a node of the canonical graph.
