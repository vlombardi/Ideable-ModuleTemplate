...
> opt-in prefix `IDEABLE_ALLOW_DIRECT=1` (which runs the tests but writes **no**
> `TEST_REPORTS/` entry — e.g. `IDEABLE_ALLOW_DIRECT=1 pytest modules/host_app/TESTS/`).
...
> `IDEABLE_ALLOW_DIRECT=1`.** Without it, the repo-root `conftest.py` / `playwright.config.ts`
...
```bash
IDEABLE_ALLOW_DIRECT=1 pytest modules/host_app/TESTS/ -v --tb=short
```
...
Database suite only:
```bash
IDEABLE_ALLOW_DIRECT=1 pytest modules/host_app/TESTS/database/ -v --tb=short
```

Backend suite only:
```bash
IDEABLE_ALLOW_DIRECT=1 pytest modules/host_app/TESTS/backend/ -v --tb=short
```

Authentik + Traefik suite only:
```bash
IDEABLE_ALLOW_DIRECT=1 pytest modules/host_app/TESTS/authentik_traefik/ -v --tb=short
```

Authentik + Traefik endpoint-reference smoke tests only:
```bash
IDEABLE_ALLOW_DIRECT=1 pytest modules/host_app/TESTS/authentik_traefik/test_authentik_traefik.py -v --tb=short -k TestExternalEndpointReference
```
...
```bash
cd modules/module_template/frontend/TESTS/playwright && npm install && npx playwright install chromium && IDEABLE_ALLOW_DIRECT=1 npm test
```
...
```bash
IDEABLE_ALLOW_DIRECT=1 pytest modules/host_app/TESTS/frontend/ -v --tb=short
```
...
(`<n> passed / <n> failed / <n> pending`), and refresh the Status summary. Also set the
...
`pytest`/`playwright` run (even with `IDEABLE_ALLOW_DIRECT=1`) writes **no** `TEST_REPORTS/`
entry by design.
