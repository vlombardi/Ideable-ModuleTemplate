...
updates for its node (`Implementing`, `Fixing`, or `Documenting`) per
...
**At `Documenting`, one clause of this atom reads differently and only there.** *Propose spec
changes, never make them unasked* is the rule everywhere else; `Documenting` is the node whose
deliverable **is** the spec edit. It may **reconcile** — update a spec to match approved, tested
reality, a decision already taken when the change was implemented. It may not **legislate**: if
aligning would require deciding something the plan never decided, or the code contradicts a
contract the spec exists to impose, that is a conflict and the plan goes to `Asking human
direction`. See `rules/implementation-plan.md` § *Documenting*.
