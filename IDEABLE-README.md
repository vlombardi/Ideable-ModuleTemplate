...
```bash
./scripts/dev/module/adopt_framework.sh 1.6.3     # adopt a published release
./scripts/dev/module/adopt_framework.sh latest    # follow the channel
./scripts/dev/module/sync-template-updates.sh     # re-sync the release already named
```

**Naming a release is one command on each side.** The maintainer runs `publish_framework.sh <version>`; a project runs `adopt_framework.sh <version>`. The publish *writes* the two files that name a release — `framework.env` and `framework.lock.json` — and the adoption *installs* the same two, reading the lock straight out of the release's tag before the sync runs, so a project whose lock is missing or corrupt is repaired by adopting rather than stuck. Both write the version into `IDEABLE_FRAMEWORK_VERSION` themselves and then do the work, so neither side asks anyone to edit `framework.env` and remember to act on it — and if the sync fails, `adopt_framework.sh` puts the file back rather than leaving the project naming a release whose files it does not have. Run `sync-template-updates.sh` directly to re-sync the release the project already names.
...
- **Propagation to existing modules:** `scripts/dev/module/sync-template-updates.sh` idempotently injects the hook (the `index.html` `<script>`, the nginx `no-store` rule) and drops the `config/theme-override.css` scaffold when missing — without overwriting developer customizations — so modules generated before this feature gain runtime theming on their next sync (then one frontend rebuild to bake the hooks in).
...
- A declared dependency is a **prerequisite**: if the target is not enabled the build fails
  ...
  (`scripts/dev/common/module_deps.py`) validates the enabled set (target enabled, capability
  provided, **acyclic**) and orders modules **providers-first** (host_app first) for build.
- **Container startup:** at deploy time `scripts/dev/common/compose_deps.py` emits an *additive*
...
§5.1. Validate locally with `scripts/dev/common/validate_modules.sh` — besides resolving the
...
4. If it requires a skill directory → create a symlink from its skill directory to `.agents/skills/`, add the tool dir name to `TOOL_DIRS` in `scripts/dev/common/update_skills.py`.
...
7. Add its config directory/files to the `is_infrastructure` function in `scripts/dev/module/sync-template-updates.sh` and to the infrastructure copy list in `scripts/dev/master/push-updates-to-module_template-repo.sh`.
...
Implementing ──▶ Building & Deploying ──▶ Testing ──pass──▶ Documenting ──▶ Committing ──▶ Done ──▶ Merged
  ...
  Implementing *, Fixing *, Documenting *  ─────▶  Asking human direction
                                                   exits: Implementing, when the answer
                                                          grew the sub-set's scope
                                                          the step that asked, otherwise

  * — this step can ask for human direction.
```
...
- **Nodes / state.** Each node maps to a skill: `Implementing` (`ideable-implement-specs`),
  ...
  (`rules/implementation-plan.md`), so a reader sees at a glance where the run stands. Beside it the
  sub-set table says what each sub-set has **cost**: the router accrues an `Exec time` per sub-set at
  every transition, counting only the spans it spent executing, so a plan's own price is readable
  without reconstructing it from git timestamps.
- **Documentation is a step, not an afterthought.** `Documenting` sits between a green `Testing`
  ...
  deciding something the plan never decided goes to `Asking human direction`. And when the node
  discovers work rather than drift — a sub-set still holding a thing whose `Impl` is 🔲/🔄 — the
  plan goes back to `Implementing`, because what is unwritten has nothing for a document to
  describe. That is the
  first exit named on the ask's panel rather than an arc out of `Documenting`: what adds a row is a
  question and its answer, and the router chooses the route when it resumes an answered plan, by
  reading the executing sub-set's cells. The docs gate keeps its own route-back as the safety net
  for a sub-set that grew with no question behind it. `Documenting` never branches into the
  build/test loop: the fix for prose is prose.
- **`Done` and `Merged` are different claims.** `Done` is green, documented and committed on the
  ...
  `scripts/dev/common/dev-cycle.sh deliver`. The message is the plan's abstract either way (purpose, every
  ...
  itself and then deletes the plan branch. **Either route certifies the tree it lands**: when HEAD
  differs from the commit the newest recorded run names in any path that is not run bookkeeping,
  `deliver` runs the suite itself and refuses on red — before renaming the plan or moving the card,
  so a refusal costs nothing — which is why delivering needs no manual test run and no
  `SKIP_TEST_GATE`. See `rules/version-control.md` § *Delivering a plan*.
- **The gate / oracle is tests, not the LLM.** A thing/spec is `Done` only when its contract
  tests pass — never because "the skill finished". `scripts/dev/common/run_enabled_tests.sh` is the
...
- **Incremental by default.** `ideable-implement-specs` uses `scripts/dev/common/spec_workset.py`
...
- **Thin deterministic router.** `scripts/dev/common/dev-cycle.sh` reads the active plan, shows the current
  node and the recommended next transition (`status`), and runs the current node itself (`run` →
  tests / redeploy for the deterministic ones, the skill performed through the Claude Agent SDK for
  the LLM ones). **`run` is the only advance**: one command writes everything a transition must
  write — the graph, `Current step`, the executing sub-set's label, the sub-set advance, the
  `Commit` cells and the scope gate — so a plan cannot record its state somewhere `run` does not
  look, and no skill, agent or other command writes a plan's node. Who calls `run` changes nothing —
  an agent caller gets the node performed exactly as a human one does. What is refused is a
  **second worker on one plan**: every action that works a plan holds a lock on it for its duration,
  and a second one names the holder and exits rather than starting, because two workers edit the
  same tree and each then reads the other's diffs as unexplained change. The router **answers the
  agent it runs**: a permission the allow-list does not settle, or a question only the maintainer
  can decide, is put to the person who ran the command. A question nobody answers **blocks the
  plan** — `run` moves it to `Asking human direction` with the question and the node to come back
  to, puts it again on the next `run` or reads the answer written into the plan, and resumes once
  every question is
  answered: at the node that asked, or at `Implementing` when the answer left the executing sub-set
  holding an unimplemented thing. The prompt it gives each LLM node asks that node to write **the
  decisions it took** into the plan, because an agent's memory is never something the next node may
  rely on. **One agent per sub-set, not per node**: the session is resumed across the LLM nodes of a
  single sub-set, so `Documenting` and `Committing` judge the diff `Implementing` wrote rather than
  rebuilding what it means, while a **new sub-set starts a fresh agent** — the boundary where
  reading only the plan catches whatever the last agent believed and never wrote down. Each node is
  handed a **brief** (the node, its sub-set's rows, what is already decided, the last verdict, the
  diff) instead of being told to read a plan that has grown to six figures of bytes; the brief names
  the plan and does not replace it. The router also **skips a node whose input the sub-set's diff
  cannot touch** — a sub-set that changed no build input runs no `BuildDeploy` — and records the
  skip in the plan with its reason, because a step that silently did not happen reads exactly like
  one that did. The tests are never skipped and never scoped.
  This is deterministic scaffolding around non-deterministic steps, the same pattern the test runner
  already uses. **The router describes itself**: `./scripts/dev/common/dev-cycle.sh --help` is the
  current account of its actions, flags, environment variables and permission policies, and none of
  those is restated here — a second copy of a command's behaviour is a copy that goes stale
  (`scripts/TESTS/test_the_router_is_described_once.py`). The contracts it implements are
  `rules/implementation-plan.md` § *Git integration*.
- **Exactly one plan is active, and the router says which.** A plan becomes the active one by being
  **started**: `dev-cycle.sh start <description>` creates its `ACTIVE PLAN - <description>.md` link,
  and that link is what resolution reads — a plan file with no link is work somebody wrote down, not
  work in hand, and a parked plan never resolves as active even on its own branch. Because links are
  counted rather than files, one plan stays one plan however many files its history mode leaves
  behind (`rules/implementation-plan.md` § *History mode*). When **two** plans are still in flight — any state before `Done` — `run` and `status` name
  both and stop with a non-zero exit code rather than letting the newest timestamp win a decision
  nobody took, and `start` refuses to create the second one in the first place; `pause` and
  `deliver` are not refused, because they are how it is settled. See
  `rules/implementation-plan.md` § *Active plan resolution*.
- **A plan can be parked and picked up again.** `pause [<description>]` checkpoints the tree on the
  plan's branch, renames the plan `(Paused)` with the resume node still highlighted, and checks the
  delivery target out so the next plan branches from a clean base; `resume [<description>]` puts it
  back at the node it left. The two take a description symmetrically, so the remedy the two-plans
  refusal names reaches the plan the reader means; bare `pause` is still never refused, and reports
  which plan it parked. `Paused` is a filename state, not a node — it says nobody is working the plan,
  while the highlight keeps saying where the work is. A parked plan never resolves as the active
  one, which is what makes a pause hold even while standing on its own branch. See
  `rules/implementation-plan.md` § *Git integration*.
...
├── redeploy.sh  start.sh  stop.sh      ← the commands you type: relative SYMLINKS into
├── status.sh  dev-cycle.sh             ←   scripts/dev/common/, so the root holds
├── update_backend.sh                   ←   no script of its own
├── update_frontend.sh
├── scripts/                            ← two questions, one per folder: WHEN it runs, then WHO runs it
│   ├── dev/                          ← used while developing, building, deploying, publishing, syncing
│   │   ├── common/                   ← both roles, shipped
│   │   │   ├── build_and_deploy.py
│   │   │   ├── redeploy.sh           ← the dev-time wrappers: the repo root links to these six,
│   │   │   ├── start.sh              ←   (not to be confused with runtime/start.sh, a site's own)
│   │   │   ├── stop.sh
│   │   │   ├── status.sh
│   │   │   ├── update_backend.sh
│   │   │   ├── update_frontend.sh
│   │   │   ├── dev-cycle.sh          ← beside the router it wraps, and the seventh link
│   │   │   └── tool.sh
│   │   ├── module/                   ← module-maintainer projects, shipped
│   │   │   └── sync-template-updates.sh
│   │   └── master/                   ← the framework maintainer, never shipped
│   │       ├── write_framework_lock.py
│   │       └── push-updates-to-module_template-repo.sh
│   ├── runtime/                      ← IS the deployed scripts folder: copied whole to deployment_root/scripts/
│   ├── SPECS/                        ← specs for the runtime scripts
│   └── README.md                     ← the only file at the top of scripts/
├── rules/
...
├── redeploy.sh  start.sh  stop.sh      ← the same seven symlinks into scripts/dev/common/
├── status.sh  dev-cycle.sh
├── update_backend.sh update_frontend.sh
├── scripts/                            ← everything the framework has except dev/master/ and TESTS/
│   ├── dev/                          ← `master/` is absent: it is never shipped
│   │   ├── common/                   ← holds the real files those seven links point at
│   │   └── module/
│   │       └── sync-template-updates.sh  ← pull framework updates from template
│   ├── runtime/                      ← copied whole to deployment_root/scripts/
│   ├── SPECS/                        ← specs for the runtime scripts you received
│   └── README.md                     ← the catalogue of all of them
├── rules/
...
`scripts/dev/common/schema.sh` generates the migration that applies them and regenerates
...
- To publish images to a registry, run `./scripts/dev/common/push_module_images_to_registry.sh` **after** the build step.
- Choose the exact modules to publish and the tag to apply, for example:
  ```bash
  ./scripts/dev/common/push_module_images_to_registry.sh -a -t v1.2.3
  ./scripts/dev/common/push_module_images_to_registry.sh host_app module_template -t v1.2.3
  ```
- The push script is the only step that tags and pushes images to the registry; for a single-arch push it does not rebuild them (a multi-arch push rebuilds from `SOURCES`, which is how one manifest covers both architectures).
- **Publishing the framework itself is one command, not this one.** `scripts/dev/master/publish_framework.sh <version>` publishes the host_app images, the dev tools image, `framework.lock.json` and the template repository together under the version it is given — maintainer-only, and the only way those four stay in step.
...
├── start.sh / stop.sh / status.sh  ← runtime control scripts, also present under scripts/
├── framework.lock.json         ← the release this site runs; no framework.env, a site inherits its choice
├── scripts/                    ← scripts/runtime/ copied whole — the folder IS what a site gets
...
│   ├── authz.sh                ← authorization operations with no redeploy and no rebuild
│   ├── backup.sh / restore.sh / verify-backup.sh   ← and the rest of scripts/runtime/
│   └── framework_version.py    ← reads the lock beside it; needs --root at a site
└── modules/
    ├── host_app/
    │   ├── config/
...
**Sync brings the framework version your project asks for.** The one line in `framework.env` decides which: `latest` fetches the newest framework files (the channel), and `X.Y.Z` fetches the `vX.Y.Z` the framework tagged when it published that version. A pinned version the framework never published is an error, not a quiet fall-back to the newest — so a pin actually holds.

To move to another framework version, edit that line and sync:

```bash
# framework.env → IDEABLE_FRAMEWORK_VERSION=1.7.0
./scripts/dev/module/sync-template-updates.sh
./redeploy.sh                                     # carries the new lock to deployment_root/
./deployment_root/scripts/pull-hostapp-images.sh  # the digests that lock names
```

**The puller is a deployed script, and the deployed copy is the one to run.** A site reads its release from the `framework.lock.json` the deploy placed at the deployment root, through the resolver placed beside the scripts — so the puller works from a bundle that is nothing but a copy of `deployment_root/`. The source copy under `scripts/runtime/` deliberately cannot resolve anything: a script a site runs must not name a path that exists only in a checkout.
...
- `scripts/` — **everything under it except `dev/master/` and `TESTS/`.** One exclusion rather than a list, so which folder a file sits in decides whether you receive it. You get `dev/common/` and `dev/module/` (the build, development and sync scripts), `runtime/` (what a deployed site runs), `SPECS/` (the specifications for those runtime scripts) and the `README.md` that catalogues them all. `dev/master/` holds the maintainer-only scripts — the template push, the framework publish, the remote-shape checks — which is why `dev/` is split by who runs a script rather than kept as one folder; `TESTS/` is the framework's own suite, run where the scripts are authored. **A layout move arrives whole:** the sync removes the old locations in the same run that delivers the new ones, so no project is left holding two answers to which script runs
...
- `modules/host_app/.env.config` — **replaced whole, not merged into.** host_app is framework-owned in its entirety, and this file holds *structural* defaults: which container authentik's database is, which port a service listens on. Those change when the framework changes, and a merge would keep your old value — which is the one thing that has to move. Anything of your own goes in `project.env.config` instead: it is merged **first** and the first writer of a key wins, so a value declared there overrides the framework's default and survives every sync. The sync says so when it replaces the file, naming it and pointing at `project.env.config`
- `reusable.ui/` — the shared `@ideable/ui` widget library (widgets, primitives, precompiled `styles/compiled.css`, hooks, i18n)
- `framework.lock.json` — the digests published for your framework version. Generated by the framework publish and force-synced: it must match the version you ask for, and the resolver refuses to derive anything from a lock that does not.
- The seven repo-root commands (`redeploy.sh`, `start.sh`, `stop.sh`, `status.sh`, `dev-cycle.sh`, `update_backend.sh`, `update_frontend.sh`) — each a relative symlink into `scripts/dev/common/`, reproduced as a link by the sync

**What you keep:**
- `framework.env` — **the one line you own.** Sync never writes it; a sync that reset it would take away your only control over which framework you run. A new project receives it from `module-init.sh`, set to `latest`.
- `project.env.config` and `project.env.secrets` — **where your own values belong**, and the reason the entry above is safe. They are merged before every module's env, and the first writer of a key wins, so anything you set here beats the framework's default for the whole project — in the merged `deployment_root/.env.config` and in each per-module one alike. Only their `.example` files are synced, never these.
...
| Task | Command |
|------|---------|
| Initialize module | `./scripts/dev/module/module-init.sh MyModule` |
| Implement specs | `@[/ImplementSpecs]` |
| Build & deploy | `@[/Build&Deploy]` |
| Run tests | `@[/Tests&Fix]` |
| Full deploy | `./redeploy.sh` |
| Start services (root symlink) | `./start.sh` |
| Stop services (root symlink) | `./stop.sh` |
| Show status (root symlink) | `./status.sh` |
| Advance the dev cycle (root symlink) | `./dev-cycle.sh status` |
| Update backend only | `./update_backend.sh` |
| Update frontend only | `./update_frontend.sh` |
| Check template updates | `./scripts/dev/module/sync-template-updates.sh --list-changes` |
| Sync updates | `./scripts/dev/module/sync-template-updates.sh` |
...
- `scripts/dev/common/build_and_deploy.py` deploys per-module compose/env files to `deployment_root/`.
- A merged `deployment_root/docker-compose.yml` is generated for enabled modules.
- `deployment_root/scripts/` is `scripts/runtime/`, copied whole; `start.sh`, `stop.sh` and `status.sh` are additionally placed at the deployment root, where a deployer runs them.
...
| `FRONTEND_PUBLISH` | `3000:8080` | Same, host_app SPA | Container side is `8080`: the image is nginx-unprivileged, which cannot bind a privileged port |
| `TEMPLATE_BACKEND_PUBLISH` | `8002:8002` | Same, module API | |
| `TEMPLATE_FRONTEND_PUBLISH` | `3001:8080` | Same, module bundle | Container side is `8080`, same reason |
...
```
scripts/dev/common/loadtest.py "https://$EXTERNAL_BASE_HOST/module/template/api/items?limit=10" \
...
```
scripts/runtime/rolling-deploy.sh template-backend --dir deployment_root --project "$APP_SLUG"
```
...
- **Deploy tooling is single-host.** `redeploy.sh` and `build_and_deploy.py` target the local daemon;
  ...
  (`scripts/dev/common/push_module_images_to_registry.sh`) instead of local builds.
...
./scripts/dev/common/push_module_images_to_registry.sh -a -t 1.1.0
./scripts/dev/common/push_module_images_to_registry.sh host_app module_template
./scripts/dev/common/push_module_images_to_registry.sh -a -t 1.1.0 --platform linux/amd64,linux/arm64
./scripts/dev/common/push_module_images_to_registry.sh -a --single-arch
```

- **Without `-t`, only `latest` is published** — the work-in-progress channel, and the only name a local build has. **With `-t X.Y.Z`, every image is published under `X.Y.Z` *and* `latest`, from one build**, because a release is the newest thing published and `latest` must not become a different image that merely resembles it.
- **`-t` is refused twice**: from a dirty working tree (the bytes would come from a tree no commit describes, and `latest` would move to them), and when the registry already holds that tag (re-publishing a version changes what it means for everyone who already resolved it). Publish a new version instead.
