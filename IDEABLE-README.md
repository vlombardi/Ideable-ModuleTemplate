...
- **Thin deterministic router.** `scripts/dev/common/dev-cycle.sh` reads the active plan, shows the current
  ...
  automatically** through the Claude Agent SDK by default, and the router **answers that agent as it
  works**: a permission the allow-list does not settle, or a question only the maintainer can decide,
  is put to the person who ran the command (`DEV_CYCLE_AGENT_PERMISSIONS` sets the policy; `deny` is
  the default with no terminal). `--deterministic` opts out entirely — the router
