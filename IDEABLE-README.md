...
- Docker image builds stage `reusable.ui` into the frontend build context via each frontend's `SPECS/build.sh` (a repo-root sibling is otherwise outside the SOURCES-rooted context) and install it with `npm install --install-links`. The dev-only Widget Examples gallery is gated by the `WIDGET_EXAMPLES` build arg (`false` on publish → gallery + heavy deps tree-shaken out).
...
- **Module maintainers** author **module‑specific specs** (e.g. `module-ui-specs.md`, `module-bl-specs.md`, `datamodel.sql`) in their own module; they never edit framework‑owned guidance.
...
```
Implementing ──▶ Building & Deploying ──▶ Testing ──pass──▶ Committing ──▶ Done
     ▲                    ▲                   │
     │                    └───── Fixing ◀──fail┘
...
- **Nodes / state.** Each node maps to a skill: `Implementing` (`ideable-implement-specs`),
  ...
  `ideable-bugfixing-and-changes`), `Committing` (`ideable-commit-changes`). The current node is
...
- **Thin deterministic router.** `scripts/dev-cycle.sh` reads the active plan, shows the current
  ...
  the deterministic transitions itself (`run` → tests / redeploy). LLM nodes are **advised, not
  auto-run** — the human/agent stays in control (decision-authority rule). This is deterministic
  scaffolding around non-deterministic steps, the same pattern the test runner already uses.
