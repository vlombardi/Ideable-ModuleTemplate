...
It also renames the long-form repository docs to `IDEABLE-README.md` and `MODULE-README.md` in the exported template repo, then leaves placeholder `README.md` files in the repo root and module root for template users to customize.
...
| Sync `.env.example` and backfill `.env` | ✅ Automatic | Missing keys from `.env.example` are appended to the matching `.env` without overwriting existing values |
| Create `project.env` | ✅ Automatic | Copied from repo-root `project.env.example` if missing, then normalized for the current repo path |
...
Edit `project.env` and `modules/<NewModuleName>/.env`:

- **Project-wide identity** — `APP_SLUG`, `APP_NAME`
- **Module-local identity** — `MODULE_SLUG` inside each module's `.env` for module-specific naming and runtime isolation
...
1. Loads project-wide config first from `project.env`
2. Loads module `.env` files for module-scoped build/runtime variables
...
6. If you want to publish registry images, run `./scripts/common/push_module_images_to_registry.sh` separately with the modules and tag you want
7. Optionally executes `deployment_root/start.sh` to start the stack using the project namespace
...
├── .env                        ← merged project + module environment
...
4. **JWT emission.** When a user logs in, Authentik emits:
   - `<module_slug>.permissions` → permissions where `action != "menu_access"`.
   - `<module_slug>.menu_access` → menu resource codes for permissions whose action is `menu_access` (controlling shell navigation only).
...
- Claims are generated via a `Property Mapping` (not RBAC permissions), as a single Scope property mapping named “Ideable Permissions Claims”. This mapping emits the runtime claims needed by HostApp, including `hostapp.permissions`, `hostapp.menu_access`, and `hostapp.company_ids`, so every JWT contains the authoritative state at mint time.
...
3. **Use the standard claim layout.**
   - `<module_slug>.permissions` controls actions like `read`, `create`, `update`, `delete`.
   - `<module_slug>.menu_access` controls whether the menu item is visible.
   - `hostapp.company_ids` is used for company scoping and comes from Authentik user attributes.

4. **Map claims to UI behavior.**
   - Hide menu entries when the matching `*.menu_access` claim is missing.
...
- `menu_access` permissions (`action == "menu_access"`) are emitted to `<module_slug>.menu_access` claim, not to `<module_slug>.permissions`.
...
Menu items declared in `config/menu_definition.json` are shown or hidden based on the `<module_slug>.menu_access` claim.
...
HostApp shows the menu entry when `authorization_resource` (here `"items"`) is present in the `<module_slug>.menu_access` array of the JWT. That array is populated when the user has a permission with `resource == "items"` and `action == "menu_access"` — i.e., `inventory.items:menu_access` in the merged plan.
...
|---|---|---|
| `MODULE_SLUG` | module `.env` | Your module's slug (e.g. `inventory`). Must match `module.json`. |
| `AUTHENTIK_JWKS_URL` | project `.env` / module `.env` | JWKS endpoint. Use internal Docker URL in production: `http://authentik-server:9000/application/o/<APP_SLUG>/jwks/`. |
| `AUTHENTIK_ISSUER` | project `.env` | Optional. If set, must equal the `iss` claim exactly: `https://<host>/application/o/<APP_SLUG>/`. |
| `AUTHENTIK_API_URL` | project `.env` | Authentik REST API base (needed only if your module calls Authentik directly). |
| `AUTHENTIK_API_TOKEN` | project `.env` | Bootstrap token reused as API token. Equal to `AUTHENTIK_BOOTSTRAP_TOKEN`. |
| `EXTERNAL_BASE_HOST` | project `.env` | Public hostname. Used to construct OIDC/JWKS URLs. |
| `APP_SLUG` | project `.env` | Authentik application slug (`hostapp`). Used in JWKS and OIDC paths. |
...
| Using `allowed_roles` lists for menu visibility | Use `authorization_resource` in `menu_definition.json` matched against `<module_slug>.menu_access` claim |
...
When a module has already been built locally, you can push its Docker images to the registry configured in `project.env` via `DOCKER_REGISTRY`.

Use one of the following commands:

```bash
./scripts/common/push_module_images_to_registry.sh -a
./scripts/common/push_module_images_to_registry.sh -a -t 1.1.0 --multi-arch
./scripts/common/push_module_images_to_registry.sh HostApp ModuleTemplate
./scripts/common/push_module_images_to_registry.sh -a -t 1.1.0 --multi-arch --platform linux/amd64,linux/arm64
```

- `-a` / `--all` pushes every enabled module that is declared as `enabled` in `modules/enabled.md`.
...
- Modules marked as `enabled-remote` are skipped, because their registry push must be done from the owning project.
...
- `--multi-arch` switches the push flow to `docker buildx build --push` and publishes a multi-architecture manifest instead of tagging the local image.
- `--platform` sets the comma-separated platform list for `--multi-arch` and defaults to `linux/amd64,linux/arm64`.
...
The script reads `project.env` first and defaults `DOCKER_REGISTRY` to `ghcr.io` when the variable is not set. When the value is just `ghcr.io`, the script expands it with the repository owner before tagging and pushing.

## Quick Start

1. Copy `project.env.example` to `project.env` if needed, then configure project identity there.
2. Configure enabled modules in `modules/enabled.md`.
3. Check module settings in `modules/<Module>/.env`.
