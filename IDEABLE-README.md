...
**Naming a release is one command on each side.** The maintainer runs `publish_framework.sh <version>`; a project runs `adopt_framework.sh <version>`. Both write the version into `IDEABLE_FRAMEWORK_VERSION` themselves and then do the work, so neither side asks anyone to edit `framework.env` and remember to act on it — and if the sync fails, `adopt_framework.sh` puts the file back rather than leaving the project naming a release whose files it does not have. Run `sync-template-updates.sh` directly to re-sync the release the project already names.
