...
- Consistent L&F — widgets inherit host_app's design tokens automatically, so a remote looks native inside the host with no styling work.
...
- **Branding** = override token *values*, never class names (`.ideable-scope[data-lf='hostapp']` inherits host tokens; `[data-lf='module']` allows module overrides).
